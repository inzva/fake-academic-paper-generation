...
\end{abstract}
%   We elaborate on the fact that the back propagation process in current single stream CNN to LSTM mapping approaches ``go blindly'' with the dense inputs from the convolutional layer and we can improve the process with the aid of proposed two stream approach where we include a seperate LSTM stream with ``important clues'' to discriminate and then jointly propagate between the streams. 
...
%But in \cite{Baccouche2011} authors have shown that LSTM networks are more suited for modelling long term relationships in the sequences. Therefore, predicting action classes while observing one or two frames is sub-optimal compared to predicting action classes after observing ten or twenty frames. Therefore, in contrast to \cite{Baccouche2011,Jeff2015} we define the action recognition as a sequence-to-one prediction problem, where we observe sequence of video frames before predicting an action class. 
...
% Our action recognition model is based on the joint training of a convolutional neural network together with an LSTM network. Fig.\ref{fig:fig1} outlines our approach. At each time step a fixed number of video frames are provided as an input. Then the RGB pixel values from each frame is fed to the convolutional neural network. The output of a last dense layer has been extracted (size of $1 \times{4096}$) and is fed to the LSTM network. 
% 
%  \begin{figure}[h]
%        \begin{flushleft}
%        	\includegraphics[width=0.5\textwidth]{arc.pdf}
...
%    In a study by Zhu et al. \cite{Zhu2016}, have used a method called hyper-column features for a facial analysis task. This concept of hyper-columns is based on extracting several earlier CNN layers and several later layers, instead of extracting last dense layers of the CNN. The main reason that leads to hyper-columns is that the information provided by the last layers are considered as semantic information which does not hold any relation to the spatial information of the input. However, these type of methods are more relevant for the tasks where the localisation is necessary. But in our case action localisation is not important. Therefore we extract the output of the first dense layer of the CNN network with the size of $1 \times{4096}$, to feed it as the input for the LSTM network. To extract the sequential data required by the LSTM, we provide the CNN outputs of t frames per video, extracting t temporally sequential deep feature vectors to feed as input to the LSTM.     
...
%	\caption{LSTM Layer Architecture}
%	\label{fig:fig2}
%        \end{flushleft}
%  \end{figure}
%
%
%Fig.\ref{fig:fig2} outlines the basic architecture of a recurrent neural network that we have used in our study. The functionality has been described as follows.
% The motivation behind the usage of an LSTM network is to model the temporal relationship between the salient features detected by CNN network.
%
%\begin{equation}\label{eq:1}
%f_t = sigmoid (W_f .[h_{t-1},x_t] + b_f)
%\end{equation}
%
%The first sigmoid function of the LSTM is recognised as the `forget gate' of the LSTM cell. It takes both the input $x_t$ and the previous layer output $h_{t-1}$ and output a number between 0 and 1 where 0 indicates to forget the input value and 1 indicates to consider them. In our case $x_t$ is the $1 \times{4096}$ feature vector describing the $t^{th}$ frame of the video.
%
...
%\bar{S_t} = tanh (W_s .[h_{t-1},x_t] + b_s)
%\end{equation}
%
%
%The next sigmoid function is called the 'input gate' which decide whether to update each variable values. Then the first tanh layer outputs a value that can be added to the state. Then both values will be used to update the state value depending on the output of the forget gate. 
%
%\begin{equation}\label{eq:4}
%S_t =f_t \times{S_{t-1}} + i_t \times{\bar{S_{t}}}
%\end{equation}
%
%The third sigmoid function decides the portion of the cell state that can be added to the final output $h_t$. Then the cell state value is sent through the tanh function to compress the value to keep between the range [-1,1]. Then the output $h_t $ contains the product of both outputs from the sigmoid and the tanh functions.  
%
...
%\end{equation}
%
%Once the sequence reaches the final cell of the LSTM network, corresponding hidden state is passed through a fully connected layer to get the final class label.
...
% \begin{figure}[!h]
%        \begin{flushleft}
%        	\includegraphics[width=0.45\textwidth]{conf_mat/conv_L_ucf11.pdf}
%	\caption{Confusion matrix of our \textit{conv-L} for UCF11 dataset with 11 action classes}
%	\label{fig:cnfmat_ucf11_cov}
%        \end{flushleft}
%  \end{figure}
%  
%  \begin{figure}[!h]
%        \begin{flushleft}
%        	\includegraphics[width=0.45\textwidth]{conf_mat/dense_L_ucf11.pdf}
%	\caption{Confusion matrix of our \textit{fc-L} for UCF11 dataset with 11 action classes}
%	\label{fig:cnfmat_ucf11_fc}
%        \end{flushleft}
%  \end{figure}
...
%    \hline
%    brush\_hair & catch & clap & climb\_stairs & golf &  jump & kick\_ball & pick & pour & pull\_up & push \\
%    \hline   
%     93.33 & 81.25 & 47.62 & 75.00 & 61.54 & 46.33 & 63.33 & 46.33 & 75.00 & 82.76 & 80.00   \\
%    \hline
%    \hline 
%      run & shoot\_ball & shoot\_bow & shoot\_gun& sit & stand & swing\_baseball & throw & walk & wave \\
%    \hline
%      44.33 & 75.42 & 92.59 & 88.24 & 56.25 & 37.50 & 69.23 & 92.86 & 46.67 & 47.62\\
%     \hline
%  \end{tabular}}
%  \end{center}
%  \caption{Accuracies of different action classes in jHMDB dataset}
%  \label{tab:tab_3}
%\end{table*}
