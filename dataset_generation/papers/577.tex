...
\begin{center}
    ...
    %\vspace{-0.2in}
    %\captionof{figure}{{\bf Video object segmentation by unsupervised learning of a dictionary of deep visual words as object representation.} The proposed method meta learns a dictionary of deep visual words for representing objects in the video (bottom), in order to perform video object segmentation (top). Note that no ground-truth for the visual words are provided at training or test time. The algorithm yields a meaningful set of visual words, which implicitly represent object parts, and are robust over time.
...
%This is a form of meta learning where the model learns to learn the visual words from the first of the video, to learn a bett for objects, to minimize the pixel-wise classification loss over the video.  a set on their object parts and then performs object segmentation by matching pixels to object parts, to using a dictionary of deep visual words and then classifies each pixel by assigning it to the most relevant word. Since no ground-truth information is available for the visual words, we design an iterative meta-training algorithm by performing unsupervised learning of deep visual words, followed by supervised learning of object segmentation. Our experiments show that the proposed method performs on-par with state-of-the-art methods, while being much faster.
%Meta learning, or learning to learn, has attracted a lot of attention recently. In this paper, we propose a fast and novel meta learning based method for video object segmentation that quickly adapts to new domains without any fine-tuning. The proposed model represents objects using a dictionary of deep visual words and then classifies each pixel by assigning it to the most relevant word. The model learns to learn the visual words for objects, to minimize the pixel-wise classification loss over the video. Since no ground-truth information is available for the visual words, we design an iterative meta-training algorithm by performing unsupervised learning of deep visual words, followed by supervised learning of object segmentation. Our experiments show that the proposed method performs on-par with state-of-the-art method, while being much faster.
...
%%Meta learning, or learning to learn, is a well-established approach to the problem of learning from a few training examples, a.k.a few-shot learning problem~\cite{Hugo, NIPS2016_6385, NIPS2017_6996, Schmidhuber, bengio:1995:oban}. By analyzing the learning process over multiple tasks, a meta learning algorithm learns how to learn a new but similar task quickly and more efficiently.%The field of meta learning has attracted a lot of attention recently \cite{Freitas, vanschoren2018metalearning, Hugo, NIPS2016_6385, NIPS2017_6996, MAML} in the machine learning community. 
...
%Semi-supervised video object segmentation, which is defined as the task of segmenting one or multiple objects in videos, given their ground-truth masks only in the first frame, is often formulated as a one-shot problem \cite{OSVOS, Newswanger, masktrack}. %A one-shot video object segmentation system should be able to learn all about the objects very quickly, i.e., just from the first frame.%This problem comes in the limited data regime, as the classifier is shown the ground-truth segmentation masks of the objects only in the first frame, and is expected to segment the objects in the rest of the video.
...
%Moreover, even a static object could be composed of quite dissimilar object-parts, and forcing them to be represented by one prototype may lead to an overfitted representation.
...
%Fig.~\ref{fig:teaser} demonstrates that the proposed approach delivers meaningful deep visual words, which implicitly represent object parts.
...
%These approaches achieve good performances and are quite useful for offline applications.
...
% In contrast, our method is computationally much more efficient as it does not need any fine-tuning. Furthermore, it enables objects to be represented by newer visual words on demand, as they change in shape or pose over time.%In this work, we propose a novel meta learning based approach for quick adaptation in video object segmentation, which inherently captures the intra-class variance, and also accommodates for class adaptation in time, without much computational effort.% As shown in the experimental results, our model is very simple and intuitive and does not include sophisticated modules that are often incorporated in more complex methods. Nevertheless, it performs on par with them by achieving a promising score of $\mathcal{J}\&\mathcal{F}=67.3\%$ on DAVIS-2017, without any fine-tuning on the first frame of the video, while running at $\sim$ 3.5 fps, which is 1-2 orders of magnitude faster than them as shown in Fig.~\ref{fig:chart}. 
...
%In this work, we propose a novel meta learning based approach for quick adaptation in video object segmentation, which inherently captures the intra-class variance, and also accommodates for class adaptation in time, without much computational effort.%The proposed model yields a robust object representation using a dictionary of deep visual words, as shown in Fig.~\ref{fig:teaser}.%Eventually each pixel is classified by assigning it to its closest visual word from the dictionary.% and incremental \footnote{By incremental we specifically mean that the algorithm is capable of processing its input frame-by-frame, without having the entire input available from the start.}%The problem of quick adaptation for few-shot learning has been addressed in the machine learning community using meta learning~\cite{vanschoren2018metalearning, MAML, NIPS2016_6385, NIPS2017_6996}. %For instance, Prototypical Networks \cite{NIPS2017_6996} is a state-of-the-art meta learning approach, designed for few-shot classification without fine-tuning, which performs on-par with the methods that involve fine-tuning~\cite{MAML}. In particular, it learns to learn one prototype as an approximate representation for class-conditional data distributions, which leads to a faster classification on unseen examples. This method, however cannot learn a sufficiently good representation in high-dimensional structured output prediction problems, such as video object segmentation, as shown in our experiments. \paragraph{Contributions:}\mbox{}\\
...
%The fine-tuning process leads to significant improvements in performance as reported by \cite{OSVOS, onavos, hu2017maskrnn, Cheng_2017_ICCV, masktrack, lucid, Yoon_2017_ICCV, Ci_2018_ECCV}, but this boost comes at the cost of dramatically slowing down the algorithm run-time, which makes them unsuitable for online applications such as autonomous driving and robotics. 
...
%{\it MaskTrack}~\cite{masktrack} proposed to leverage temporal information via mask propagation, i.e. guiding the segmentation network with the mask from the previous frame.%%Jampani et al.~\cite{Jampani_2017_CVPR} presented {\it Video Propagation Networks} ({\it VPN}), in which they used bilateral filtering~\cite{bilateral} for propagating information through time.%Oh et al. \cite{Oh_2018_CVPR} proposed {\it Reference-Guided Mask Propagation}, where the mask propagation network is augmented with a refinement module using residual connections to refine segmentation outputs. %Methods based on mask propagation are vulnerable to object occlusion and may lose the track of objects at some points. %To overcome this limitation, Li et al.~\cite{Li_2018_ECCV} augmented MaskTrack with a re-identification module that traverses the video back-and-forth to recover any potential missed object prediction. This approach, as well as some other methods that analyze all video frames together~\cite{Koh_2018_ECCV, Yang_2017_CVPR}, however, may not be practical for streaming and online purposes, where the decision on a frame should be made only based on the current and previous frames. %%
...
%Han et al. \cite{Han_2018_CVPR} formulated the video object segmentation problem as a Markov decision process and applied reinforcement learning followed by an FCN, to segment the objects.
...
%Furthermore, they still have to fine-tune their model on the first frame to properly adapt to the test video and perform on par with state-of-the-art methods, which slows down their approach in consequence. On the contrary, our method is much faster than state-of-the-arts and yields promising results without any parameter update or fine-tuning process. 
...
%In other words, we learn these visual words to learn a better pixel-wise object classifier.\iffalse\section{Related Work}\label{sec:relwork}\paragraph{Video object segmentation papers}\mbox{}\\
...
%%Pixel Level Matching and reference-guided mask propagation
...
% Mask track based papers%MaskTrack \cite{masktrack} Propose to guide current segmentation network to segment a particular object by both online fine-tuning on the first frame and by feeding in the mask from the previous frame. They specifically train the model in offline fashion to segment object of interest given the image and a rough estimate of the mask, thus using static images for this task. Mask-Track: Along with the  rgb channel, they also feed a previous frame mask channel. They can generate a lot of artifical pair data for training this kind of thing. They also use online fine-tuning using lucid to fine-tune it on first frame. They call it guided/conditioned instance segmentation problem. How to tell network which instance to segment? 1. guided 2. online fine-tuning.%Li et. al. \cite{Li_2018_ECCV} proposed to formulate the problem as one of mask propagation and re-identification. The mask propagation first warps the mask onto the next frame using optical flow, and then refines it using a mask-net. The mask net has 2 streams, the first stream takes as input the bounding box from the image and the bounding box of warped mask, and the second stream takes as input the optical flow instead of the rgb image. The two streams are later combined. The re-identification module is used when the object is lost by the mask-propagation module, for eg. after occlusion. faster-rcnn gives bounding box proposals and the one with a high re-identification score is selected if it was occluded before and then mask propagated bi-directionally. Drawbacks: Have to do it independently for each object, won't scale very well.offline. Good-points: we can use optical flow as input to network. We can also use re-identification network.%Tokamkov et al. \cite{Tokmakov_2017_ICCV} introduced a visual memory module which captures the evolution of objects over time alongside capturing spatial and temporal iformation and treat it as a fg/bg problem.%Cheng et al. \cite{Cheng_2017_ICCV} showed how sharing information by simultaneously predicting both optical flow and segmentation can help improve results for both tasks. They use ResNet-101.%Yeo et al. \cite{Yeo_2017_CVPR} first generate superpixels using SLIC and then construct a graph using MRF to connect them in time.%Yang et al. \cite{Yang_2017_CVPR} proposed to use multiple granularities for the task. They use bounding box trackers, superpixel tracking to get coarse segmentation and then on top graph-cut is applied to get fine pixel-wise segmentation.
Jang et al. \cite{Jang_2017_CVPR} use three decoders and then a MRF on top.
%Ci et al. \cite{Ci_2018_ECCV} propose to deal with the problem of similar looking objects by using location sensitive embeddings. To tackle with appearance changes in an object over time, they pre-scan entire video and generate pseudo fg/bg samples and retrain the model on these.%Koh et al. \cite{Koh_2017_CVPR} propose a Primary object segmentation method(POS) by first generating primary pool of candidate regions and then using recurrence property. These are later refined by reduction and augmentation with missing parts.%Jain et al. \cite{Jain_2017_CVPR} use a separate motion and apppearance branch and then fuse them to combine information in both.%Jampani et al. \cite{Jampani_2017_CVPR} use bilateral filtering to propagate information till current time to next time frame, and then use spatial network on top of it.%OSVOS \cite{Caelles_2017_CVPR} propose to fine-tune a segmentation model on the first frame. They get results without any temporal consistency. They also use a separate contour branch.%OnAVOS \cite{onavos} propose to do online updation on top of the OSVOS model. For online update, they get high conidence pixels from their own prediction and mix them with first frame pixels to avoid the model from drifting while updating.%Bao et al. \cite{Bao_2018_CVPR} proposed to use a spatio-temporal MRF. Their main idea is that the temporal dependencies are not utilized properly and each frame is considered individually. They propose a nice solution for achieving this and show how it can give huge improvements.%Yang et al. \cite{Yang_2018_CVPR} deal with fast adaptation. They propose to keep a separate modulator network that can tell how to modify the weights, they use three modulators, conditional batch normalization to modify batch-norm layers, a spatial modulator and a visual modulator. Not at all intuitive.%Koh et al. \cite{Koh_2018_ECCV} propose to first find instrances in individual frames by using instance segmentation method, and then linking them in time. Thus not relying too much on spatial or temporal coherence, which they believe they cause problems.% unsupervised methods%Some unsupervised methods have also been proposed \cite{Li_2018_ECCV_un, Hu_2018_ECCV_un, Li_2018_CVPR} %motion%Some people have tried to use motion information efficiently \cite{Hu_2018_CVPR, Xiao_2018_CVPR}. This is something that we have not tried yet. This is complementary to our work and can further help improve performance. They first use motion information to extract the foreground object, and then refine this prediction.%re-inforcement%Han et al. \cite{Han_2018_CVPR} formulate it as a markov decision process and apply re-inforcement learning algorithms.\paragraph{The part based tracking papers and bag-of-words model}
...
%In video object segmentation, each video presents a task $\mathcal{T}$ of transferring the labels from the first frame (hereafter referred to as the support set $S$) onto the subsequent frames (hereafter referred to as the query set $Q$).%Let us denote by $\mathcal{T}^{tr}_i$ the train tasks of video object segmentation and by a new task at test time. %Thus, our meta-learning objective is to learn how to transfer the labels from S -> Q such that given S we can learn to better predict the labels for Q.%Thus, by learning how to transfer these labels from the $S_i$ to $Q_i$ where $i$ in 
...
%to learn model parameters $\theta$ such that the obtained model performs well across a variety of tasks(videos), which are sampled from the distribution $p(\mathcal{T})$ of training tasks (i.e. meta-training set). 
Let the performance of the model on the $n^{th}$ task, $\mathcal{T}_n$, be rendered by the loss $\mathcal{L}_{\mathcal{T}_n}(\theta)$. 
%$\mathcal{L}_{\mathcal{T}_n}(\theta)$ is defined as a function of model parameters, and computed given the predicted labels and ground-truth labels of the query pixels, as defined in Sec.~\ref{sec:training}.
Then, the meta-training objective becomes:

% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%\begin{align*}
\theta^*
&= \argmin_\theta \sum_{\mathcal{T}_n \sim p(\mathcal{T})} \mathcal{L}_{\mathcal{T}_n}(\theta).
%\\ &= \argmin_\theta \sum_{\mathcal{T}_n \sim p(\mathcal{T})} \sum_{t \in \mathcal{T}_n} \mathcal{L}_{\mathcal{T}_{nt}}(\theta)
\label{eq:loss} \numberthis
...
%, where the support and query sets for each task are indicated. 
In this setup, the support set $\mathcal{S}$ is the set of all labeled pixels in the first frame, $ \mathcal{S} = \{ x_i, y_i \}_{i=1}^{N}$. Here $x_i$ represents the pixel $i$ in the first frame, $y_i \in \mathcal{C} = \{ 1,...,C \}$ is the ground truth class label of pixel $x_i$, $N$ is the number of labeled pixels in the frame, and $C$ is the number of object classes that need to be tracked and segmented in the video. Similarly the query set is defined by $ \mathcal{Q} = \{ x_j, y_j \}_{j=1}^{N\times F}$,  where $x_j$ denotes pixel $j$ from the video, $y_j$ represents the ground-truth class label for pixel $j$, and $F$ is the number of frames in the video (excluding the first frame). 
The output of each task $\mathcal{T}$ is the set of predicted class labels for the pixels in $ \mathcal{Q}$, i.e. $\mathcal{\hat{Y}} = \{\hat{y}_j\}_{j=1}^{N\times F}$. 
\fi
...
%In order to predict the object class for each pixel in the query set $\mathcal{Q}$, we need to learn a representation for each object, using the information provided in the support set $\mathcal{S}$. In this paper, we propose to represent the objects in each video using a dictionary of deep visual words (Fig.~\ref{fig:ourmodel}). Each pixel in the query set is then classified into one of the object classes, based on the deep visual word it is assigned to. This process is described in more detail in Sec.~\ref{sec:unsupervised} and Sec.~\ref{sec:supervised}. 
...
%In order to predict the object class for every pixel in the query set $\mathcal{Q}$, we learn a multi-class classifier for each task, given the information provided within the support set $\mathcal{S}$. In particular, we propose to represent every object class that is present in the support set using a dictionary of visual words. During the meta-training phase, our model learns to learn a better classifier by optimizing these visual words. \HB{elaborate this. It should look non-trivial from here only.}%We argue that \cite{MAML, NIPS2017_6996} are on two extreme ends of the meta-learning spectrum, and our method attempts at filling this gap by providing a degree of freemdon  a m using non-parametric classifiers.%object is represented as a bag-of-codewords. Firstly, we get the bag-of-codewords from the support set. %Now on the query set, we want to find the class for each pixel. We use each bag-of-codwords to determine the class.%Most recent methods formulate video object segmentation as a one-shot problem. On the other hand, we formulate it as a few-shot problem. This comes from the fact that segmentation can be thought of as a pixel-wise classification problem, thus ground-truth segmentation on the first frame provides multiple labeled examples for each object.%There are two key components in our model: first is to automatically learn the group of clusters to represent the object with, from the support set; and second is to find out for a particular pixel in each frame of the query set, which object it belongs to.%\paragraph{Generating visual words from $\mathcal{S}$:}\mbox{}\\\subsubsection{Unsupervised Learning of Deep Visual Words}\label{sec:unsupervised}
...
%This is in contrast to prototypical networks~\cite{NIPS2017_6996}, where the entire distribution of the data points is represented by only one single prototype vector, which leads to a considerable information loss and performance drop, as shown in our experimental results. \HB{Write difference better}%\paragraph{Retrieving the respective class of each pixel in $\mathcal{Q}$:}\mbox{}\\\subsubsection{Supervised Learning for Pixel Classification}\label{sec:supervised}%Let $\mathcal{C}_l$ be the set of non-overlapping regions/clusters that span object of class $l$, where $l \in \{1, 2, \dots, L\}$, and $L$ is the number of objects (including background) that should be tracked and segmented throughout the video sequence. 
...
%\HB{There are 2 directions of information transfer. 1. For updating model parameters, we use the unsupervised deep visual words (as pseudo labels, we rely on them. This is one direction of information transfer).2. The visual words are getting updated with time because of the knowledge getting transferred from the ground truth segmentation.}%\HB{Thus, in each new video(train or test time), model learns deep visual words from the first frame, in an unsupervised manner. During meta-training, model learns to learn deep visual words from the first frame to minimize a object segmentation loss over the rest of the video.}%In other words, the probability of assigning pixel $x_j$ to object class $c$ is determined by assessing the most similar visual word from that object to pixel $x_j$.%We argue that, it is sufficient for a pixel to be very similar to at least one of the visual words within the object, in order for that pixel to be labelled as that object class, thereby the maximum operation in Eq.\ref{eq:label_prob} is justified. The key point is that we allow the model to account for intra-class variations by corresponding a pixel to only one relevant part of object, rather than forcing it to resemble all object parts. \HB{ellaborate this}%One could use a soft nearest neighbour classifier to match the query pixels with the examples in the support set, as in \cite{Hu_2018_ECCV}. 
...
%Furthermore, unlike \cite{DAVIS2018-Interactive-2nd}, where the support set is reduced to a sparse set of training samples using a stochastic pooling layer, we do not discard any data points. 
...
%Note that this is in accordance with \cite{NIPS2016_6385}, where {\it full context embedding} has been utilized to link between the examples in the support set, using a bidirectional Long-Short Term Memory (LSTM)~\cite{lstm}. 
The model architecture is described in more detail in Sec.~\ref{sec:exp}.

%Multimodal networks are essentially learning a classifier $c_\mathcal{S}(x_i)$ from the support set $\mathcal{S} $. %We look at the nature of this classifier in Sec.\ref{sec:ME}.%\subsection{Training}\label{sec:training}%One of the main contribution of our paper is to formulate the problem of video object segmentation as a meta learning problem, and successfully apply it. %Training the proposed model is not very straightforward as we do not have access to the ground truth visual words for the pixels during training. %This means that we do not know what clusters the objects in the support set are composed of, which makes this an ill-posed problem. Furthermore, the cluster assignments for pixels in the query set are not known either.%To address this problem, we treat cluster assignments as latent variables and try to optimize them by progressively%updating k-means clustering given the updated network parameters \cite{Caron_2018_ECCV}. The cluster construction and assignment is thus done, based on similarity as function of current representation space, and this representation space keeps updating during training. \HB{Unclear}%\subsection{Label Transfer}\label{sec:labeltransfer}%Once the model is properly trained, it could be used at test time to segment and track object(s) of interest throughout a video. More specifically, we initially compute a set of visual words from the pixels in the first frame, using Eq.~\ref{eq:unsupervised}. Next, class labels of these visual words are transferred to all the pixels in the query set (i.e. the rest of the video), according to Eq.~\ref{eq:label_prob}. Note that the class label of each visual word is known since we have access to the pixel-wise ground-truth masks of the first frame. Furthermore, we perform online adaptation at every five frames, as described in Sec.~\ref{sec:onlineadaptation}, to gradually update the set of reference visual words throughout the video as the shapes and appearances of the objects and surrounding scene change.%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%\paragraph{Loss Function}\mbox{}\\%The loss function for a task in Eq.\ref{eq:loss} is calculated over the query set and is the sum of losses over all the frames in the query set for that task:%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%\begin{equation}%\mathcal{L}_{\mathcal{T}_n} = \sum_{t \in \mathcal{T}_n} \mathcal{L}_{\mathcal{T}_{nt}},%\label{eq:loss_all}%\end{equation}%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%where $\mathcal{L}_{\mathcal{T}_{nt}}$ is the loss over $t^{\text{th}}$ frame in the query set, and is defined as:%Training is fully end-to-end because the loss is computed on the query set. Thus, once the prototypes are computed from the support set, they can be treated as fixed for the corresponding query set. **make it look end-to-end in figure as well, maybe give a new name to the layer.**%It is important that the different modes do not dissolve into one mode, but this is automatically taken care of by the k-means algorithm.%%%%%%%%%%%%%%%%%%%%%%**describe about how unsupervised learning is used here** %{\color{blue} DO we need the following paragraph?} It is important to note that, during training, we extract knowledge in an unsupervised fashion and use it as pseudo supervision, on top of the supervision available. %Thus our training algorithm is a combination of supervised and unsupervised techniques. It can be seen in the ablation studies that this gives improvements in results, compared to a fully-supervised approach \cite{NIPS2017_6996}. This shows that our algorithm is able to extract more knowledge and learn better with the same amount of supervision.%*Multiple tasks can be made from a single video*%* \cite{NIPS2016_6385}.*\iffalse\begin{algorithm}[bt!]
...
%via $y_i^\delta = \argmax_{c}p(y_i=c|x_i^\delta)$, and $p(y_i=c|x_i^\delta)$ is computed using Eq.~\ref{eq:label_prob}. 
...
%In the next few sections, we first delve into our model design and implementation, then explain the training and inference procedures, and then finally report the experimental results.\subsection{Implementation Details}\label{sec:implementation}
...
%More specifically, we train a Siamese network, similar to the one in \cite{DAVIS2018-Interactive-2nd}, which is built on our encoder-decoder architecture. 
...
%, which encourages all pairs of pixels selected from one instance to have similar embeddings, and pushes pixels of different instances away from each other in the embedding space~\cite{DAVIS2018-Interactive-2nd}. %We pre-trained our model on Pascal instance segmentation dataset for $50k$ iterations with a fixed learning rate of $10^{-4}$ for the encoder and $2\times 10^{-4}$ for the decoder, momentum of $0.9$, and weight decay of $0.0005$. Standard data augmentation process (random cropping and mirroring) was applied, and input images to the network were all re-scaled to a resolution of $[321,321]$. 
...
%\subsection{Evaluation Metrics}\label{sec:metrics}%Following the standard benchmarks for video object segmentation, we report our results based on two metrics: {\it Jaccard} index ($\mathcal{J}$) and boundary F-score ($\mathcal{F}$). The former ($\mathcal{J}$) is defined as the intersection-over-union (IoU) of the predicted object mask and its respective ground-truth mask. The latter ($\mathcal{F}$) indicates how accurate the boundaries of the predicted mask are with respect to the ground-truth boundaries~\cite{davis_2017}. One can obtain per-object evaluation results by computing the average of these two metrics over the video that contains the object. The final evaluation result for a dataset is defined as the average of per-object performances.\subsection{Experimental Results}\label{sec:results}%In this section we report the meta-training configuration and evaluation results for ? publicly available video object segmentation datasets.
We report our results based on two standard metrics: {\it Jaccard} index ($\mathcal{J}$) and boundary F-score ($\mathcal{F}$).

%\vspace{-0.2in}\paragraph{DAVIS-2017}\mbox{}\\
...
%Meta training was performed for 150k episodes, where each episode was composed of the first frame of a sample video as the support set, and two randomly selected frames from the rest of that video as the query set. 
...
%In addition, online adaptation is conducted by first clustering the predicted masks after every five frames and then extracting $10$ new visual words for each object. These new visual words are added to the dictionary of visual words subsequently if they pass the criteria in Sec.~\ref{sec:onlineadaptation}.%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%\begin{table}[t]
    \centering
    \small
...
%As a result, our method runs at 1-2 orders of magnitude faster frame-rate, while still producing promising results, which are at-par with highly sophisticated techniques. 
...
%\vspace{-0.1in}\subsection{Ablation Study}\label{sec:ablation}
...
%Note that the performance flattens when the size of the dictionary exceeds 50 visual words per object. This result shows that 50 visual words are probably sufficient on average to describe the pixel distribution of the objects in DAVIS-2017 dataset.%\vspace{-0.2in}%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%\begin{figure}[]
	\centering
...
%As noted in Sec.~\ref{sec:onlineadaptation}, an important component of online adaptation is the outlier removal step. Table~\ref{table:outlierremoval} indicates the effect this module in the performance of online adaptation algorithm.
...
%\HB{We show more results in supplementary paper. We show more seg}%Next, as described in Sec.~\ref{sec:Model}, we compute a set of visual words from the upsampled embeddings, using k-mean clustering.%\subsection{Datasets}%\paragraph{DAVIS-2017}\mbox{}\\ %This dataset was introduced by \cite{davis_2017} as a standard benchmark for video object segmentation task. It consists of $50$ videos with dense ground-truth annotations of one foreground object throughout each video. The dataset is split into two partitions of training set ($30$ videos) and validation set ($20$ videos). Videos are released in both Full-HD and $480$p resolutions. However, all the experiments in this paper are performed on $480\times 854$ resolution videos.%This dataset ~\cite{davis_2017} is a super-set of DAVIS-2016 that contains $150$ videos (with similar resolutions to those in DAVIS-2016) and is subdivided into four different splits of training ($60$ videos), validation ($30$ videos), test-dev ($30$ videos), and test-challenge ($30$ videos). The main addition to this dataset is multi-object segmentation, where the number of objects/instances to be segmented per video range from one to ten. %\paragraph{DAVIS-2016}\mbox{}\\ %dataset was introduced by~\cite{DAVIS} as a standard benchmark for video object segmentation task. It consists of $50$ videos with dense ground-truth annotations of one foreground object throughout each video. The dataset is split into two partitions of training set ($30$ videos) and validation set ($20$ videos). Videos are released in both Full-HD and $480$p resolutions. However, all the experiments in this paper are performed on $480\times 854$ resolution videos.\iffalse\subsection{Implementation Details}\iffalse
