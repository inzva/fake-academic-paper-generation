...
%Therefore, we argue that the prior left-right consistency check utilizes a sub-optimal strategy for the stereo disparity estimation.
...
%Stereo matching is a classical problem in computer vision. A general routine to solve stereo matching problem consists of four stages: matching cost computation; cost aggregation; disparity selection/optimization; disparity refinement \cite{scharstein2002taxonomy}. 
...
%The whole pipeline is formulated as a recurrent network in our work.
...
%However, even with the improved deep neural networks, the accurate disparity estimation remains difficult for the occluded objects, textureless regions and object borders. We also observed that the mismatched pixels between the disparity maps of the two views mostly lie on these error-prone regions, as shown in Fig. \ref{label}.
...
%Achieving consistent disparity result for a pixel on $D_{\mathrm{left}}$ and $D_{\mathrm{left}}^{\prime}$ requires both the pixel and its mirror  to have the lowest cost among all the candidate matching pairs. Therefore, the consistent match under such restrict requirement may easily fail on the uniform regions,  repeated regions, occluded objects and sophisticated boundaries.  %Achieving consistent disparity result for a pixel on $D_{left}$ and $D_{left}^{'}$ requires the results of two independent argmin functions $d_{\text{left}}^{*}$ and $d_{\text{right}}^{*}$  on the matching costs to be similar for the pixel and its correspondence in the opposite view.  Eqn.~\ref{dstar} defines $d_{\text{left}}^{*}$ and $d_{\text{right}}^{*}$ mathematically, where $d_{i}$ is a disparity value in the disparity set $\textbf{d}$ containing all the possible disparity values. $C_{\text{left}}^{x}(d_{i})$ is the cost of pixel $x$ on the left image being matched to the pixel $x-d_{i}$ on the right image, and $C_{\text{right}}^{x-d^{*}_{\text{left}}}(d_{i})$ is the cost of pixel $x-d_{\text{left}}^{*}$ being matched to the pixel $x-d_{\text{left}^{*}}+d_{i}$ on the left image.  

%\begin{equation}
%	\begin{aligned}
%	d^{*}_{\text{left}} =& \argmin_{d_{i}\in \textbf{d}} C_{\text{left}}^{x}(d_{i}) \\
%	d^{*}_{\text{right}} =& \argmin_{d_{i}\in \textbf{d}} C_{\text{right}}^{x-d^{*}_{\text{left}}}(d_{i}) \\
%	\end{aligned}
%	\label{dstar}
%\end{equation}
...
\begin{figure*}
	...
	%\vspace{-0.6cm}
	%\hspace{-0.2cm}
	%\subfloat[]{\includegraphics[width=5.7cm,height=2cm]{./figures/000018_10.png}
	%}
	%\hspace{0cm}
	%\subfloat[]{\includegraphics[width=5.7cm,height=2cm]{./figures/000018_10_d.png}
	%}
	%\hspace{0cm}
	%\subfloat[]{\includegraphics[width=5.7cm,height=2cm]{./figures/000018_10_e.png}
	%}
	%\\
	\vspace{-0.7cm}
	\hspace{-0.2cm}
