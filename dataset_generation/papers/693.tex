...
% to compile a camera-ready version, add the [final] option_do, e.g.:
...
\label{sec:experiments}
  ...
  % This figure also compares the results obtained for different number of filters and different values of the TV weight.
  ...
  % Calculating the embedding space distance of the produced images to the desired target embedding, we demonstrated that \dots
  \begin{figure}[t]
  \centering
    \begin{subfigure}{.14\textwidth}
      \centering
...
\omtnips{
  ...
  \label{sec:gaussian}
    ...
    % Face reconstructions obtained using the Gaussian regularizer are shown in \Fig{fig:stat}.
  ...
  \label{sec:minima}
    ...
    % By performing a discrete Fourier transformation of $\Img$ in \Eq{eq:cont}, it is possible to convince oneself that the Laplacian operator $\Laplace$ effectively damps higher-frequency harmonics while letting $\pld{L}{\Img}$ define the evolution in the ``low-frequency subspace''.
    % As a result, the gradient descent is restrained from finding one of the false local minima of $L$ densely populating a very high-dimensional space of all images.
  ...
  \label{sec:regrest}
    ...
    % {\color{red} This does not guarantee a particular property of the output spectrum, but \dots}
