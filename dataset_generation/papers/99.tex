...
%\title{Feeding Two-Stream Convolutional Networks with Magnitude and Orientation Images}
%\title{Magnitude Orientation Based Temporal Stream for Activity Recognition}
%\title{\changed{Magnitude-Orientation} Stream: A Temporal Convolutional Network for Activity Recognition}
%\title{Magnitude-Orientation Stream for Activity Recognition}
\title{Activity Recognition based on a Magnitude-Orientation Stream Network}
%\title{Temporal Convolutional Network Based on Magnitude and Orientation Information}
%\title{Feeding Two-Stream Convolutional Networks with Magnitude and Orientation for Activity Recognition}
%------------------------------------------------------------------------- 
...
\begin{abstract}
	...
	%We also combine the Magnitude-Orientation Stream with a Very Deep Two-Stream method.
...
%\todo{seria bom colocar a sigla MOS aqui tambem, depois que fala o que vai fazer, seria bom dizer como serah chamada e qual eh a sigla, pois ficou apenas no abstract}
...
%\todo{colocar aqui rapidamente uma diferenciacao do nosso metodo com os demais baseados em gradiente, jah estah no paragrafo anterior, mas poderia ser um sumario bem conciso aqui.}

%According to the experimental results, our proposed temporal stream is able to recognize activities accurately on two well-know datasets (UCF101 and HMDB51) outperforming the results achieved by the original two-stream network as well as other deep networks available in the literature.
...
%The remainder of this paper is organized as follows. In Section~\ref{related}, we give a brief explanation of the works in literature that explore temporal information to perform activity recognition. In Section~\ref{approach}, we introduce our approach to extract temporal information based on magnitude and orientation. Then, Section~\ref{experiments} presents our experimental results, validating the performance achieved. Finally, Section~\ref{conclusions} presents conclusions obtained and future works.
...
\begin{figure*}[!t]
	...
	\end{tabular}
	%\caption{Video architectures considered in this work. \todo{nao seria arquitetura de video, seria  arquitetura para extracao de informacoes e representacoes a partir de videos, certo?}}
...
%	\centering
%	\begin{tabular}{cc}
%		\includegraphics[width=0.35\textwidth]{figs/Pipeline1a.pdf} & \includegraphics[width=0.27\textwidth]{figs/Pipeline2a.pdf} \\
%		\footnotesize{(a) Very Deep Two-Stream~\cite{Wang:2015}.} & \footnotesize{(b) Fusion of our approach and Very Deep Spatial Network.} \\	\includegraphics[width=0.35\textwidth]{figs/Pipeline3a.pdf} & \includegraphics[width=0.35\textwidth]{figs/Pipeline4a.pdf} \\
%		\footnotesize{(c) Our Magnitude-Orientation Stream approach.} & \footnotesize{(d) Fusion of our approach and Very Deep Two-Stream~\cite{Wang:2015}.} \\
%	\end{tabular}
%	%\caption{Video architectures considered in this work. \todo{nao seria arquitetura de video, seria  arquitetura para extracao de informacoes e representacoes a partir de videos, certo?}}
%	\caption{Architectures considered in this work for extracting spatiotemporal information.}
%	\label{img:pipeline}
...
%A significant portion of the progress in visual recognition tasks has been achieved with the design of handcrafted local feature descriptors. The extraction process is very important because it is through it that the image/video contents will be represented before being sent to a classification step to learn a function to assign discrete labels to the data.
...
%\todo{Fazer um paragafo que compare, de certa forma, nosso metodo com esses de local feature.}
...
%In order to perform motion estimation in video compression, pixels are grouped in macroblocks and motion vectors are then computed for each block. Known as MPEG motion vectors~\cite{Richardson:2003}, Perez et al.~\cite{Perez:2017} used them as a different way to feed a two-stream network input in order to explore temporal information. They show that both optical flow and MPEG motion vectors provide equivalent accuracies, but the latter allow a more efficient implementation. 
...
%\todo{nao ficou claro no texto, mas estou assumindo que colocamos nossos dados na entrada da  da rede very deep, certo?}
...
%\changed{quanto alterar algo significativo use o comando changed para facilitar a minha revisao.}
...
%
%Our hypothesis is based on the assumption that the motion information on a video sequence can be described by the spatial relationship contained on local neighborhoods of magnitude and orientation relationships from the optical flow. More specifically, we assume that the motion information is adequately specified by fields of magnitude and orientation.
%
...
%Figure~\ref{img:orimag} illustrates the magnitude and orientation information comparison with the optical flow x and y displacement extract from two consecutive frames. \todo{nao entendi essa ultima sentenca, nao estah construida direito.}
...
%\todo{essa parte ficou confusa: decreases to its $1/10$ at $5,000$ iterations and stops at $15,000$ iterations}.
...
\begin{table*}[th!]
	...
	\begin{small}
		...
		\begin{tabular}{clp{1.5cm}p{1.5cm}p{1.5cm}p{1.5cm}}
			...
			%\multirow{1}{*}{\textbf{Our}} & \multirow{2}{*}{Magnitude-Orientation Stream (MOS)} & \multicolumn{1}{c}{\multirow{2}{*}{90.8}} & \multicolumn{1}{c}{\multirow{2}{*}{\textbf{}}} & \multicolumn{1}{c}{\multirow{2}{*}{91.5}} & \multicolumn{1}{c}{} \\
...
%\todo{padronizar o termo, ou usar split1 ou split 1 nos paragrafos a seguir.}
...
%\todo{Estou querendo colocar uma figura com a matriz de confusao e comentar sobre as classes que mais erraram. Porem a matriz e grande podendo consumir muito espaco do paper. Veremos...}
...
\begin{table}[!htb]
	...
	\begin{small}
		...
		\begin{tabular}{clc}
			...
			%& OFCM + BoW~\cite{Caetano:2016} & \multicolumn{1}{c}{-}  \\ 
			...
			%& MoFAP~\cite{Wang:2016b} & \multicolumn{1}{c}{88.3} & \multicolumn{1}{c}{61.7} \\ 
			\midrule
...
%\todo{pensar em algo sobre porque nao fez a fusao no HMDB51}

%\begin{table}[th!]
%	\centering
%	\begin{small}
%		\caption{Action recognition accuracy (\%) results of handcrafted methods, neural networks (NN) and our Magnitude-Orientation Stream on UCF101~\cite{Soomro:2012} and HMDB51 activity dataset~\cite{Kuehne:2011}. Results for features + BoW were obtained from~\cite{Shi:2015} and features + FV were obtained from~\cite{Shi:2014:Dissertation}.}
%		\begin{tabular}{clcc}
%			\toprule
%			& & \multicolumn{1}{c}{\textbf{UCF101}} & \multicolumn{1}{c}{\textbf{HMDB51}} \\
%			& \textbf{Approach} & \multicolumn{1}{c}{\textbf{Acc. (\%)}} & \multicolumn{1}{c}{\textbf{Acc. (\%)}} \\
%			\toprule
%			& HOF + BoW~\cite{Laptev:2008} & \multicolumn{1}{c}{61.8} & \multicolumn{1}{c}{35.5} \\ 
%			& HOG-HOF + BoW~\cite{Laptev:2008} & \multicolumn{1}{c}{71.8} & \multicolumn{1}{c}{43.6} \\ 
%			& MBH + BoW~\cite{Dalal:2006} & \multicolumn{1}{c}{77.1} & \multicolumn{1}{c}{51.5} \\ 
%			& GBH + BoW~\cite{Shi:2015} & \multicolumn{1}{c}{68.5} & \multicolumn{1}{c}{38.8} \\ 
%			& HOG3D + BoW~\cite{Klaser:2008} & \multicolumn{1}{c}{61.4} & \multicolumn{1}{c}{36.2} \\ 
%			& OFCM + BoW~\cite{Caetano:2016} & \multicolumn{1}{c}{-} & \multicolumn{1}{c}{56.9} \\ 
%			
%			& HOF + FV~\cite{Laptev:2008} & \multicolumn{1}{c}{65.9} & \multicolumn{1}{c}{39.9} \\ 
%			\multirow{1}{*}{\textbf{Handcrafted}} & HOG-HOF + FV~\cite{Laptev:2008} & \multicolumn{1}{c}{75.4} & \multicolumn{1}{c}{45.6} \\ 
%			\multirow{1}{*}{\textbf{Methods}} & MBH + FV~\cite{Dalal:2006} & \multicolumn{1}{c}{81.0} & \multicolumn{1}{c}{54.7} \\  
%			& GBH + FV~\cite{Shi:2015} & \multicolumn{1}{c}{74.2} & \multicolumn{1}{c}{44.7} \\  
%			& HOG3D + FV~\cite{Klaser:2008} & \multicolumn{1}{c}{64.7} & \multicolumn{1}{c}{38.2} \\ 
%			
%			& IDT~\cite{Wang:2013} & \multicolumn{1}{c}{85.9} & \multicolumn{1}{c}{57.2} \\ 
%			& IDT + higher FV~\cite{Peng:2016} & \multicolumn{1}{c}{87.9} & \multicolumn{1}{c}{61.1} \\
%			& IDT + MVSV~\cite{Cai:2014} & \multicolumn{1}{c}{83.5} & \multicolumn{1}{c}{55.9} \\
%			%& MoFAP~\cite{Wang:2016b} & \multicolumn{1}{c}{88.3} & \multicolumn{1}{c}{61.7} \\ 
%			\midrule
%			
%			& Deep Networks~\cite{Karpathy:2014} & \multicolumn{1}{c}{65.4} & \multicolumn{1}{c}{-} \\
%			& Composite LSTM~\cite{Srivastava:2015} & \multicolumn{1}{c}{75.8} & \multicolumn{1}{c}{44.0} \\
%			& C3D~\cite{Tran:2015} & \multicolumn{1}{c}{85.2} & \multicolumn{1}{c}{-} \\
%			\multirow{1}{*}{\textbf{NN}} & Factorized CNN~\cite{Sun:2015} & \multicolumn{1}{c}{88.1} & \multicolumn{1}{c}{59.1} \\
%			\multirow{1}{*}{\textbf{Methods}} & Two-Stream~\cite{Simonyan:2014} & \multicolumn{1}{c}{88.0} & \multicolumn{1}{c}{59.4} \\ 
%			& Two-Stream F~\cite{Feichtenhofer:2016} & \multicolumn{1}{c}{92.5} & \multicolumn{1}{c}{65.4} \\
%			& KVMF~\cite{Zhu:2016} & \multicolumn{1}{c}{93.1} & \multicolumn{1}{c}{63.3} \\
%			& TSN~\cite{Wang:2016} & \multicolumn{1}{c}{94.2} & \multicolumn{1}{c}{69.4} \\
%			\midrule
%			&\multirow{1}{*}{MOS} & \multicolumn{1}{c}{\multirow{1}{*}{}} & \multicolumn{1}{c}{\multirow{1}{*}{66.2}} \\
%			\multirow{1}{*}{\textbf{Our}} & \multirow{1}{*}{MOS + VD Temp. Stream} & \multicolumn{1}{c}{\multirow{1}{*}{}} & \multicolumn{1}{c}{\multirow{1}{*}{}} \\
%			\multirow{1}{*}{\textbf{results}} & \multirow{1}{*}{MOS + VD Spat. Stream} & \multicolumn{1}{c}{\multirow{1}{*}{}} & \multicolumn{1}{c}{\multirow{1}{*}{}} \\
%			& \multirow{1}{*}{MOS + VD Two-Stream} & \multicolumn{1}{c}{\multirow{1}{*}{}} & \multicolumn{1}{c}{\multirow{1}{*}{}} \\
%			\bottomrule
%		\end{tabular}
%		\label{tab:ucf101-comparison}
%	\end{small}
