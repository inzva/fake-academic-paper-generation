...
%\author{Aaron Wetzler \qquad Ron Slossberg \qquad Ron Kimmel\\
%Technion Computer Science Department\\
%Haifa, Israel\\
%{\tt\small {twerd@cs,ronslos@tx,ron@cs}.technion.ac.il}
%}
...
%\begin{algorithmic}[1]
%\Procedure{Derotate}{$R$}   
%	\State $r_{align}=\argmax_{r_i \in \{r_1,r_2,r_3\}} \|\left(0,0,1\right)\cdot r_i\|$
%	\If {$r_{align}$ == $r_2$ (thumb aligned axis) }
%	   \State $\alpha = atan2(r_{3x},r_{3y}) - 90$
%	\Else
%	   \State $\alpha = atan2(r_{2x},r_{2y}) + 90$	   
%	\EndIf
%	\State Return $\alpha$
%\EndProcedure
%\end{algorithmic}
