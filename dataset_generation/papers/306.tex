...
%            %pagebackref,       % can comment this line out to save space if needed 
%            linkcolor=black,   % hyperref's default is red; that's pretty ugly in my view... Oren (Søren set it to black). 
%            ]{hyperref}
...
%  (as we will explain, a side effect of this two-step generative procedure is that it also captures certain non-diffeomorphic deformations).
...
%   i.e.\ $\mathcal{F}(\bx) = \mathcal{F}(T^{\btheta} \circ \bx)$,
...
%   has also been studied 
  ...
  was 
  %is \OREN{I usually prefer present simple in this section, but it fell odd to say that a dataset *is* generated}
...
%  \subsection{Other Meanings of the term Data Augmentation}%    We have used the terminology of the recent machine-learning literature, %    however, we note that data augmentation has several other meanings.%    In the statistics literature data augmentation refers to the introduction of%    latent variables to further inference in complex models~\cite{tanner1987calculation,van2001art}.%    Yet another use of the term is found in the business-intelligence community,%    where data augmentation refers to methods for enriching data through the%    use of external sources, partly as to compensate for missing data~\cite{lam2008user,baecke2010improving,nambiar2011data}.%%  \subsection{The Role of Dreaming in Cognitive Information Processing}%    \emph{Dream sleep} is believed to be strongly linked to learning and memory%    consolidation and has been used earlier as a metaphor for generative%    modeling~\cite{NIPS2014_5268,kingma2014semi}. In dreams, rodents replay%    behavior, seemingly at a high speed~\cite{euston2007fast}, while frontal-cortex %    replay during %the so-called %    \emph{slow-wave sleep} is related to rule %    learning~\cite{peyrache2009replay}. In a recent review Stickgold and %    Walker pointed out that sleep is selective~\cite{stickgold2013sleep}: %    Sleep seems to consolidate relevant aspects and suppress irrelevant %    ones when replaying experiences, the decision %on %    which aspect to retain being based on \emph{salience tags} attached to memories at the time %    of encoding. Such ``dream processes'' improve learning and generalizability~\cite{diekelmann2010sleep}, %    hence also form an appropriate metaphor for the%    process proposed in the present work which is aimed at emphasizing salient variability%    and suppressing the irrelevant.
...
% To avoid a manual specification of the transformations of interest (such as rotations, translation, etc.),% we focus our attention on the large class of first-order $\Rtwo \rightarrow \Rtwo$% diffeomorphisms, i.e.\ we consider $T^{\btheta}$ that are smooth (with at least one derivative),% and has a smooth inverse $(T^{\btheta})^{-1}$.
...
\begin{figure*}
    ...
    %The space of \emph{all} diffeomorphisms is an infinite-dimensional Lie group \cite{arnold1989mathematical},%which is problematic for our purposes as it makes image alignment intractable. % (recall we want to align a large number of image pairs).%We, thus, opt to restrict our attention to a finite-dimensional subset of%this Lie group.
    ...
    \begin{split}
%       \Vcal = \Vcpabdry(\Omega;\Pcal) \triangleq \set{\bv^\btheta: \bv^\btheta \text{ is an } \Omega\to\Rtwo \text{ CPA map (w.r.t.~}\Pcal\text{) } \text{and} \,   
    ...
    %\OREN{Consider instead:%We have found this choice of $\Pcal$, which together with the boundary condition implies that $d = 50$, %to strike a nice balance between computation time and expressiveness.}
...
% % % % %     (which maps % % % % %     the space of all continuous velocity fields into the space of diffeomorphisms) \OREN{I commented this out, since I am not exactly sure about this sentence}
    to $\Vcal$ coincides with the mapping of $\bv^\btheta$ to $T^\btheta$ via
    the integral equation \eqref{eqn:intEqn}. 
    % \OREN{I moved it elsewhere, since what follows the ``Thus,'' does not actually follow from the previous sentence.% % %     Thus, $\Vcal$ is a tangent space% % %     of $M$, where the identity transformation $T^{\bzero}$ is the point of% % %     tangency. 
    We 
%     may then 
    thus
    write $T^\btheta = \Exp_{T^{\bzero}} (\bv^\btheta)$,
    where 
%     $\Exp_{T^{\bzero}} (\cdot)$$\Exp_{T^{\bzero}}:\Vcal\to M$
...
%     $\bx_m \circ T^{-\btheta_{mn}} \approx \bx_n$. \OREN{This is wrong - it was my bad - sorry...}$\bx_m \circ T^{\btheta_{mn}} \approx \bx_n$.
...
%    What makes MCMC computationally tractable here are %    the facts %    that for CPA velocity fields, the exponential map -- which is needed for the likelihood evaluations -- %    has a fast numerical implementation \cite{freifeld2015transform}, %    that $d$ is not too large,%    and that evaluating the prior is easy (since it is a Gaussian).%    Using this sampling scheme, we then set $T^{\btheta_{mn}}=T^{\btheta}\sim p(T^{\btheta_{}} | \bx_m, \bx_n) $.% Statistics on manifolds\subsection{Statistical Models of Transformations}
Having aligned images pairs as discussed above, we now treat the inferred $\set{T^{\btheta_{mn}}}$ as observations. 
  ...
  % By a slight abuse of notation, we may alsdue o identity % $\bv^{\btheta_{nm}}$ with $\btheta_{nm}$, and simply write $\btheta_{nm} = \Log_{T^{\bzero}}(T^{\btheta_{nm}})$.
...
%  denotes the sample covariance of the transformations as expressed in the tangent space~\cite{pennec:jmiv:2006}, 
  where $N_y$ is the number of sample pairs used for this class.
  %More compactly, we can write $\bSigma_y = \frac{1}{N_y} \sum_{nm} {\btheta_{nm}} \btheta_{nm}\T$.
  We then have
%     $p(T^\btheta | y) = \mathcal{N} (\Log_{T^{\bzero}}(T^\btheta) | \bzero, \bSigma_y)$.\begin{align}
    p(T^\btheta | y)
      &\propto \exp\left(
...
%  This shows that a good augmentation scheme can make deep learning a viable option even%  for fairly small datasets.
