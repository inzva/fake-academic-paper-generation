...
%The $SPET_{lThresh}$ hyper-parameter is determined during DNN training.
...
%LeNet-5~\cite{} for the MNIST dataset,
%16and VGG~\cite{}
...
%Similar improvements can be demonstrated with BLAS, by integrating our techniques at finer level of granularity.
...
%However, their huge size is a problem in the deployment phase, particularly in -device computing platforms, which are limited in memory, hardware and energy. Even when deployed at global scale, with millions of nodes and state-of-the-art hardware, this has issues regarding energy requirements of servers in addition to network costs associated with sending the inputs to the cloud for processing.

%These networks tend to be over-parametrized, due to the non-convexity of the optimization. The resulting redundancies in the network cause wasteful computations. Various efforts have been made to identify and tackle these. Hardware accelerators have been designed to maximise reuse of computed values. In particular focus has been the observation that the vector operations in a neural network do not need to be  precise. Efforts have been made to parallelize computations at different levels of granularity from bit-level to data-level. Algorithms have been proposed to modify the learning process to reduce the redundancies in weights or connections in the network. We propose a new dynamic approach to this problem
%Secondly, not all inputs are equally "hard". However, deep networks are "static", that is they expend fixed computational effort across all inputs and regions of the input. So, WHAT? intuitively the "region of interest" is the position of the object to be classified, which typically appears in a rather uniform background. Intuitively, more effort should be spent here as compared to the rest of the image. 

%However, as promising as these opportunities sound, it is a huge challenge to tap into them and translate it into savings in computations. There is a trade-off between the computations used to dynamically predict the critical computations in the network for the current input and the corresponding savings. We observe that it is more () to detect redundant computations, that can be skipped rather than identifying the most critical computations on the fly.

%We have a pre-trained network, N, and we want to introduce certain knobs, $K_i$, which we can use the constrain the number of computations performed by the network. In this section, we'll motivate the policy at each level of granularity and describe it in detail. If we can detect that a neuron is going to deactivate while performing the MACC, we can save wasteful computations by terminating the MACC.

%Thus, we'll focus on the ReLU activation, though the idea can be easily applied to any saturating function.

%The probability of activation in a convolutional layer is usually low (sparsity) and decreases further, the deeper the layer is in the network. This is illustrated in figure.
%Instead, we settle for the following scheme (illustrated in Algorithm), where we rearrange the convolution, such that odd indexed multiplications in a MACC are computed first, skipping the even ones. Then, we compute the even indexed multiplications in the second pass. This means that when half the MACC has been computed, we have performed half the convolution in each input feature. This is much more representative of the output than the original ordering and also adheres to the memory access pattern.

%In the previous section, we observed that most neurons in a feature are deactivated. There exists large regions within the feature, where the values are uniformly zero. We can save computations by sampling the neurons that we compute in the feature, and skipping the others in such regions. Further, consider the scenario when the "region of interest" in the input image appears in a largely uniform background. The output of the convolution performed over different windows on the background is going to be largely identical even if it is not zero. We would want to minimise the effort spent on these regions. We use these observations to frame our policy, SS.

%For each feature, we compute one in every k neurons along each row, where k is a parameter that we can set. We call this the stride of the sampling. The first neuron computed in each subsequent row is also shifted, so as to increase the coverage of sampled neurons over the feature. For the remaining neurons, we linearly interpolate the output, using closest neighbours along the horizontal and vertical axes.  This is illustrated in figure 
%If values in a feature or region within the feature are very similar (which is what exploit in SS), the contribution of any convolutional window falling within this can be approximated using a single MACC operation of the average of the input and the sum of weights of the corresponding connections. 

%The combination is fairly trivial, given that they are orthogonal policies. The only point of interest is the manner in which ET and SS are applied together. We do not perform ET on neurons that SS determines are to be recomputed. Such neurons are unlikely to be deactivated, since they appear in a neighbourhood of high activations and variance. It is also prudent to assume that even small approximation errors can affect the overall quality of the output based on our criticality hypothesis.

%Static optimizations try to reduce redundancy in the parameters of the network, thereby ensuring an overall more efficient network in terms of memory and computations. Among static optimisations, some techniques primarily target the memory required to store the network. This can be a bottleneck when neural networks are deployed in mobile phones and other limited resource devices such as IoT devices, where application size and bandwidth are constrained. The others focus more on the computational aspect, which directly correlates to the energy consumption and usually time. This dichotomy arises because convolutional layers contribute to around 90\% of the computations whereas fully connected layers dominate the storage requirement.

%Further, the weights are grouped into bins, that are learnt from the data unlike Hashnets. Finally, huffman coding is used to further reduce the number of bits needed to represent the weights. Deep compression reduces the size of state-of-the-art networks by 35-50$\times$. (Gong et al) put forward the use of vector quantisation to compress the number of weights in a layer.
%and demonstrate 2-3x speedup in convolutional layers on both CPUs and GPUs. Jad also us e a low rank decomposition to achieve a 4.5x speedup on scene text character recognition. V propose using approximate computing to exploit the resilience of neurons to inexactness to reduce the energy requirements by almost 2x. This resilience is estimated from the training data and such neurons are replaced with an "approximate neuron", which performs computations at lower precision. Feng et al show that the structure of a deep convolutional network, specifically the number of features in each layer can be learnt from the training data. Using this process to train AlexNet results in a network, which only 37.5\% of the number of filters in AlexNet on an average, albeit that achieves a top-1 accuracy that is almost 3\% lesser. 

%\begin{algorithm}[htb]
%\SetAlgoLined
%\KwResult{Output Tensor $O$}
%$O  \leftarrow 0$\;
%$index \leftarrow 0$\;
%\For{$f$ in OutputChannels} {
%  \For{$c$ in InputChannels} {
%    $kernel_y \leftarrow 0$\;
%    $kernel_x \leftarrow 0$\;
%    \While{$kernel_y \leq kernel_h$} {
%      \While{$kernel_x \leq kernel_w$} {
%        index $\leftarrow$ index + 1\;
%        \If{index $\equiv 0\ (\textrm{mod}\ 2)$} {
%          \While{$o_y \leq output_h$} {
%            \While{$o_x \leq output_w$} {
%              $o_y \leftarrow 0$\;
%              $o_x \leftarrow 0$\;
%              $(i_x, i_y) \leftarrow$ Input index corresponding to $(o_x, o_y, kernel_x, kernel_y)$\;
%              $O(o_x, o_y, f) = O(ox_, o_y, f) +  W(kernel_x, kernel_y, f) * I (i_x, i_y , c)$\;
%            }
%          }
%        } 
%      }
...
%  }
%}
%ActivatedIndices $\leftarrow$ \{\} \;
%\For{$f$ in OutputChannels} {
%$o_y \leftarrow 0$\;
%$o_x \leftarrow 0$\;
% \While{$o_y \leq output_h$} {
%    \While{$o_x \leq output_w$} {
%      \If{$O(o_x, o_y, f) \geq SPET_{lThresh}$} {
%        ActivatedIndices $\leftarrow$ \{ ActivatedIndices, $(o_x, o_y, f)$ \} \;
%      } 
%    }
%  }
%}
...
%    \While{$kernel_x \leq kernel_w$} {
%      index $\leftarrow$ index + 1\;
%      \If {index $\equiv 1\ (\textrm{mod}\ 2)$} {
%        \For{$(o_x, o_y, f)$ in ActivatedIndices} {
%          $(i_x, i_y) \leftarrow$ Input index corresponding to $(o_x, o_y, kernel_x, kernel_y)$\;
%          $O(o_x, o_y, f) += W(kernel_x, kernel_y, f) * I (i_x, i_y , c)$\;
%        }
%      } 
%      
%    }
%  }
%}
%\caption{SPET Convolution}
%\label{algo:spetConv}
%\end{algorithm}

%\begin{table}[htb]
%\centering
%\caption{Operations and execution time benefits of DyVEDeep at Iso-accuracy}
%\label{tab:ops-time}
%\begin{tabular}{|c|c|c|c|c|}
%\hline
%                           & \multicolumn{2}{c|}{\textbf{\#Operations}} & \multicolumn{2}{c|}{\textbf{Run Time (ms)}} \\ \hline
%Network                    & Original             & DyVEDeep            & Original             & DyVEDeep             \\ \hline
%\textbf{CIFAR-10}          & 29679872             & 3657856             & 124                  & 33                   \\ \hline
%\textbf{AlexNet}           & 1449602976           & 214706617           & 7394                 & 2027                 \\ \hline
%\textbf{Overfeat Accurate} & 5231363620           & 608267526           & 27606                & 6133                 \\ \hline
%\end{tabular}
%\end{table}

%It is interesting to see that we are able to achieve almost the maximum possible savings with each knob in the last three layers, highlighting the redundancy present in these layers.
