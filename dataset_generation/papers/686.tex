...
% Importance/applications of 3D model
...
% methods to consume 3D data: multi view 2D cnn, uniform 3D voxel cnn, point cloud, data structures
...
% basic ideas to solve spatial distribution problem, and local/global feature aggregation 
...
%can leverage the efficiency of ConvNets. 
...
% indexing structures, Kd-Net, oct-net, o-cnn
...
% PointNet/PointNet++, set learning
...
%In various recognition tasks, SO-Net demonstrates competitive performance compared with existing point cloud based networks, and even state-of-the-art voxel / multi-view based deep networks.
...
% dataset
% network architecture, faiss
% present results/loss, no previous work, effect of 2 branches
%
\begin{figure}[h]
        \centering
...
% network structure, present results, outperform all methods, all single networks, except MVCNN-MultiRes, VRN ensemble that combine several networks.
% effect of pre-training
% training time
% robustness: point number, som size, som noise
...
%Compared to methods that provide receptive field overlap like PointNet++ \cite{qi2017pointnet++}, our network has advantages of explicit utilization of the spatial distribution and systematically adjustable receptive field, which could be the sources of performance improvement.
...
% the testing point cloud sizes vary from 2048 to 256. Our performance remains satisfying until the number of points decrease to 512, i.e., 1/4 of the training configuration.
...
% The effect of noisy SOM is not that severe in ModelNet10 compared to ModelNet40, most likely because the 10-class ModelNet10 is relatively easier.
...
% \begin{appendices}
% \include{cvpr2018_sm/supplementary}
% \end{appendices}
