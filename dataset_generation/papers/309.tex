...
%The service automatically downloads the resulting inference code, including the learned model, to the user's embedded device for immediate use in applications.
\end{abstract}

% \keywords{
%      embedded device; binary neural network; smart device; wearable %computing;
% }
%
% NOTE
%
...
% We have described eBNN with an image classification application in mind; however, the same concept applies for other types of tasks as well.
...
%\begin{figure}[ht!]
%  \centering
%    \includegraphics[width=.2\linewidth]{figures/fused_network}
%  \caption{An eBNN consisting of two layers of fused binary convolution-pool blocks. \textbf{f\textsubscript{1}} and \textbf{f\textsubscript{2}} represent the number of filters in the corresponding layer. The number of filters are screened over in the evaluation to vary the memory size of the network. The first $/2$ in the layer description is the stride for convolution, and the second pool $/2$ is the stride for pooling.}
%  \label{fig:fused_network}
%\end{figure}
...
%\centering
%\subfloat{\includegraphics[scale=.45]{figures/mnist_mlp}}
%\end{minipage}%
%\begin{minipage}{.33\textwidth}
%\centering
%\subfloat{\includegraphics[scale=.45]{figures/mnist_conv}}
%\end{minipage}%
%\begin{minipage}{.33\textwidth}
%\centering
%\subfloat{\includegraphics[scale=.45]{figures/mnist_convpool}}
%\end{minipage}\par\medskip
%\centering
%\begin{minipage}{.33\textwidth}
%\centering
%\subfloat{\includegraphics[scale=.45]{figures/mlp_cifar10}}
%\end{minipage}%
%\begin{minipage}{.33\textwidth}
%\centering
%\subfloat{\includegraphics[scale=.45]{figures/conv_cifar10}}
%\end{minipage}%
%\begin{minipage}{.33\textwidth}
%\centering
%\subfloat{\includegraphics[scale=.45]{figures/convpool_cifar10}}
%\end{minipage}

%\caption{The accuracy and memory size of the eBNN evaluation models for the MNIST and CIFAR10-Easy datasets, as the number of filters increases. The accuracy show at each point for eBNN is equivalent to the corresponding BNN model. For MLP models, the temporary size is always under $0.005$ KB. The maximum memory cutoff was set at $15$ KB in order to not exceed the memory limit of the device (Arduino 101). The gap between each pair of blue diamonds (Network Weights) and red squares (Network Weights + Temporaries) shows the size needed to store the temporaries for that network. Note that for convolutional structure the temporaries never exceed $0.5$ KB.}
%\label{fig:accuracy_memory}
%\end{figure*}
...
% The API provides four service endpoints: \textit{collections}, \textit{discover}, \textit{classifiers} and \textit{generate}, as described below.

% The \textit{collections} service endpoint (i.e. /collections) has POST, GET, PUT, DELETE methods used to create, list, update and delete data collections. For each collection, it has \textit{images} endpoints (i.e. /collections/\{collection\_id\}/images) with POST, GET, PUT, DELETE methods to add, list, update and delete images. While we describe these collections with images in mind, the service architecture is not limited to that type of data. Additional endpoints can added for each type of data. For example, /collections/\{collection\_id\}/voices endpoint is used to add, list, update and delete voice data.
% \begin{itemize}
%     \item POST /collections \\
%     Create a dataset collection
%     \item GET /collections \\
%     List collections
%     \item GET /collections/\{collection\_id\} \\
%     Detail a collection
%     \item PUT /collections/\{collection\_id\} \\
%     Update a collection
%     \item DELETE /collections/\{collection\_id\} \\
%     Delete a collection
%     \item POST /collections/\{collection\_id\}/images \\
%     Add an image to a collection
%     \item GET /collections/\{collection\_id\}/images \\
%     List images in a collection
%     \item GET /collections/\{collection\_id\}/images/\{image\_id\} \\
%     Detail an image to a collection
%     \item DELETE /collections/\{collection\_id\} \\
%     /images/\{image\_id\} \\
%     Delete an image
% \end{itemize}

% The \textit{discover} endpoint (i.e. /discover) has GET method for discovering neural network models given a collection and a device memory constraint. This method returns identifiers of classifiers that fit within the given constraint. Users of the service can use these identifiers to update the classifiers as they see fit using the \textit{classifier} service endpoint and generate a standalone C header file for use on embedded devices using the \textit{generate} service endpoint.
% \begin{itemize}
%     \item GET /discover \\
    
% \end{itemize}

% The \textit{classifier} service endpoint has POST, GET, PUT, DELETE methods for create, list, update and delete classifiers. These models are trained using the python Chainer implementation.
% \begin{itemize}
%     \item POST /classifiers \\
%     Create a classifier
%     \item GET /classifiers \\
%     List classifiers
%     \item GET /classifiers/\{classifier\_id\} \\
%     Detail a classifier
%     \item PUT /classifiers/\{classifier\_id\} \\
%     Update a classifier
%     \item DELETE /classifiers/\{classifier\_id\} \\
%     Delete a classifier
% \end{itemize}

% The \textit{generate} service endpoint has GET method for generating a standalone C header file for use on embedded devices.

% The REST APIs can be used in the following fashion:
% \begin{enumerate}
%     \item Users employ the \textit{collections} service endpoint to create a collection of data (e.g., images, voices, videos and text).
%     \item Users then utilize the \textit{discover} service endpoint given a collection of data and a memory constraint to discover multiple neural network configurations.
%     \item Optionally, users use the \textit{classifier} service endpoint to update the configuration of the neural network models as they see fit.
%     \item Finally, the \textit{generate} service endpoint generates the standalone C eBNN library which can be downloaded and compiled on the embedded device.
% \end{enumerate}
...
% Get Image Detail
% GET /collections/{id}/images/{image_id}
% Add Image
% POST collections/{id}/images
% Delete Image
% DELETE collections/{id}/images/{image_id}
...
% TODO: Add citation to IBM Bluemix API
