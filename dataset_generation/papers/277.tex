...
%Among all methods for evaluation, only ITQ and ITQ-CCA are hashing methods trained from pointwise data,  which is less deteriorated by the data imbalance problem. This explains their relatively better performance.
...
%Fundamentally, by continuous relaxation, existing methods solve an optimization problem which may deviate significantly from the original hashing problem, resulting in suboptimal binary codes. 
...
\begin{equation}\label{eqn:dinnerproduct_case}
  ...
  \end{cases}
\end{equation}
%Thus, loss $L$ is monotonically decreasing as ${s_{ij}} = 1$, while it is monotonic increasing as ${s_{ij}} = 0$
