...
%%However, the distribution gap between synthesized data and real data often determines the success of such an approach. 
...
%Based on the discussions for both goals, several problems are open to be solved for medical images: 1) Learning from unpaired data; 2) Keeping the tomography invariance for unsupervised image-to-image translation; 3) Using synthetic data to improve segmentation. This paper presents a general-purpose method to address those problems.
...
%\cite{zhu2017unpaired,kim2017learning,liu2017unsupervised} have shown solutions to learning from unpaired data for natural image style transfer, 
%Our method adopts \cite{zhu2017unpaired} and we carefully consider and address problems that is unique for medical 3D images, e.g. shape consistency.

%Conditional GANs \cite{isola2016image} provide a general-purpose framework to address this task by simply learning GANs in a conditional setting. Acquirng paired training data is difficult and even impossible for some tasks, several recent studies \cite{} have addressed this issue 
%CycleGAN \cite{zhu2017unpaired} further generaizes it for the situation that source and target domains have no paire data. 

%Image segmentation with adversarial learning
...
\end{split}
\end{equation}
%where $S_A: A \rightarrow Y $ and $S_B: B \rightarrow Y$ produce a shared shape space data $Y$ (i.e. semantic annotation) for domain A and domain B data. 
...
% \begin{equation}
% \begin{split}
% \mathcal{L}_{GAN}(G_A, D_A) & = \mathbb{E}_{x_B \sim p_{d}(x_A)}[\text{log} D_A(x_A)] \\
% 								 & = \mathbb{E}_{x_B \sim p_{d}(x_B)}[\text{log} (1- D_A(G_A(x_B)))]
% \end{split}$
% \end{equation}
\begin{figure*}[t]
	\begin{center}
...
%Totally two $2{\times}2$ downsampling and upsampling are supported by stride-2 convolutions and transpose-convolutions, respectively. 
...
%The purpose of this experiment is to evaluate how well our method makes use of synthetic data to improve segmentation. 
