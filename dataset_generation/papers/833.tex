...
%%Mine, to separate cells in two lines
\usepackage{makecell}
\usepackage{multirow}
...
Thus, early approaches translated the 3D medical image segmentation into a 2D patch-wise image classification approach ~\citep{Cir12,Pra13},  getting several thousands of training samples from just a few datasets. In other approaches ~\citep{Rot14} 3D data is represented as 2.5D slices, where axial, sagittal and coronal planes are stored as layers of a RGB image and the training is done with multiscale image patches.  However, according to related publications, these approximations only consider local context, are prone to failure and suffer from memory and time efficiency issues. More recent approaches combine 2D network predictions with voting approaches ~\citep{Mil16a} or traditional level-sets ~\citep{Cha16}. Spatial consistency is enforced at a second stage through post-processing computation. The natural evolution of these approximations is to train a network with the whole 2D slices. This was proposed by ~\citep{Ron15}, which defined the U-Net architecture that works with few training images and uses data augmentation strategies to compensate for the reduced amount of data and to achieve rotation or translation invariant predictions.  Fully 3D CNNs come with an increased number of parameters and significant memory and computational requirements due to 3D convolutions. In addition, the number of available medical images is always limited and training in 2D provides the ability to utilize pretrained networks and fine-tuning. We decided to leverage the advantages of 2D training and design an architecture inspired by Fully Convolutional Networks (FCN) ~\citep{Lon15} and the Holistically-nested Edge Detection (HED) network proposed in ~\citep{Xie15}. 
 ...
 %Brebisson et al. \cite{Bre15} moved closer to 3D CNNs by extracting large 2D patches from multiple scales of the image and combining them with small single-scale 3D patches, in order to avoid the memory requirements of fully 3D networks. Lately, preliminary approaches to direct 3D CNN-based volume segmentation have been presented, which employ 3D image patches and 3D convolutions \cite{Kam17}. A 3D CNN trained end to end for prostate MRI segmentation is presented in \cite{Mil16v}. 
...
%In a final experiment to test the whole pipeline together, the output of the detection phase is directly used for the segmentation. This data consists of...
...
%We have applied cross-validation for training and testing. Thus, we have trained and tested Detectnet 4 times with different datasets to provide more robustness. The number of training and testing images changes depending on the cross-validation submodel. 
%Training data consists of all the slices extracted from the post-operative CTA volumes of 10 different patients, which leads to a total of 3682 images, the thrombus being present in about 35\% of them. 



%The employed CTAs have been obtained with scanners from different manufacturers and models, Toshiba Aquilion, GE Lightspeed RT16, GE optima CT660 and GE Lightspeed VCT. They have varying spatial resolution, ranging from 0.72-0.97 in x and y, and 0.625-0.8 in z. These images have been divided into training set (90\%) and validation set (10\%). The labels corresponding to the images that contain thrombi are the 2D coordinates of the rectangular bounding boxes around the thrombus region, delimited manually by an expert. 
...
%Finally, we have tested the detection of each subnetwork with images of 3 previously unseen datasets from different patients.The number of training and testing images for each cross-validation subnetwork changes, as presented in Table~\ref{tab:dataDetect}. These datasets have been obtained with GE Lightspeed RT16 and GE Lightspeed VCT scanners and have a spatial resolution ranging from 0.725 to 0.925 in x and y, and 0.625-0.799 in z. 
...
%As seen in our literature review, most DCNN-based medical image segmentation approaches  still work with 2D slices, since medical image data is very limited for 3D training to have a good generalization. Training in 2D provides additional advantages regarding speed,  lower  memory  consumption and the ability to utilize pre-trained nets and fine-tuning. Hence, we have decided to train and test our network with 2D slices, providing the 3D coherence in a posterior step. 
...
%In all of our experiments for thrombus segmentation, training data consists of 2D images extracted from regions of interest from 10 patients' post-operative volumes, and include both thrombus and context information. We assume that this contextual information is essential to obtain an accurate segmentation, since relative location of the thrombus with respect to other anatomical structures provides valuable information. As explained in  Sec.~\ref{sec:soa}, small windows fail to capture the available contextual information, decreasing the classification accuracy. The number of pixels corresponding to the thrombus is then much smaller than the number of pixels corresponding to background. 
%Testing data consist of the 2D images from the remaining 3 post-operative datasets not utilized for the training in each case. The number of images used for training and testing varies for each cross-validation model depending on the utilized subset, as summarized in Table.~\ref{tab:data}. The training images have been divided into training set (90\%) and validation set (10\%). The labels corresponding to the images are 2D binary masks delimited by an expert. 

%We have fine tuned our network from weights from the original HED network, setting the learning rate to 10e-3, with step-wise learning rate decay and Stochastic Gradient Descent (SGD) optimization. We also fine tuned FCN and HED for comparison purposes. For FCNs we use batch size of 2 with batch accumulation of 2. For HED and our proposed architecture we use batch size of 4 and no batch accumulation. 
...
%{} & Method & \multicolumn{2}{c|}{Dataset 1} & \multicolumn{2}%{c|}{Dataset 2} & \multicolumn{2}{c|}{Dataset 3}\\
%\multirow{4}{*}{M0} &
%{} &  Z\textsubscript{min} & Z\textsubscript{max} & Z\textsubscript{min} & Z\textsubscript{max} & Z\textsubscript{min} & Z\textsubscript{max}  \\
%\bottomrule
%{} & Expert 1 & 189 & 339 & 155 & 326 & 246 & 431\\
%{} & Expert 2 & 182 & 337 & 164 & 326 & 248 & 432\\ 
%{} & Expert 3 & 190 & 336 & 164 & 326 & 250 & 430\\
%{} & Expert's mean & 187\(\pm\)4 & 337\(\pm\)2 & 161\(\pm\)5 & 326\(\pm\)0 & 248\(\pm\)2 & 431\(\pm\)1\\
%{} & Automatic DCNN & 165 & 336 & 152 & 326 & 230 & 427\\
%\bottomrule
%\multirow{2}{*}{M1} &
%Expert 1 & 140 & 313 & 145 & 286 & 142 & 318\\
%{} & Expert 2 & 143 & 312 & 154 & 287 & 145 & 319\\ 
%{} & Expert 3 & 145 & 308 & 163 & 286 & 191 & 319\\
%{} & 
%Expert's mean & 143\(\pm\)3 & 311\(\pm\)3 & 154\(\pm\)9 & 286\(\pm\)1 & 159\(\pm\)27 & 319\(\pm\)1\\
%{} & Automatic DCNN & 161 & 288 & 147 & 310 & 176 & 313\\
%\bottomrule
%\multirow{2}{*}{M2} &
%Expert 1 & 154 & 301 & 277 & 471 & 165 & 350\\
%{} & Expert 2 & 157 & 315 & 284 & 457 & 170 & 349\\ 
%{} & Expert 3 & 203 & 302 & 327 & 457 & 199 & 345\\
%{} &
%Expert's mean & 171\(\pm\)27 & 306\(\pm\)8 & 296\(\pm\)27 & 462\(\pm\)8 & 178\(\pm\)18 & 348\(\pm\)3\\
%{} & Automatic DCNN & 147 & 295 & 250 & 471 & 151 & 335\\
%\bottomrule
%\multirow{2}{*}{M3} &
%Expert 1 & 135 & 223 & 242 & 431 & 182 & 332\\
%{} & Expert 2 & 141 & 252 & 236 & 434 & 174 & 329\\ 
%{} & Expert 3 & 140 & 222 & 252 & 431 & 180 & 315\\
%{} & 
%Expert's mean & 139\(\pm\)3 & 232\(\pm\)17 & 243\(\pm\)8 & 432\(\pm\)2 & 179\(\pm\)4 & 325\(\pm\)9\\
%{} & Automatic DCNN & 126 & 272 & 296 & 428 & 194 & 296\\
%\bottomrule
%\end{tabular}
%\caption{\label{tab:detect iniend}Results of initial and final thrombus slices as mean and standard deviation of the values selected by three expert observers and the automatic detection network. }
%\end{table}
...
%{M0} & {FNR} &  {0.006} & {0} & {0.02}\\
%{M1} & {FNR} &  {0.107} & {0} & {0.143}\\
%{M2} & {FNR} &  {0.081} & {0} & {0.076}\\
%{M3} & {FNR} &  {0} & {0.300} & {0.282}\\
%\bottomrule
%\end{tabular}
%\caption{\label{tab:detect fn}False negative rate computed as the ratio between the undetected thrombus slices and the total thrombus slices considering the mean of all the expert observers.}
%\end{table}
...
%Net & \thead{Skip \\ connections} & \thead{Stride \\ deconv} & \thead{Training loss \\ (Mean)} &\thead{Validation loss \\ (Mean)} & \thead{Learned \\ parameters} \\\hline
%FCN14 & 2 & 14 & 0,017 & 0,024 & 134.409,791 \\
%FCN26 & 1 & 26 &  0,009 & 0,016 & 134.486,882 \\
%FCN46 & 0 & 46 & 0,013 & 0,011 & 134.702,069 
%\end{tabular}
%\caption{\label{tab:FCN training}Summary of FCN training.}
%\end{table}
...
%\centering
%\begin{tabular}{l|r|r|r|r|r|}
%%Net & Total overlap & Jaccard & Dice & FN & FP \\\hline
%FCN14 & 0.79 \(\pm\) 0.085 & 0.39 \(\pm\) 0.063 & 0.54 \(\pm\) 0.077 & 0.21 \(\pm\) 0.085 & 0.56 \(\pm\) 0.088 \\
%FCN26 & 0.71 \(\pm\) 0.104 & 0.47 \(\pm\) 0.046 &  0.63 \(\pm\) 0.048 & 0.29\(\pm\) 0.104 & 0.40 \(\pm\) 0.079\\
%FCN46 & 0.88 \(\pm\) 0.077 &  0.56 \(\pm\) 0.078 & 0.71 \(\pm\) 0.063 & 0.12 \(\pm\) 0.077 & 0.38 \(\pm\) 0.077
%\end{tabular}
%\caption{\label{tab:FCN testing} Results of FCN testing after post-processing.}
%\end{table}
...
%\subsection{Complete pipeline evaluation}
...
% Hence, the automatic segmentation results are reasonably good, but the measured volume values still need to be refined to be directly applicable in the clinical practice for quantitative progression assessment. \par
