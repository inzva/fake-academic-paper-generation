...
%What is COPD and why do we want to classify images of it?
...
%How is this being done now and what is wrong with it?
...
%What are two main types of MIL methods and how do they work?
...
%An initial study of bag dissimilarities for COPD~\cite{sorensen2010image} shows promising results, and demonstrates the potential of dissimilarity-based methods.
...
%simple logistic noisy & 65.3 & 62.7 & 57.8 ( 1.7)\\
%simple logistic avg   & {\bf 72.5 }& {\bf 72.9 }& 70.4 ( 2.4)\\
%simple k-NN noisy     & 68.2 & 62.5 & 60.6 ( 3.8)\\
%simple k-NN avg       & {\bf 70.2 }& {\bf 69.1 }& 67.6 ( 2.6)\\
%em-dd									& 56.1 & 64.1 & 61.5 ( 4.5) \\
%miSVM noisy           & 66.9 & 67.4 & 62.3 ( 5.2)\\
%miSVM avg             & {\bf 73.6 }& {\bf 72.1 }& 70.3 ( 2.6)\\
%milboost           & 63.7 & 55.1 & 57.5 ( 9.4)\\
%citation k-NN         & 64.1 & 69.1 & 66.4 ( 2.6)\\
%mean-inst SVM         & {\bf 75.5 }& {\bf 76.1 }& {\bf 71.8 ( 3.2)}\\
%extremes SVM          & 66.5 & 69.6 & 68.0 ( 3.5)\\
%bow SVM               & {\bf 72.3 }& 65.7 & 68.3 ( 3.8)\\
%MILES                 &  64.9 &  67.8 & 65.2 ( 5.4)\\
%meanmin SVM           & {\bf 74.0 }& {\bf 75.4 }& {\bf 71.8 ( 4.3)}\\
%meanmin k-NN          & {\bf 71.1 }& 66.1 & 64.5 ( 2.0)\\
%emd SVM               & {\bf 74.7 }& {\bf 74.0 }& {\bf 70.8 ( 3.7)}\\
%emd k-NN              & 68.7 & 65.1 & 65.5 ( 3.2)\\
...
%tr: significant (p=0.0157)
%sub: not significant (p=0.3580)
...
%Furthermore, using $\mathcal{X}_{val}$ to set the parameters also has a positive effect on performance.
...
%Because of the difference in performances of noisy-or and averaging, we also examine other fusion rules, where the fraction $n_p$ of instances needs to be positive for a positive bag label. The plots in~\ref{fig:quantiles} show the performances when varying $n_p$ for the two training datasets, $\mathcal{X}_{sub}$ and $\mathcal{X}_{tr}$. Here we see that for $\mathcal{X}_{sub}$, the best performances correspond to a higher (0.7) fraction of positive instances, than for $\mathcal{X}_{tr}$. This could be caused by the differences in separability of the two datasets.

%The dataset $\mathcal{X}_{sub}$ only contains bags with moderate to severe COPD, where a larger portion of the image is affected. Therefore, the propagated instance labels are less noisy, as more of the instances in positive bags are, in fact, positive. The instance boundary is then in a low density area, in between the positive and negative instance classes. As there is still class overlap, a lot of instances need to be classified as positive for the bag to be positive.  The label noise in $\mathcal{X}_{tr}$ is higher, because bags with mild COPD are included in the data. This causes the density of the positive class to increase in the area where the instance boundary for $\mathcal{X}_{sub}$ is located. This shifts the instance boundary such that more instances are classified as positive.

%\begin{figure}[ht]
% \centering
%  \includegraphics[width=0.8\columnwidth]{../quantile_paper.pdf}
% \caption[]{AUC performance for simpleMIL when varying the fraction $n_p$ of instances, needed to define a positive bag label, when training on $\mathcal{X}_{sub}$ and $\mathcal{X}_{tr}$}
% \label{fig:quantiles}
%\end{figure}
...
%To further evaluate the classifiers, we examined the COPD stages~\cite{rabe2007global}, from mild (I) to moderate (III), of the images that still get misclassified. Most of the confusion is between the healthy scans, and stage I scans, which supports our intuition about where the class overlap is largest.
...
%By averaging the posterior probabilities of the best three classifiers (mean-inst, $d_{meanmin}$+SVM and simpleMIL+logistic, which are all trained on $\mathcal{X}_{tr}$), we achieve an AUC of 0.786, which is higher than any of the individual AUCs of these classifiers.
...
%In Figure~\ref{fig:pca}, the top right part of the plot contains only instances from positive bags, and is therefore a candidate for a concept region. However, the lower performances of the noisy-or rule demonstrate that it is not sufficient to only have a few such concept patches for the overall COPD diagnosis, possibly because COPD is characterized by more signs than what this concept is detecting, and/or because negative bags also contain instances in this region. Indeed, when we project the less-separable $\mathcal{X}_{te}$ onto the PCA plot, we see that instances from negative bags are also found in the candidate region. The average rule is more robust to this situation than noisy-or.

%\begin{figure}[ht]
% \centering
%  \includegraphics[width=0.8\columnwidth]{../pca_Xsub_contourXte.pdf}
% \caption[]{Density contours of the PCA projection of instances of $\mathcal{X}_{sub}$ with original features, and of negative instances of $\mathcal{X}_{te}$, projected onto the same plot.}
% \label{fig:pca}
%\end{figure}
...
%[TODO 5] What do we now know about viewing COPD as a MIL problem?
...
%The goal of the experiments is to better understand the type of a MIL problem that holds for the COPD dataset. On the one hand, it could be the case that there is a concept region with emphysema patches, and that affected lungs have at least one of these patches. On the other hand, the disease could be more spread out across the lungs, affecting most or all of the patches. Of course, a situation in between these extremes is also possible.

%We have emphasized that the class of MIL problems is larger than the traditional problems where the presence of a concept is assumed, and that in some problems, the complete distribution of instances may be more important. In such problems, methods that (incorrectly) assume the presence of a concept may fail, whereas methods that do not make this assumption and consider bags as a whole, are likely to be more successful. Such approaches include kernel, distance or dissimilarity-based methods. The latter approach works by defining dissimilarities between bags, and using standard supervised classifiers in the resulting dissimilarity space.
