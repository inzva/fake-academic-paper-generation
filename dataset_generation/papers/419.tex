...
%\usepackage{lipsum}  %rai
%\usepackage[none]{hyphenat}  %rai
%% \tolerance=1
...
%% \usepackage{colortbl}  % Coloring text
...
The subscript is often omitted.
%
%\item We use $\vE_{ij}$ to denote a matrix in which $(i,j)$th entry is one and
...
%
%% \item The set of real nonnegative numbers is denoted by $\bR_{+}$.
%
%
%\item Let $\X$ be an $n\times n$ square matrix whose diagonal entries are
%$X_{1,1}, X_{2,2}, \dots, X_{n,n}$. Define
%$\text{argdiag}(\X) := \left[ X_{1,1}, X_{2,2}, \dots, X_{n,n} \right]^\top$.
%
Let us define
$\bR_{+}:=\{ x\in\bR \,|\, x\ge 0 \}$,
$\bR_{++}:=\{ x\in\bR \,|\, x> 0 \}$,
...
%
%\item The operator $(\cdot)_{+}$ is defined as
%\begin{align*}
%\forall x\in\bR,\quad
%(x)_{+}:=
%\begin{cases}
...
%\end{align*}
%
%and $(\x)_{+}:=\left[x_{1},\dots,x_{n}\right]^\top, \, \forall \x\in\bR^{n}$.
%
...
%
%\item The operator $\odot$ is used to denote the Hadamard product. Namely,
%for three $m\times n$ matrices, $\vA$,$\vB$, and $\vC$,
%\begin{align*}
%& \vC = \vA\odot\vB
%\\
%& \Leftrightarrow \qquad
%\forall (i,j)\in\bN_{m}\times\bN_{n}, \,
...
%
%% The definition implies that $\bO^{m\times n} = \emptyset$ if $m<n$.
% A \textit{permutation} of the set $\bN_{n}$ is a bijective map from $\bN_{n}$ to $\bN_{n}$.
% We use $\cP_{n}$ to denote the family of permutations of $\bN_{n}$.
%% The $n$-dimensional probabilistic simplex is denoted by
%% \( \vDel_{n} := \{ \x\in\bR_{+}^{n}\,|\,\x^\top\1_{n}=1\}. \)
%% The symbols $\le$ and $\ge$ are used to denote not only the standard inequalities between scalars,
%% but also the componentwise inequalities between vectors.
...
%
%We wish to stop the algorithm when the solution attains the
%optimum.
...
it in practice.
%% Next, we repeated the above numerical
%% experiments 100 times, to count the number of epochs the
%% optimization algorithms consumed. We stop the algorithms
%% when the upper-bound of the objective gap is under $10^{-5}$
%% or the number of epochs reaches $500$. The box plots of the
%% observed numbers of epochs are shown in FIGURE. In the
%% setting of $\lambda=10^{-5}$, the numbers of trials the
%% cyclic algorithm attains the optimum within $500$ epochs are
%% $99$ for $n=10$ and $95$ for $n=50$, whereas the two
%% stochastic Dykstra algorithms attains the optimum in all the
%% trials.
