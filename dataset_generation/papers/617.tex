...
\usepackage{graphicx}
\usepackage{caption}
% * <sr.kheradpisheh@gmail.com> 2016-03-07T11:18:01.054Z:
%
% ^.
\usepackage{xspace}
\usepackage{multirow}
...
% Through a local inhibition mechanism, when a pooling neuron fires, it inhibits neurons in other neuronal maps located in the same position and do not allow them to fire in the next time steps. Hence, there would be at most one spike at each location throughout the time steps. It helps to
...
%Also, the complexity of the visual features gradually increases through the layers in such a way that neurons in the highest layer converge to object prototypes.
...
%Logistic regression~\cite{zou2015supervised} & 77.0\\ \hline
...
%Note that, the other methods were also trained and tested with the same protocol, five distinct instances per object category being used for each of the training and testing phases. The results indicate that the proposed SDNN outperforms the other methods with a recognition accuracy of $82.8\%$. This demonstrates the ability of the proposed SDNN to achieve an invariant object representation for multiple object categories.
...
\hline
% Spiking convolutional neural network [] & Rate-based & unsupervised &  Autoencoder & $99.2\%$\\
