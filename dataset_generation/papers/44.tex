...
%\usepackage{amssymb,amsmath,amsthm,turnstile,txfonts}
\usepackage{color}
...
%In \cite{monti2016geometric}, the authors proposed a mixture model network (MoNet) whereby they learn a patch operator to interpolate the functional value to a template. The convolution kernel is set to be a Gaussian function with learnable mean and covariance matrices. However, MoNet requires a choice of local coordinates which may suffer from the same drawback as GCNN and ACNN.
...
%\textbf{Contributions:} In this paper we develop a new generalization of the convolution operator, called parallel transportation convolution (PTC), with compactly supported kernels on continuous Riemannian manifolds their discrete counterparts. PTC is based on parallel transportation which is able to translate information along a manifold in a way that preserves orientation in an intrinsic sense. PTC is robust to manifold deformations and allows for the construction of compactly supported filters. This allows for us to preform wavelet-like operations as well as defining deep convolution networks on curved domains.
...
%Hence for any sufficiently small vector $v$ in the tangent plane at $x$ we have a unique mapping $v \rightarrow exp(v) \in \M$. 
...
%These bases are consistent (as the mesh goes to a manifold the function which maps one basis to another becomes an affine connection) and can be used to quickly compute the parallel translation of our kernel.
%To do so we find the coefficients of all of the reference vectors for $k(x_0,\cdot)$ in the local frame $\mathfrak{F}_{x_0}$. Then for any other point we can reconstruct the reference vectors by using these coefficients for the current frame. We can then interpolate the functional values from the original kernel at the newly created vectors to define the kernel values for any vector in this frame. Finally by projecting these values down to the manifold we create the translated kernel.
...
%\item Pooling layer: $f^{out}_i(x) = \max_\alpha f^{in}_{i,\alpha}(x)$. The pooling layer can be implemented using several non-linear functions among which the max pooling is the most common way.
\end{itemize}
...
%The only additional challenge is the computation of a new filter during training while using back propagation. Traditionally this is accomplished by back-convolving the error at the convolution layer with the feature map the convolution creates. However, our notion of back propagation is only well defined for compactly supported domains. Instead we simply exploit the linearity of our operator, take the derivative of $K$ and update our filters.
...
\begin{figure}[htp]
\begin{center}
%\begin{wraptable}{f}{0.67\linewidth}
...
%\caption{Comparison of results from learning on single and multiple domains and then testing on a new manifold}
%\end{minipage}
...
\centering
%\includegraphics[scale=.65]{PTCpaperPics/FaustFeats.png}
...
%
%\begin{figure} \label{fig:FaustResult}
%\includegraphics[scale=.35]{PTCpaperPics/Faust1.jpg}
%\caption{Geodesic Errors in predicted correspondence}
%\end{figure}
