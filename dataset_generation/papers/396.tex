...
% The paper headers
%\markboth{IEEE TRANSCATIONS ON MULTIMEDIA}%
%{IEEE TRANSCATIONS ON MULTIMEDIA}
...
%\subsection{Graph-based unsupervised extension}
%The above formulation is under a semi-supervised setting, however, our proposed SCH-GAN can also be extended into a unsupervised version. In our proposed SCH-GAN, only the discriminative model involves labeled information, which uses the semantic information to guide the training of generative model. In the unsupervised SCH-GAN, instead of using label information, we use the underlying data manifold to guide the training process. Specifically, we first construct undirected graphs $G_i=(V,W_i)$ and $G_t=(V,W_t)$ for image and text modality respectively, where $V$ denotes the vertices and $W$ is the similarity matrix defined as follows:
%\begin{equation}
%\label{equknn}
%w_i(p,q)=\left\{ \begin{array}{rrr}
%1 :&  x_p^i \in NN_k(x_q^i)~or~x_q^i \in NN_k(x_p^i)\\
%0 :&  otherwise \\
%\end{array}
%\right.
%\end{equation}
%where $NN_k(x_q^i)$ denotes the $k$-nearest neighbors of $x_q^i$ in the training set. Then in equations~(\ref{minimaxgame_ti}) and (\ref{minimaxgame_it}), we sample the data of the true distributions based on the constructed graph. For $p_{true}(i|q_t^j,r)$, we select $i_k$ as the true relevant image of given text $q_t^j$, when $w_t(k,j)=1$. It is noted that pairwise information is naturally existed in cross-modal data, thus if the corresponding text $t_k$ is similar with query $q_t^j$, the paired image $i_k$ should also be similar with $q_t^j$. By this definition, we intend to utilize the underlying data manifold of different modality to guide the training of generative model, and improve the retrieval accuracy of unsupervised cross-modal retrieval.
...
%		MSAE~\cite{MSAE}     &       &       &       &       &       &       &       &       \\ %\hline
%		MDNN~\cite{MDNN}     &       &       &       &       &       &       &       &       \\ %\hline
...
\begin{itemize}
	...
	%\item MLBE~\cite{mlbe} is a probabilistic latent factor model, where the hash codes are the binary latent factors in a common Hamming space which
	%determine the generation of both intra-modality and inter-modality similarities.
	...
	%\item MSAE~\cite{MSAE} utilizes stacked auto-encoder to map features of different modality into a common Hamming space by preserving the intra and inter correlations.
	%\item MDNN~\cite{MDNN} is a supervised deep learning based method, which uses deep convolutional neural network (DCNN) model and a neural language model (NLM) to learn mapping functions for the image modality and the text modality respectively.
