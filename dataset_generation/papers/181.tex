...
%\usepackage[colorlinks,urlcolor=blue,citecolor=red,bookmarks=false]{hyperref}

%\renewcommand\cap[3]{\caption[#2]{\label{#1}\textsc{#2} \small{#3}}}
%\newcommand\ic[2][1]{\includegraphics[width=#1\textwidth]{#2}}
...
%\title{CEREALS -- Cost-Efficient REgion Exploiting Active Learning for Semantic Segmentation}
\title{CEREALS -- Cost-Effective REgion-based Active Learning for Semantic Segmentation}
% Enter the paper's authors in order
% \addauthor{Name}{email/homepage}{INSTITUTION_CODE}
% \addauthor{Radek Mackowiak\raise0.7ex\hbox{\scriptsize{1,}}}{radek.mackowiak@de.bosch.com}{2}
...
%The modified FCN8s-architecture utilized in this work has significant lower network capacity than state of the art semantic segmentation models as \textit{DeepLabv3+} \cite{DBLP:journals/corr/abs-1802-02611} or \textit{PSPNet} \cite{DBLP:conf/cvpr/ZhaoSQWJ17}. 
...
% Though real annotation costs are not only being dependent on the number of clicks but also on the time to travel to a specific location \cite{doi:10.1137/1.9781611972795.70} and other factors like the human annotators \cite{DBLP:conf/ijcai/HuangCMZ17}, clicks can only be seen as a part contributing to the overall costs. Since we are restricting the areas to be annotated to small quadratic regions, we can relax the dependency in terms of travel time. We propose future training sets to be released with ground truth cost annotation for example the information how much time a labeler has been spent in total on each individual pixel, for more accurate cost information. % The next steps towards the improvement of our active learning framework for semantic segmentation are focusing on the investigation of dynamic region sizes by utilizing reinforcement learning. Furthermore, we are aiming to look into the power of diversely trained ensembles for query selection. \section{Acknowledgements}
