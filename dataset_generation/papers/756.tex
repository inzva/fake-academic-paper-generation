...
% There has been a recent push in robotics to move from analytical reasoning to more data-driven and self-supervised learning of planning and control~\cite{}. Specifically, the paradigm of end-to-end learning has gained a lot of prominence. In an end-to-end learning framework, the input is the perceived image and the output is the action primitives or torques themselves. Amazingly, it has been shown that given a specific task, if enough amount of data is collected, these end-to-end learning frameworks can outperform manually designed mathematical models~\cite{Pinto16,Guided policy search?, GoogleISER16}.

% However, the critics of self-supervised learning often argue that the amount of data required to learn these models effectively is huge and presents a big bottleneck. For example, in a recent paper, ~\cite{Pinto16} collect more than 50K examples to learn a grasping model. Furthermore, since most end-to-end learning approaches deal with large amounts of data, they use self-supervision from other sensors. This more than often leads to weaker notion of success. For example, ~\cite{Pinto16} uses force sensor in the wrist to see if the robot was successful in grasping the object. This often leads to unstable grasps (see figure~\ref{} for examples) being classified as a good grasps in the self-supervised framework. 
\end{abstract}
...
%In fact, we show that having an adversarial robot is better than having two robots collecting data in parallel and pooling the data-points to learn the final model for the task. \section{Adversarial Learning Framework}\subsection{Formulation}
...
%Note that the agent's and the adversary's state representations, $s^P$ and $s^A$ respectively, may be different. However $s^A$ is a function of both $s^P$ and $u^p$, since different actions by the agent could present the adversary with different states of the world.\begin{figure*}[t!]
\begin{center}
...
%$L^P(u^p)=1$ if the agent action $u^p$ is fails to perform the task and $L^P(u^p)=1$ if it succeeds. $L^A(u^a)=1$ if the adversary action $u^a$ succeeds and causes the failure of a successful agent action and $L^A(u^a)=0$ if the adversary's action is unable to fail the successful agent.
...
%Hence to encode the $(x_s,y_s)$ configuration, we sample a patch $I_G$ centered at the location of $(x_s,y_s)$ in the image $I$. 
...
% If this grasp is successful, the loss/cost of this grasp $L^P(u^p)=0$ and if it fails, the loss of this grasp $L^P(u^p)=1$\begin{figure*}[t!]
\begin{center}
...
%We also report the number of times a failure of grasp occurred because of the object slipping away or rotating out after an initial grasp. This is measured by noting the initial grasp force applied on the object before lifting it up. 
