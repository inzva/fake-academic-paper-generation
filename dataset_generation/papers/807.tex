...
Applying inversion techniques on the \emph{untrained} VGG with random weights, we
 ...
 %Therefore, what is gained by the training and what is gained by just the CNN network structure?
%And what else could we explore using networks with purely random weights?
...
and create artistic imagery using random weight CNN.
 ...
 %We wish our work shed light on what is learned by the training and what is learned by purely the network structure.
...
%\makeindex book.idx \makeindex book.idx
...
%\footnote{The code can be downloaded from the GitHub after the review.}.
%based on the VGG-19 network architecture.

%\subsection{Inverting the deep representations}
...
%\texttt{x}^* = \operatornamewithlimits{argmin}_{ \texttt{x} \in \mathbb{R}^{H \times W \times C} } \mathcal{L} (F^l(\texttt{x}), F^l(\texttt{x}_0)) = \frac{1}{2 N_l \cdot M_l} \operatornamewithlimits{argmin}_{ \texttt{x} \in \mathbb{R}^{H \times W \times C} } \frac{\lVert F^l(\texttt{x})- F^l(\texttt{x}_0) \rVert _2^2}{  avg[F^l(\texttt{x}_0)] ^2 }
...
%is described in the following by Equation (\ref{eq:EdisWl2}).


%The normalization by $\lVert F^l(\texttt{x}_0) \rVert _2^2$ doesn't affect the reconstruction result, but is added for the combination of content and style
%artwork generation described in the third portion of this section.
%And we normalize the objective by the square of the mean on elements in $F^l$.
...
%by minimizing the average squared Euclidean loss between the target image and the pre\_image,
and apply the L\_BFGS gradient descent using standard error backpropagation to morph the input pre\_image to the target.
 %\texttt{x}_{t+1}  = \texttt{x}_t - \frac{ \partial  \mathcal{L}^l(\texttt{x},\texttt{x}_0) }{ \partial \texttt{x} }
\setlength{\belowdisplayskip}{2pt}
\setlength{\abovedisplayskip}{2pt}
\begin{align}
...
%For the current representation layer $l$, we calculate the approximate maximum gradient by Equation (\ref{eq:EdisWl2}), and back propagate the gradient to the input layer,
%and use the mean gradient over all pixels and RGB channels as the weighting factor value.
...
%The intuition on the estimation is that when $\texttt{x}$ approaches $\texttt{x}_0$, we could estimate the maximal gradient by replacing $\texttt{x}$ to $\texttt{x}_0$, and assuming the activations of the other image $\texttt{x}'$ are all zeros.
...
\setlength{\abovedisplayskip}{2pt}
%\begin{equation}
%\frac{\partial \mathcal{L}^l(\texttt{x}_0,\texttt{x}')}{\partial{F_{i,k}}}\bigg|_{F^l(\texttt{x}')=0} = \frac{1}{N^l M^l}F^l(\texttt{x}_0)_{i,k}
%\label{eq:EdisWl1}
%\end{equation}
\begin{equation}
...
%Due to random weights, it's difficult to balance the different layer's influence
%on the reconstruct image. Therefore, we propose a pre-processing method to solve this problem.
%In every ? layer, we calculate the gradient on this layer and back propagate gradient from
% this layer to input layer. Then  we obtain the gradient on input layer, and calculate the mean,
% as $\omega_{l}$. When we do the inversing image or texture synthesis, the loss on $l$ layer needs
% to divide $\omega_{l}$. We want scale each layer to make sure that they have the same mean of gradient
% on input layer.
...
% $F_{ik}^l$ is the activation of the $i^{th}$ filter at position $k$.
...
%And a combination of the loss contributions of several candidate layers gives rise to total loss.
\setlength{\belowdisplayskip}{2pt}
\setlength{\abovedisplayskip}{2pt}
\begin{equation}
...
%\texttt{x}^* = \operatornamewithlimits{argmin}_{ \texttt{x} \in \mathbb{R}^{H \times W \times C} } \mathcal{L} [G^l(F^l(\texttt{x})), G^l(F^l(\texttt{x}_0))] = \frac{\mu_l}{4N_l^2 \cdot M_l^2} \operatornamewithlimits{argmin}_{ \texttt{x} \in \mathbb{R}^{H \times W \times C} }  \lVert G^l(F^l(\texttt{x}))- G^l(F^l(\texttt{x}_0)) \rVert _2^2
\label{eq:textureLoss}
\end{equation}
%\begin{equation}
%\texttt{x}^* = \operatornamewithlimits{argmin}_{ \texttt{x} \in \mathbb{R}^{H \times W \times C} } \mathcal{L} [G^l(F^l(\texttt{x})), G^l(F^l(\texttt{x}_0))] = \frac{1}{4N_l^2 \cdot M_l^2} \operatornamewithlimits{argmin}_{ \texttt{x} \in \mathbb{R}^{H \times W \times C} } \frac{\lVert G^l(F^l(\texttt{x}))- G^l(F^l(\texttt{x}_0)) \rVert _2^2}{ avg[ G^l(F^l(\texttt{x}_0)) ]^2}
%\label{eq:textureLoss}
%\end{equation}
...
%In every ? layer, we calculate the gradient on this layer and back propagate gradient from
% this layer to input layer. Then  we obtain the gradient on input layer, and calculate the mean,
% as $\gamma^{l}$. When we do the inversing image or texture synthesis, the loss on $l$ layer needs
% to divide $\gamma^{l}$. We want scale each layer to make sure that they have the same mean of gradient
% on input layer.

%The weighting factor $\mu_{l}$ is applied to normalize the gradient impact on the input layer.
%For each candidate layer $l$, we approximately calculate the gradient scale by Equation (\ref{eq:TextureW})
%and back propagate the gradient to the input layer.
%Then we regard the reciprocal of the mean gradient over all pixels and RGB channels as the value of $\mu_{l}$.
...
\setlength{\abovedisplayskip}{2pt}
%\begin{equation}
% \frac{\partial E(\texttt{x},\texttt{x}_0,l)}{\partial{F^l_{i,k}}} = \frac{1}{N^2_l M^2_l}[(F^l(\texttt{x}_0))^T(G^l(F^l(\texttt{x}_0))-0)]_{i,k}
%\end{equation}
\begin{equation}
...
%by combining the reconstruction loss $\mathcal{L}_{content}$ and the texture loss $\mathcal{L}_{texture}$.
...
%In the experiments, we select conv2\_2 as the content layer, and use the combination of conv1\_1,  conv2\_1, ..., conv5\_1 as the style. We set the ratio of $\alpha :\beta : \gamma = 100 : 1 : 1000$.
...
%For texture synthesis and style transfer which only need representations of the convolutional layers and pooling layers,

\label{sec:experiments}
%\input{040Experiments}
%Gatys et al.\cite{Gatys2015texture} have tried the VGG architecture but random weights and failed in the texture synthesis. And they claim that the learned filters are crucial for texture generation.
...
%obtain insights into the invariances captured by the representation.
...
%\centering
%	%\subfigure{
%		%\label{fig:monkeySingleLayerconv11} %% label for first subfigure
...
%			~~~~~~~\scriptsize{conv3\_2}~~~~		%}
%            \\
%			\includegraphics[width=0.65in]{./monkey_conv1_1.png}%
%			\includegraphics[width=0.65in]{./monkey_conv1_2.png}%
%			\includegraphics[width=0.65in]{./monkey_pool1.png}%
%			\includegraphics[width=0.65in]{./monkey_conv2_1.png}%
%			\includegraphics[width=0.65in]{./monkey_conv2_2.png}%
%			\includegraphics[width=0.65in]{./monkey_pool2.png}%
%			\includegraphics[width=0.65in]{./monkey_conv3_1.png}%
%			\includegraphics[width=0.65in]{./monkey_conv3_2.png} \\
%		%\end{minipage}%}
%          \vspace{.3em}
%	%\subfigure{
%		%\begin{minipage}[b]{1\textwidth}
...
%			~~~~~~~\scriptsize{pool4}~~~~~~	%}
%			\\
%			\includegraphics[width=0.65in]{./monkey_conv3_3.png}%
%			\includegraphics[width=0.65in]{./monkey_conv3_4.png}%
%			\includegraphics[width=0.65in]{./monkey_pool3.png}%
%			\includegraphics[width=0.65in]{./monkey_conv4_1.png}%
%			\includegraphics[width=0.65in]{./monkey_conv4_2.png}%
%			\includegraphics[width=0.65in]{./monkey_conv4_3.png}%
%			\includegraphics[width=0.65in]{./monkey_conv4_4.png}%
%			\includegraphics[width=0.65in]{./monkey_pool4.png}\\
%		%\end{minipage}%}
%    \vspace{.3em}
%	%\subfigure{
%		%\begin{minipage}[b]{1\textwidth}
...
%			~~~~~~~\scriptsize{fc7}~~~~~~~~~~~~
%			~~~~~~~\scriptsize{fc8}~~~~~~~~~~	\\%}
%			\includegraphics[width=0.65in]{./monkey_conv5_1.png}%
%			\includegraphics[width=0.65in]{./monkey_conv5_2.png}%
%			\includegraphics[width=0.65in]{./monkey_conv5_3.png}%
%			\includegraphics[width=0.65in]{./monkey_conv5_4.png}%
%			\includegraphics[width=0.65in]{./monkey_pool5.png}%
%			\includegraphics[width=0.65in]{./monkey_fc6.png}%
%			\includegraphics[width=0.65in]{./monkey_fc7.png}%
%			\includegraphics[width=0.65in]{./monkey_fc8.png}
%		%\end{minipage}%}
%	\vspace{-1em}
%	\caption{\textbf{Reconstructions of the monkey from each layer of the random weight CNN, ranVGG.} The monkey image is well reconstructed from activations of any of the 16 convolutional layers after the rectifier and the 5 average pooling layers, and we could still see the rough contours from the first two fully connected layers.}% The figure is best viewed in color/screen. }%The last layer fc8 rarely shows the contours and hence is ignored here. }
%	\label{fig:MonkeySingleLayer} %% label for entire figure
...
%($11 \times 11$, $5 \times 5$ for the first two convolutional layers, and $3 \times 3$ for the following 3 convolutional layers)
...
%			\rotatebox{90}{~\scriptsize{Ours on VGG}~~~~} \\
%			\rotatebox{90}{\scriptsize{\cite{Mahendran2015CVPR} on AlexNet}~~~}
%		\end{minipage}}
%	\subfigure{%[pool1]
%		\begin{minipage}[b]{0.12\textwidth}
%			\centering{\scriptsize{pool1}} \\
%			\includegraphics[width=0.7in]{./rand_monkey_pool1.png} \\ %\vspace{.1em}
%			\includegraphics[width=0.7in]{./vgg_monkey_pool1.png} \\ %\vspace{.1em}
%			\includegraphics[width=0.7in]{./AlexPool1.jpg}\\
%		\end{minipage}}
%	\subfigure{
%		\begin{minipage}[b]{0.12\textwidth}
%			\centering{\scriptsize{pool2}} \\
%			\includegraphics[width=0.7in]{./rand_monkey_pool2.png} \\
%			\includegraphics[width=0.7in]{./vgg_monkey_pool2.png} \\
%			\includegraphics[width=0.7in]{./AlexPool2.jpg}
%		\end{minipage}}
%	\subfigure{
%		\begin{minipage}[b]{0.12\textwidth}
%			\centering{\scriptsize{pool3/conv3}} \\
%			\includegraphics[width=0.7in]{./rand_monkey_pool3.png} \\
%			\includegraphics[width=0.7in]{./vgg_monkey_pool3.png} \\
%			\includegraphics[width=0.7in]{./AlexConv3.jpg}
%		\end{minipage}}
%	\subfigure{
%		\begin{minipage}[b]{0.12\textwidth}
%			\centering{\scriptsize{pool4/conv4}} \\
%			\includegraphics[width=0.7in]{./rand_monkey_pool4.png} \\
%			\includegraphics[width=0.7in]{./vgg_monkey_pool4.png} \\
%			\includegraphics[width=0.7in]{./AlexConv4.jpg}
%		\end{minipage}}
%	\subfigure{
%		\begin{minipage}[b]{0.12\textwidth}
%			\centering{\scriptsize{pool5}} \\
%			\includegraphics[width=0.7in]{./rand_monkey_pool5.png} \\
%			\includegraphics[width=0.7in]{./vgg_monkey_pool5.png} \\
%			\includegraphics[width=0.7in]{./AlexPool5.jpg}
%		\end{minipage}}
%	\vspace{-0.5em}
%	\caption{\textbf{Reconstructions from layers of ranVGG (top) and the pretrained VGG (middle) and \cite{Mahendran2015CVPR} (bottom).} As AlexNet only contains 3 pooling layers, we compare their results on conv3 and conv4 with ours on pool3 and pool4. Our method on ranVGG demonstrates a higher perceptive quality, especially on the higher layers. Note that VGG is much deeper than AlexNet even when we compare on the same pooling layer.} %, even of we double the gradient descent iterations.} %The figure is best viewed in color/screen.} stronger generative power
%	\label{fig:EdisCompareMonkey} %% label for entire figure
...
%    \centering
%	\subfigure{
%		\begin{minipage}[b]{0.015\textwidth}
%			%\rotatebox{90}{~~~~~~~~~~\small{random}~~~~~~~~	} \\
%			%\rotatebox{90}{~~~~~~~~~~\small{trained}~~~~~~~~~} \\
%			\rotatebox{90}{~~~~~~~~\scriptsize{pool1}~~~~~~~} \\
%			\rotatebox{90}{~~~~~~~~\scriptsize{pool3}~~~~~~~} \\
%			\rotatebox{90}{~~~~~~~~\scriptsize{pool5}~~~~~~~}
%		\end{minipage}}
%	\subfigure{%[pool1]
%		\begin{minipage}[b]{0.12\textwidth}
%			\centering{\small{ranVGG}} \\
%			\includegraphics[width=0.7in]{./rand_house_pool1.png} \\
%      \includegraphics[width=0.7in]{./rand_house_pool3.png} \\
%			\includegraphics[width=0.7in]{./rand_house_pool5.png}
%		\end{minipage}}
%	\subfigure{%[pool1]
%		\begin{minipage}[b]{0.12\textwidth}
%			\centering{\small{VGG}} \\
%			\includegraphics[width=0.7in]{./vgg_house_pool1.png} \\
%			\includegraphics[width=0.7in]{./vgg_house_pool3.png} \\
%			\includegraphics[width=0.7in]{./vgg_house_pool5.png}
%		\end{minipage}}
%	\subfigure{%[pool1]
%		\begin{minipage}[b]{0.12\textwidth}
%			\centering{\small{ranVGG}} \\
%			\includegraphics[width=0.7in]{./rand_longbird_pool1.png} \\
% 			\includegraphics[width=0.7in]{./rand_longbird_pool3.png} \\
%			\includegraphics[width=0.7in]{./rand_longbird_pool5.png}
%		\end{minipage}}
%	\subfigure{%[pool1]
%		\begin{minipage}[b]{0.12\textwidth}
%			\centering{\small{VGG}} \\
%			\includegraphics[width=0.7in]{./vgg_longbird_pool1.png} \\
%			\includegraphics[width=0.7in]{./vgg_longbird_pool3.png} \\
%			\includegraphics[width=0.7in]{./vgg_longbird_pool5.png}
%		\end{minipage}}
%	\subfigure{%[pool1]
%		\begin{minipage}[b]{0.12\textwidth}
%			\centering{\small{ranVGG}} \\
%			\includegraphics[width=0.7in]{./rand_girls_pool1.png} \\
%			\includegraphics[width=0.7in]{./rand_girls_pool3.png} \\
%			\includegraphics[width=0.7in]{./rand_girls_pool5.png}
%		\end{minipage}}
%	\subfigure{
%		\begin{minipage}[b]{0.12\textwidth}
%			\centering{\small{VGG}} \\
%			\includegraphics[width=0.7in]{./vgg_girls_pool1.png}	\\
%			\includegraphics[width=0.7in]{./vgg_girls_pool3.png}	\\
%			\includegraphics[width=0.7in]{./vgg_girls_pool5.png}
%		\end{minipage}}
%	\vspace{-0.5em}
%	\caption{\textbf{Reconstructions from different pooling layers of the untrained ranVGG and the pretrained VGG.}  ranVGG demonstrates a higher perceptive quality, especially on the higher layers. The pretrained VGG could rarely reconstruct even the contours from representations of the fifth pooling layer.} %, even of we double the gradicent descent iterations.} %The figure is best viewed in color/screen.} stronger generative power
%	\label{fig:EdisMultiPooling} %% label for entire figure
...
%\begin{figure}
%\centering
%\includegraphics[width=5in]{./figure/Edis/girl_combine.jpg}
%\caption{\textbf{Variations on the girl image.} Explain.}
%\label{Fig:girlsEdis}
%\end{figure}

%\begin{figure}
%\centering
%\includegraphics[width=3.5in]{./figure/Edis/layer_curve.png}
%\caption{Mean distance on the pooling layers.}
%\label{Fig:meanEdis}
%\end{figure}
...
%\begin{figure}
%\centering
%\includegraphics[width=3in]{./figure/Edis/monkeySingleLayer/monkey_conv1_1.png}
%\caption{Conv1\_1 of Monkey image.}
%\label{Fig:MonkeyConv11}
%\end{figure}

%\begin{figure}
%\centering
%\includegraphics[width=1.2in]{./figure/Edis/monkeySingleLayer/monkey_conv1_1.png}
%\includegraphics[width=1.2in]{./figure/Edis/monkeySingleLayer/monkey_conv1_2.png}
%\caption{Conv1\_1 of Monkey image.}
%\label{Fig:MonkeyConv11}
%\end{figure}
...
Considering only the lowest layer, conv1\_1, the synthesised textures are of lowest granularity, showing very local structure.
 Increasing the number of layers on which we match the texture representation (conv1\_1 plus conv2\_1 for the second row, etc),
  ...
  %The fifth column illustrates the textures of the artwork of starry night by Vincent van Gogh, 1889, which has global structure as compared with the previous four columns.
...
%			\rotatebox{90}{~~~~~~~~\scriptsize{conv1\_1}~~~} \\
%			\rotatebox{90}{~~~~~~~~~\scriptsize{conv2\_1}~~~} \\
%			\rotatebox{90}{~~~~~~~~~\scriptsize{conv3\_1}~~~} \\
%			\rotatebox{90}{~~~~~~~~~\scriptsize{conv4\_1}~~~} \\
%			\rotatebox{90}{~~~~~~\scriptsize{{\color{red}original}}~~~~} 	\\
%			\rotatebox{90}{~~\scriptsize{\color{blue}trained conv4\_1}~~}
%		\end{minipage}}
%	\subfigure{
%		\begin{minipage}[b]{0.12\textwidth}
%			\centering{\scriptsize{Camouflage}} \\
%			\includegraphics[width=0.7in]{./Camouflage_conv1_1.png}\\ \vspace{0.1em}
%			\includegraphics[width=0.7in]{./Camouflage_conv1_1_conv2_1.png} \\ \vspace{0.1em}
%			\includegraphics[width=0.7in]{./Camouflage_conv1_1_conv2_1_conv3_1.png} \\ \vspace{0.1em}
%			\includegraphics[width=0.7in]{./Camouflage_conv1_1_conv2_1_conv3_1_conv4_1.png} \\
%			%\includegraphics[width=0.7in]{Camouflage_conv1_1_conv2_1_conv3_1_conv4_1_conv5_1.png} \\
%			 \vspace{0.3em} \includegraphics[width=0.7in]{./Camouflage256.jpg} 			\\ \vspace{0.3em}
%			\includegraphics[width=0.7in]{./structTexture_Camouflage_conv1_1_conv2_1_conv3_1_conv4_1.png}
%		\end{minipage}}
%	\subfigure{
%		\begin{minipage}[b]{0.12\textwidth}
%			\centering{\scriptsize{Cargo}} 		\\
%			\includegraphics[width=0.7in]{./cargo_conv1_1.png} \\ \vspace{0.1em}
%			\includegraphics[width=0.7in]{./cargo_conv1_1_conv2_1.png} \\ \vspace{0.1em}
%			\includegraphics[width=0.7in]{./cargo_conv1_1_conv2_1_conv3_1.png} \\ \vspace{0.1em}
%			\includegraphics[width=0.7in]{./cargo_conv1_1_conv2_1_conv3_1_conv4_1.png} \\
%			%\includegraphics[width=0.7in]{cargo_conv1_1_conv2_1_conv3_1_conv4_1_conv5_1.png} \\
%			\vspace{0.3em} \includegraphics[width=0.7in]{./cargo256.jpg} \\  \vspace{0.3em}
%			\includegraphics[width=0.7in]{./structTexture_cargo_conv1_1_conv2_1_conv3_1_conv4_1.png}
%		\end{minipage}}
%	\subfigure{
%		\begin{minipage}[b]{0.12\textwidth}
%			\centering{\scriptsize{Floors}} 	\\
%			\includegraphics[width=0.7in]{./FloorsRounded_conv1_1.png} \\ \vspace{0.1em}
%			\includegraphics[width=0.7in]{./FloorsRounded_conv1_1_conv2_1.png} \\ \vspace{0.1em}
%			\includegraphics[width=0.7in]{./FloorsRounded_conv1_1_conv2_1_conv3_1.png} \\ \vspace{0.1em}
%			\includegraphics[width=0.7in]{./FloorsRounded_conv1_1_conv2_1_conv3_1_conv4_1.png} \\
%			%\includegraphics[width=0.7in]{FloorsRounded_conv1_1_conv2_1_conv3_1_conv4_1_conv5_1.png} \\
%			\vspace{0.3em} \includegraphics[width=0.7in]{./FloorsRounded256.jpg} \\  \vspace{0.3em}
%			\includegraphics[width=0.7in]{./structTexture_FloorsRounded_conv1_1_conv2_1_conv3_1_conv4_1.png}
%		\end{minipage}}
%	\subfigure{
%		\begin{minipage}[b]{0.12\textwidth}
%			\centering{\scriptsize{Flowers}} 	\\
%			\includegraphics[width=0.7in]{./FlowerBeds_conv1_1.png} \\ \vspace{0.1em}
%			\includegraphics[width=0.7in]{./FlowerBeds_conv1_1_conv2_1.png} \\ \vspace{0.1em}
%			\includegraphics[width=0.7in]{./FlowerBeds_conv1_1_conv2_1_conv3_1.png} \\ \vspace{0.1em}
%			\includegraphics[width=0.7in]{./FlowerBeds_conv1_1_conv2_1_conv3_1_conv4_1.png} \\
%			%\includegraphics[width=0.7in]{FlowerBeds_conv1_1_conv2_1_conv3_1_conv4_1_conv5_1.png} \\
%			\vspace{0.3em} \includegraphics[width=0.7in]{./FlowerBeds256.jpg} 	\\  \vspace{0.3em}
%			\includegraphics[width=0.7in]{./structTexture_FlowerBeds_conv1_1_conv2_1_conv3_1_conv4_1.png} \\
%		\end{minipage}}
%	\subfigure{
%		\begin{minipage}[b]{0.12\textwidth}
%			\centering{\scriptsize{Leaves}} 	\\
%			\includegraphics[width=0.7in]{./Leaves_conv1_1.png} \\ \vspace{0.1em}
%			\includegraphics[width=0.7in]{./Leaves_conv1_1_conv2_1.png} \\ \vspace{0.1em}
%			\includegraphics[width=0.7in]{./Leaves_conv1_1_conv2_1_conv3_1.png} \\ \vspace{0.1em}
%			\includegraphics[width=0.7in]{./Leaves_conv1_1_conv2_1_conv3_1_conv4_1.png} \\
%            \vspace{0.3em} \includegraphics[width=0.7in]{./Leaves256.jpg} 	\\  \vspace{0.3em}
%			\includegraphics[width=0.7in]{./structTexture_Leaves_conv1_1_conv2_1_conv3_1_conv4_1.png}
%		\end{minipage}}
%	\subfigure{
%		\begin{minipage}[b]{0.12\textwidth}
%			\centering{\scriptsize{Nigh Starry}} 	\\
%			\includegraphics[width=0.7in]{./night_conv1_1.png} \\ \vspace{0.1em}
%			\includegraphics[width=0.7in]{./night_conv1_1_conv2_1.png} \\ \vspace{0.1em}
%			\includegraphics[width=0.7in]{./night_conv1_1_conv2_1_conv3_1.png} \\ \vspace{0.1em}
%			\includegraphics[width=0.7in]{./night_conv1_1_conv2_1_conv3_1_conv4_1.png} \\
%			%\includegraphics[width=0.7in]{night_conv1_1_conv2_1_conv3_1_conv4_1_conv5_1.png} \\
%			\vspace{0.3em} \includegraphics[width=0.7in]{./night256.jpg} 	\\  \vspace{0.3em}
%			\includegraphics[width=0.7in]{./structTexture_night_conv1_1_conv2_1_conv3_1_conv4_1.png} \\
%		\end{minipage}}
%	\vspace{-0.5em}
%	\caption{\textbf{Generated textures using random weights.} Each row corresponds to a different processing stage in the random weight network ranVGG.
%Considering only the lowest layer, conv1\_1, the synthesised textures are of lowest granularity, showing very local structure.
...
%By comparison, results of our model on the trained VGG using four convolutional layers up to conv4\_1 are as shown at the bottom row.
...
%			~~~~~~~~~~~\small{conv4\_1}~~~~~~~
%            ~~~~~~~~~~~\small{conv5\_1}~~~~~~~ \\
%			\includegraphics[width=1in]{./figure/texture/SingleLayerScaled/cargo_conv1_1.png} %
%			\includegraphics[width=1in]{./figure/texture/SingleLayerScaled/cargo_conv2_1.png} %
%			\includegraphics[width=1in]{./figure/texture/SingleLayerScaled/cargo_conv3_1.png} %
%			\includegraphics[width=1in]{./figure/texture/SingleLayerScaled/cargo_conv4_1.png} %
%			\includegraphics[width=1in]{./figure/texture/SingleLayerScaled/cargo_conv5_1.png}\\
%			\includegraphics[width=1in]{./figure/texture/SingleLayerScaled/night_conv1_1.png} %
%			\includegraphics[width=1in]{./figure/texture/SingleLayerScaled/night_conv2_1.png} %
%			\includegraphics[width=1in]{./figure/texture/SingleLayerScaled/night_conv3_1.png} %
%			\includegraphics[width=1in]{./figure/texture/SingleLayerScaled/night_conv4_1.png} %
%			\includegraphics[width=1in]{./figure/texture/SingleLayerScaled/night_conv5_1.png}
%	\vspace{-0.5em}
%	\caption{\textbf{Single layer texture synthesis on cargo and night.} Low layer correlations correspond to spatially invariant fine-grained textures,
% while high layer correlations correspond to the global structure related to the spatial information.
% This can be explained by the increasing receptive field sizes and feature complexity along the network's hierarchy. The combinations of multiple layers
%in the previous figure helps produce clear textures of the correct granularity.}
%	\label{fig:textureSingleLayer} %% label for entire figure
%\end{figure*}

\paragraph{Style transfer.}
%We select conv1\_1,conv2\_1, conv3\_1, conv4\_1, conv5\_1 as the style representation layers, and conv2\_2 as the content representation layer
% for our artistic image generation on ranVGG.
...
%The reconstructed images and the photograph are in the same aspect ratio.
...
%	\subfigure{
%		\begin{minipage}[b]{0.02\textwidth}
%			\rotatebox{90}{~~~~~~~~~~~~\scriptsize{picture 1}~~~~~~} \\
%			\rotatebox{90}{~~~~~~~~~~~~~~\scriptsize{picture 2}~~~~~~~~~~~}\\
%			\rotatebox{90}{~~~~~~~~~~~~\scriptsize{picture 3}~~~~~~}
%		\end{minipage}}
	\subfigure{%[pool1]
		\begin{minipage}[b]{0.98\textwidth}
...
%%%help the selection of architecture and hyperparameters
%Training deep neural networks not only requires a long time but also significant high performance computational resources.
%% on the network architecture.
%The VGG network, which contains 11-19 weight layers depending on the typical architecture~\cite{Simonyan14VGG},
% takes 2 to 3 weeks on a system equipped with 4 NVIDIA Titan Black GPUs for training a single net.
%The residual network ResNet, which achieved state-of-the-art results in image classification and detection in 2015~\cite{He2015Residual}, takes
% 3.5 days for the 18-layer model and 14 days for the 101-layer model using 4 NVIDIA Kepler GPU.\footnote{http://torch.ch/blog/2016/02/04/resnets.html}
%Could we evaluate a network structure without taking a long time to train it?
%In future work, we will address the above issue by utilizing the untrained network to attempt to compare networks quickly without having to train them.
