...
% * <chenzhang@shanghaitech.edu.cn> 2018-08-06T16:29:33.204Z:
%
% ^.
...
%In addition, \emph{FocusNet} and \emph{StereoNet} complements each other: \emph{FocusNet} performs better at regions with repeated patterns, whereas results from \emph{StereoNet} is insensitive to the depth-of-field variations.
...
%In essence, DfD is a triangulation scheme \cite{schechner00}. Compared with stereo scheme, the two-dimensionality of the aperture allows more robust estimation, especially in noisy regions.
...
% \begin{center}
% %\fbox{\rule{0pt}{2in} \rule{0.9\linewidth}{0pt}}
%    \includegraphics[width=0.99\linewidth]{DFF_BDFF_Overview.pdf}
% \end{center}
% \vspace{-8pt}
%    \caption{The overall architecture of our B-DfFNet.}
% \label{fig:teaser}
% \end{figure}
...
% \begin{center}
% %\fbox{\rule{0pt}{2in} \rule{0.9\linewidth}{0pt}}
%    \includegraphics[width=0.7\linewidth]{DFF_network_mergedNets.pdf}
% \end{center}
% \vspace{-8pt}
%    \caption{Our \emph{FocusNet-v2} combines \emph{FocusNet} and \emph{EDoFNet} by using the EDoF image to refine the depth estimation.}
% \label{fig:network_mergedNet}
% \end{figure}
...
%The resultant depth from \ref{section:multiscale} has splotchy artifact, and depth bleeds across object's boundary. This is due to the fact that the convolutional network is in essence a local method. To address the problem, an all in-focus intensity image could be used to guild the filtering of the raw depth value, such as XXX. Several methods \cite{XXX} has been proposed to extract the all in-focus image from the focal stack. In this paper we construct another 20 layer VGG style network, as shown in figure \ref{fig:network_allinfocus}
...
%These cues can be distributed in different scales in an image, e.g., textureless background requiring understanding of a large area, while for complicated objects requiring attentive evaluation of small regions.
%To capture these cues in different scales, one way is to use separate streams for each resolution and fuse them in a later stage.
...
%To compensate for very large disparity, input images usually should be pre-resized. For this reason, we use an additional upsampling network at the end of the hourglass network to scale output back to non-resized resolution. However, we find that there is no significant improvement compared to the simple bilinear interpolation.
...
%On our test dataset it achieve an average mean absolute error of 3.18.
...
%Need an image of novel views of the estimated shape and with texture mapping? from a geometric approach to shape from defocus.
%
%Although there are some erroneous parts, the XXX approach get rid of large parts of the noise.
%
%Should be able to prove that using both cues will outperform the single cue.
%
%by using EDoF branch, we have a good initialization. Also, we can provide the input to the stereo layers.
%
%An example to concatenate two individual network to get a larger network: a large dataset for flow....
%
%achieving state-of-the-art accuracy among real-time methods
%
%we only use a variational approach for optional refinement of the flow field predicted by the convolutional net.
%
%A drastic difference of these methods to our approach is that they are patch based and leave the spatial aggregation to postprocessing, whereas the networks in this paper directly predict complete flow fields
%
%
%expressiveness of CNN
%
%jointly trained
%
%In order to handle trade-off between accuracy and computation time, we propose three type of network structures and their input patches.
%
%which makes the decision function more discriminative.
%

% We also shuffle the image patches after each training epoch.

% note that the cop won't move in our setup
