...
\def\iccvPaperID{907} % *** Enter the ICCV Paper ID here
...
%%%%%%%%% TITLE
% \title{Supplementary Material of Pruning Networks Using \\ Neuron Importance Score Propagation}
...
\begin{abstract}
Recent studies have demonstrated the significant redundancy in deep Convolutional Neural Networks (CNNs). 
To balance the predictive power and model redundancy of CNNs, we view the neurons in a network as a set of feature extractors. The responses of the second-to-last layer before classification (which we call the ``final response layer") are the learned features. 
% According to feature ranking techniques, the features in the final response layer have different ``importance" towards the expressiveness of the model. Given the feature importance of final responses, we 
Using feature ranking techniques, we measure how "important" (discriminative and not redundant) each neuron is in the final response layer. We then formulate network pruning as a binary integer optimization and derive a closed-form solution, with the goal of pruning the least important neurons. Based on our theoretical results, we propose the Neuron Importance Score Propagation (NISP) algorithm to propagate the importance scores of final responses to every neuron in the network. The CNN is pruned by removing neurons with least importance, and then it is fine-tuned to retain its predictive power.
We evaluate our method on several datasets with multiple CNN architectures.
The results demonstrate that our approach achieves significant full-network acceleration and compression with insignificant loss of accuracy. 

%We propose to prune neurons in the entire CNN by minimizing the response changes of the important neurons in the final response layer. To avoid the computational cost of feature ranking, we conduct it once on the final response layer, and propagate the importance of neurons in that layer to the lower layers until the first layer of a CNN. We prove that the way we propagate the importance minimizes an upper-bound of the above response changes. Then, 


%We achieve 62.69\% FLOPs reduction with 0.97\% accuracy loss by pruning AlexNet trained on ImageNet, and reduce around 43.5\% FLOPs with almost 0\% loss by pruning ResNet56/110 trained on Cifar 10.
   
%   Beyond pruning, our method can be viewed as a supervised pre-training process that helps in optimization and regularization of training a compact model to achieve better predictive power than without the pre-training.
% Our method helps in optimization and regularization of a compact model achieving better predictive power than a model of the same size that is trained from scratch.
\end{abstract}
...
%that views neurons as feature extractors and applies feature ranking techniques to identify neurons that can be safely pruned with small loss in accuracy. 

%A subsequent fine-tuning step recovers most of the accuracy lost during pruning.
%To make the measurement of importance scores tractable and consistent for an entire deep neural network, we introduce Response Importance Propagation that analyzes responses from the final response layer, and propagates the importance to the lower layers by minimizing the response changes of the important neurons in the final response layer.
%We evaluate our method on several datasets with multiple CNN structures. 

% Our method helps in optimization and regularization of training a compact model.
% Our method can also be viewed as that a supervised pre-training on a cumbersome model, which helps in optimization and regularization of training a compact model.
