...
%Conference
% \acmConference[MM'18]{ACM on Multimedia Conference}{October 2018}{Seoul, Korea} 
% \acmYear{2018}
% \copyrightyear{2018}
...
% \copyrightyear{2018} 
% \setcopyright{acmcopyright} 
% \acmConference{MM '18}{October 22--26, 2018, Seoul, Republic of Korea}
% % \ISBN{978-1-4503-5665-7/18/10}\acmPrice{$15.00}
% \doi{https://doi.org/10.1145/3240508.3240523}
...
%More extensive evaluation and analysis suggest that our HSE framework exhibits stronger superiority compared with the baseline methods when the training samples are limited. For example, if using merely 10\% training samples, it outperforms the baseline methods by about 30.0\% on the Caltech-UCSD birds dataset. 


%\emph{We will release the datasets, codes, and trained models online for further research}.
...
%Similarly, we sum up $\ell_i^c$ over the training set to obtain the total classification loss $\mathcal{L}_i^c$. 
...
% \centering
% \subfigure[]{
% \includegraphics[width=0.325\linewidth]{cmp_bl-l2.pdf}}
% \subfigure[]{
% \includegraphics[width=0.325\linewidth]{cmp_bl-l3.pdf}}
% \subfigure[]{
% \includegraphics[width=0.325\linewidth]{cmp_bl-l4.pdf}}
% \caption{Accuracy comparision of our HSE framework and the baseline method that are trained using a portion of training samples at levels $l_2$, $l_3$ and $l_4$ on the CUB dataset.}
% \label{fig:low-shot}
% \end{figure*}
...
% \subsection{Analysis on low-shot learning}
% Our HSE takes advantage of the category hierarchy that provides rich semantic correlations among categories across different levels. This can make the prediction less ambiguous since this structured correlations effectively regularizes the semantic space. Thus, the HSE can be trained using much fewer samples than the baseline methods. To validate this statement, we merely utilize a portion of training samples (i.e., 80\%, 50\%, 20\%, 10\%) of the CUB dataset to train the HSE and the baseline method, and present the accuracies in Fig. \ref{fig:low-shot}. The HSE consistently performs better than the baseline method using different portions of training samples over different levels. Remarkably, the superiority of the HSE is more evident if using fewer samples for training. For example at level $l_4$, if using 20\% training samples, the HSE outperforms the baseline method by 17.9\%, and if the training samples are even reduced to 10\%, the accuracy improvement reaches up to 30\%, a relative improvement of 114\%. Besides, the HSE  trained with merely 80\% training sample still beats the baseline method trained with all samples (95.4\%, 92.0\%, and 86.9\% by ours and 95.0\%, 91.5\%, and 85.2\% by the baseline methods at level $l_2$, $l_3$ and $l_4$). Note that the accuracy at level $l_4$ also beats all existing competitors that are trained using the whole training set, as listed in Table \ref{table:cub-sota}. These comparisons adequately support the statement that our framework requires less training samples than the baseline methods.
