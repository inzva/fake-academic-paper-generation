...
%Such transferable capabilities of deep learning features across different tasks demonstrate that they are more generic in nature.
...
%It has been shown that  deep learning  can capture local object features as well as   global features such as texture, statistics and color histograms\aacomment{needs a citation}.  As such, it has been demonstrated these extracted features can be transferred for learning new tasks, which differ from the tasks on which the neural networks were trained.\aacomment{need more citations here}
...
%Thus, by leveraging on pre-trained features and contextual information, we  train a more compact neural network for a new dataset, and for a more challenging task.

%The CLTM is used to predict the output binary labels of object occurrences in an image.
...
%We propose a model to predict structured outputs for classifying a given image to multiple-object categories using latent tree models. Our proposedmodel works as follows. We first extract fc7 features of images using  pertrained ImageNet.   We then estimate the latent group structure relating the output binary labels. We learn the parameters of the CLTM using a 3-layer neural network.
...
%skateboard, keyboard, laptop, 19 \%, 20 \%, 27 \%,

%We use the marginal probabilities of observed nodes in making binary decisions for different object categories. Note that during training there are not many instances of multiple animals being in the same image. Yet, our methods..
...
%In this work, we propose a method to construct conditional latent tree models to capture the contextual relations between objects without the need for such training sets. To learn this contextual information, we use non-parametric kernel framework that uses the distribution of fc7 features to recover the latent tree. On the other hand, such non-parametric tasks require large data-sets in high dimensional regime to perform discriminative tasks better on test sets.  So, we leverage this recovered latent tree  with neural networks, which are more effective. Through this proposed framework, we learn contextual information while training for object classification tasks alone.

%In general, learning  hierarchical structures using empirical co-occurrences of different objects requires

%\aacomment{need to add:Since there are very few positive sample images with multiple object-categories, training just based on co-occurrence is not sufficient to recover a coherent tree structure. We  leverage on extracted features to estimate moments by conditioning on them. We propose a new method to calculate the distance matrix by using a RKHS framework to estimate moments. The estimated distance matrix is then used by distance-based methods for structure recovery \cite{LTM}.}


 \subsection{Related Work}


%, from feature extraction to refined classifier models, have been proposed for various image classification tasks  \cite{Desai}. The tree structure of their model is recovered using empirical co-occurrences of objects in training images. They introduce a new image data-set with images that contain many instances of different object categories to recover the tree-structure.
...
%Contexts have been utilized in other settings, e.g.  for visual word disambiguation and for attributing semantic meanings to visual words~\cite{su2012improving}. However, the semantic attributes are explicitly introduced into the framework and are trained using independent attribute classifiers. In contrast, we learn the contextual model and allow the algorithm to introduce latent variables in order to learn various hidden attributes.
...
%They train a Markov random field (MRF) on the parameters between different locations.  They assume a fixed graph structure for the MRF based on the spatial  neighborhoods. In addition, semantic neighborhoods are learnt using correlation information.
...
%Full scene labeling is another scene understanding task consists of labeling each pixel of the image with the category of the objects it belongs to~\cite{farabet2013learning}. While we do not pursue this problem in this paper, our framework can be extended to this task, and we plan to investigate in future\aacomment{need to discuss in conclusion}

%~\cite{deng2014large} utilize probabilistic models for label relation graphs for  large scale classification tasks, but do not incorporate deep learning features.

%~\cite{nowozin2011decision} introduce decision tree fields, where local interactions between variables in a CRF are determined based on decision trees.
...
%We use a conditional latent tree model for graphical modeling of random variables $X$, $Y_{i}$ for all $i = 1,\cdots,N$ and learned latent variables $H_{m}$ for $m = 1,\cdots,M$. %The proposed model belongs to class of conditional random fields in which when conditioned on input $X$, the random variables  $W=(Y,H)$ obey the local Markov property of %graphical models, namely,  $P(W_{i}|X,W_{nbd(i)}) = P(W_{i}|X,W_{\setminus{i}})$.
...
% \begin{align*}
 %\mathcal{C}_{XX} &:= \mathbb{E} [\phi(X)\otimes \phi(X)] \\
%\mathcal{C}_{YX} &:= \mathbb{E} [ \phi(X)\otimes \Psi(Y)] \\
...
%\begin{align*}
 %\hat{\mathcal{C}}_{Y|X}  &= \frac{1}{N}\Psi_{Y}\phi_{X}^{\top}(\frac{1}{N}\phi_{X}\phi_{X}^{\top} + \lambda I)^{-1} \\
 %\mathbb{E}_{Y|x}[\Psi(Y)] &=  \Psi_{Y}(K_{XX} + \lambda NI)^{-1}\phi_{X}^{\top}
 %\end{align*}
  \begin{equation}
  %\begin{split}
...
%\begin{multline}\hat{d}_{ij} := \\
%-\log\left(\frac {|\det1(\hat{\mathbb{E} }[Y_{i} \otimes Y_{j}])|}{\sqrt{|\det1(\hat{\mathbb{E} }[Y_{i} \otimes Y_{i}])|}\sqrt{|\det1(\hat{\mathbb{E} }[Y_{j} \otimes Y_{j}])|}}\right)
%\label{distance1}
%\end{multline}
...
%\tjcomment{TODO: matlab eps files}
%\begin{figure}[H]
%\includegraphics[width =\linewidth]{classwisepr.eps}
%\psfrag{Precision}[l]{\LARGE Precision}
%\psfrag{Recall}[l]{ \LARGE Recall}
%\caption{Class-wise Precision Recall Comparison of object classes: a) Keyboard b) Baseball Glove c) Tennis %Racket and d) Bed }
%\label{fig:classwise}
%\end{figure}
