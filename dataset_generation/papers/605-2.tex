%%% THIS IS THE FINAL VERSION ACCEPTED IN ITA 2018
...
%Keith - Insert turbo coding ref here
...
%PAB: Does this suggest that we need fewer epochs to train the spare networks?

%The exact densities of every junction is given in the appendix. The only bad case is CIFAR100 `conv+3CLs' 0.22\% density, which with 3 junctions. Note that all the 0.22\% cases have $fo=1$ in all junctions. We speculate that such a low fanout in the final junction leads to degradation in performance when there are a lot of output neurons. While this theory does not explain the respectable accuracy for its 2-junction counterpart, it will be supported by results we obtained for Morse code.
...
%But accuracy drops off quickly as density is reduced.
...
%In Fig. \ref{fig-morse}(a) we showed that performance drops off for the Morse network as the density of connections is decreased below 25\%.
...
%We found that when keeping overall network density constant, optimum performance is achieved when later junctions are denser than earlier ones.
...
% PAB: Not sure this next sentence proves the point you are trying to make...  I don't' see how it proves that only the first metric is insufficient because the next sentence does not compare first metrics.
