...
%As a new methodology, network transplanting can overcome all above issues. Although current techniques do not support the learning of a huge net, preliminary experiments demonstrate the effectiveness of network transplanting.
...
%Unlike scientific issues of deep learning in Section~\ref{sec:universal}, in this subsection, we introduce technical details of network transplanting and discuss its contribution to the scope of transfer/continual/lifelong learning.
...
%Well-learned network modules usually have vast \textit{forgotten space}, and features {\small$h(x)$} in the forgotten space cannot pass through the task module {\small$g_{S}$}.
...
%This equation shows that the combination of the adapter $h$ and the generic task module {\small$g_{S}$} needs to approximate feature representations of the specific task module {\small$g_{T}$}.
...
%%%We do not revise the derivative of a convolution operation {\small$f'_{\textrm{conv}}$}, because it is independent with the input feature map of the conv-layer\textcolor{red}{\footnotemark[1]}.

%%%\textit{Note that we exclusively used all above modifications of ReLU, pooling, and dropout layers for back distillation. Besides {\small$D'_{S},D'_{T}$}, we still use original gradients {\small$D_{S}$} and {\small$D_{T}$} to optimize {\small$\mathcal{L}(y,y^{*})$}.}
...
%As discussed in Fig.~\ref{fig:problem}(b,c), for each category module $f$, we first learn an adapter to transplant $f$ to the classification module, and then learn another adapter to connect $f$ to the segmentation module. Alternatively, we reverse the order to first connect $f$ to the segmentation module and then to the classification module.
...
%%%so that features of these categories can be projected to the same feature space

%We have compared the transplant to a task module for similar categories and the transplant to a task module for dissimilar categories. It demonstrates that successful transplant requires target categories to share similar object structures.
...
%%%Learning adapters without changing the upper task module presents the core technical challenge
