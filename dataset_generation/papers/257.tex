...
\end{abstract}
%Thanks to the subspace-sparse representation, vertices corresponding to different subspaces are disconnected in the sparse similarity graph constructed by sparse subspace clustering methods via sparse representation of the data, leading to their compelling performance.
...
%In contrast, similarity-based clustering methods, e.g. K-means \cite{Duda2000} and Affinity Propagation \cite{Frey07clusteringby}, segment the data based on the similarity function and alleviate the difficult problem caused by high dimensionality. Among various similarity-based methods, subspace graph-based methods \cite{Schaeffer2007} are important wherein the edge weight of the graph serving as the data similarity, and sparse graphs, where only a few edges have non-zero weights for each vertex, are demonstrated to be especially effective for clustering high dimensional data.
%Most existing sparse subspace clustering methods use $\ell^{1}$-norm, as a convex relaxation of the $\ell^{0}$-norm, to impose the sparsity, and certain assumptions are required for the subspace-sparse recovery property.
...
\end{itemize}
%This proximal method is inspired by the proximal linearized method in \cite{BoltePAL2014}.
...
%By solving almost the same optimization problem as (\ref{eq:ssc-l1}), $\ell^{1}$-graph obtains the robust sparse representation for each data point by solving the $\ell_{1}$-norm optimization problem below:
%\begin{align}\label{eq:SR-tbd}
%\mathop {\min }\limits_{{\balpha^{i}}} {\| {{\balpha^{i}}} \|_1}\quad s.t.\;{\bx_i} = {{\bmX}}{\balpha^{i}} \quad i=1,\ldots,n
%\end{align}
%which is equivalent to
%\begin{align}\label{eq:l1graph}
%\mathop {\min }\limits_{{\balpha^{i}}} \|{\bx_i} - {\bmX}\balpha^{i}\|_2^2 + \lambda_{\ell^{1}} {\| {{\balpha^{i}}} \|_1} \quad i=1,\ldots,n
%\end{align}

%\noindent for some weighting parameter $\lambda_{\ell^{1}} > 0$, and $\bmX = [\bmX \,\,\bmI_d ]$, $\bmI_d$ is a $d \times d$ identity matrix, $\balpha^{i} \in \R^{(n+d) \times 1}$, $\balpha = [\balpha^1,\ldots,\balpha^n] \in \R^{(n+d) \times n}$ is the coefficient matrix with the element $\balpha_{ij} = \balpha_i^j$. To avoid trivial solution that $\balpha = [\bI_n \,\, \bzero]^\top$, the diagonal elements of $\balpha$ are enforced to be zero, i.e. $\balpha_{ii} = 0$ for $1 \le i \le n$.
%
%
%$\ell^{1}$-graph then performs spectral clustering on the sparse similarity matrix $\bW$ to partition the data. In the case that the subspaces from which the data are drawn are linear and independent, $\bW_{ij}$ is nonzero if and only if two points $\bx_i$ and $\bx_j$ are in the same subspace according to \cite{ElhamifarV09,ElhamifarV13}. Therefore, in this case $\bW$ exhibits block-diagonal structure with a proper perturbation of the data and spectral clustering on $\bW$ can effectively segment the data in accordance with the underlying subspaces. Extensive empirical study shows that $\ell^{1}$-graph achieves much better performance than spectral clustering on widely used Gaussian kernel graph. Moreover, it is clear that the pairwise similarity matrix (\ref{eq:W}) constructed by the coefficient matrix $\balpha$ leads to the superior performance of $\ell^{1}$-graph based clustering.
%
%In the following section, we propose $\ell^{0}$-graph, which follows the original principle of sparsely representing each data point by other data using $\ell_{0}$-norm as in (\ref{eq:ssc-l0}).
\begin{table*}[ht]
\centering
\footnotesize
...
\end{MyTheorem}
%(which often holds in practice, and it always holds in our experiments)
...
%\begin{table*}[ht]
%\centering
%\caption{\small Clustering Results on Heart, Ionosphere and MNIST Handwritten Digits Database}
...
%
%  \multirow{2}{*}{Heart}      &AC      &0.5889 &0.6037  &0.6370             &0.5963  &0.5519      &\textbf{0.6444} \\ \cline{2-8}
%                              &NMI     &0.0182 &0.0269  &0.0546             &0.0255  &0.0058      &\textbf{0.0590} \\ \hline
%
%  \multirow{2}{*}{Ionosphere} &AC      &0.7096 &0.7350  &0.5071             &0.6809  &0.6325      &\textbf{0.7692} \\ \cline{2-8}
%                              &NMI     &0.1286 &0.2155  &0.1117             &0.0871  &0.0299      &\textbf{0.2609} \\ \hline
%
