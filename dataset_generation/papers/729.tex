...
%Multi-label image recognition has recently witnessed remarkable progress due to object-proposal-based deep representation learning. However, these methods rely on hundreds of proposals, most of which either contain non-object areas that carry less discriminative information or refer to the same object, leading to redundant computation and sub-optimal performance. In this paper, inspired by the way that humans spot discriminative object regions when performing image labeling tasks, we propose a recurrent attention reinforcement learning framework to iteratively discover a sequence of attentional and informative regions that are related to different semantic objects and further predict label scores conditioned on these regions. Besides, our method explicitly models long-term dependencies among these attentional regions that help to capture semantic label co-occurrence and thus facilitate multi-label recognition. Extensive experiments and comparisons on two large-scale benchmarks (i.e., PASCAL VOC and MS-COCO) show that our model achieves superior performance over existing state-of-the-art methods in both performance and efficiency as well as explicitly identifying image-level semantic labels to specific object regions.
\end{abstract}
...
%{p{1.5cm}|p{0.30cm}p{0.30cm}p{0.30cm}p{0.30cm}p{0.30cm}p{0.30cm}p{0.30cm}p{0.30cm}p{0.30cm}p{0.30cm}p{0.30cm}p{0.30cm}p{0.30cm}p{0.30cm}p{0.30cm}p{0.30cm}p{0.30cm}p{0.30cm}p{0.30cm}p{0.30cm}|p{0.35cm}}
\hline
...
%In the future, we will explore locating the regions with better boundary and adapt the proposed method to the weakly object detection task.
