...
% SOME USEFUL LINKS% http://www.cmlab.csie.ntu.edu.tw/~blacksmith/portfolio/dedup.htm% http://docs.opencv.org/2.4/doc/tutorials/tutorials.html% http://www.cs.princeton.edu/cass/papers/spam_ceas07.pdf% http://www.cs.princeton.edu/courses/archive/spr05/cos598E/bib/p208-lv.pdf% https://github.com/jenssegers/imagehash% http://www.phash.org/\section{Introduction}%{\bf TODO: Imran \& Ferda}
...
%Contrary to existing literature on using textual social media content for disaster management, in this work, we aim at developing automatic image filtering techniques useful to filter-out deluge of irrelevant images at the onset of a disaster event.
...
% Humanitarian organizations make a call for human annotators to label features of interest (e.g., damaged shelters and roads blocked by debris) in images. For instance, the United Nations Office for the Coordination of Humanitarian Affairs (UN OCHA) employs a volunteers workforce from Digital Humanitarian Network\footnote{http://digitalhumanitarians.com/} for social media filtering.
...
%\textcolor{red}{\bf TODO: Imran and Ferda: Paper contributions paragraph needs improvement}
...
%\footnote{Due to the double-blind restriction, the online link of the system is not provided with the submission, however, the system will be available online for public use should the paper is accepted.}%.FO. due to the word count limit I decided to remove below paragraph%The rest of the paper is organized as follows. We survey a review of literature in the Related Work section. We provide details about our real-world crisis datasets and their labeling process in the Data Collection and Annotation section. We then present our automatic image filtering pipeline in the Real-time Filtering of Images section and elaborate on our experimental results and observations for the proposed pipeline in different settings in the Experimental Framework section. We further discuss the limitations of the current study and future directions in the Discussion section. Finally, we summarize our work in the Conclusion section.
...
%\parencite{petersinvestigating}: This work is on the importance of social media images for disaster management.
...
%The authors address the question of whether images within social media messages are good indicators of relevant information for disaster management. % They analyze tweets and messages from Flicker and Instagram for the flood event in Saxony in 2013. Their findings suggest that the existence of images within on-topic messages are more relevant to the disaster event, and the image content can also provide important information related to the event.
...
%%\parencite{daly2016mining}: This work is on mining and classifying images on social media
...
%\parencite{chen2013understanding} this is on understanding images posted on Twitter.
...
%\textcolor{red}{Relevant techniques from computer vision, need to be revised}
...
%by using various deep learning architectures based on convolutional neural networks (CNN)~\cite{NIPS2012_4824,sermanet-iclr-14,SimonyanZ14a,SzegedyC:CVPR15,He_2016_CVPR}. %\textbf{TODO: Note here the most recent classification performance achieved on ImageNet.} %{\it Should we list the performance in Large Scale Visual Recognition Challenge 2016 (ILSVRC2016) or do we just mention ResNet,  VGG19, VGG16 ???}%These deep CNNs integrate low, medium and high level features~\cite{Zeiler:ECCV14} and classifiers in an end-to-end fashion to optimize on the target prediction task directly from raw data. For example, the output of lower layers of deep CNN architectures corresponds to low-level representations Scale Invariant Feature Transform (SIFT)~\cite{Lowe:sift} and Histogram of Oriented Gradients (HOG)~\cite{Dalal:2005gq}. Meanwhile, the one of higher layers keeps the abstract information and more domain specific~\cite{goodfellow} of images.%suitable for low-level vision tasks while the higher layers are more domain specific~\cite{goodfellow} and eliminate the need for pre-defined feature engineering like Scale Invariant Feature Transform (SIFT)~\cite{Lowe:sift} and Histogram of Oriented Gradients (HOG)~\cite{Dalal:2005gq}.%\textcolor{red}{Something about ftune here}%We apply transfer learning to train our models in the scenario follow: 1.{\it CNNs as fixed feature extractor:} extracting CNN codes for all images, train a linear classifier (e.g. Linear SVM or Softmax classifier) for our damaged object dataset; 2){\it Fine-tuning the CNN:} not only replace and retrain the classifier on top of the CNN on the new dataset, but to also fine-tune the weights of the pretrained network by continuing the backpropagation.%Prior to deep-CNN era, the most popular techniques were based on using handcrafted features such as SIFT and HOG together with classical machine learning classifiers such as support vector machines and random forests~\cite{CsurkaG:ECCVW04,LazebnikS:CVPR06} in a bag-of-visual-words fashion. %Prior to deep-CNN era, the most popular techniques were based on bag-of-visual-words (BoVW) models~\cite{CsurkaG:ECCVW04,LazebnikS:CVPR06} where handcrafted local image features such as SIFT were first extracted and vector quantized into a visual vocabulary by k-means clustering. And each image was then represented by a histogram of how often the extracted local features were assigned to each visual word (hence the name ``bag-of-visual-words''). A support vector machine classifier was eventually used on top of these histogram representations to perform the classification task. Later on, classification-by-detection type of approaches became popular. Among others, Deformable Part-based Models (DPM)~\cite{Felzenszwalb:2010ez} and its variations~\cite{voc-release5,FidlerS:CVPR13} were the state-of-the-art techniques before the revitalization of deep-CNN architectures in image classification and object detection domain.
...
%We downloaded images using URLs extracted from the collected Tweets for all four events.
...
%\noindent{{\bf Damage severity levels instructions:}}\\\subsubsection{Damage Severity Levels Instructions}\textit{The purpose of this task is to assess the severity of damage shown in an image. The severity of damage in an image is the extent of physical destruction shown in it. We are only interested in physical damages like broken bridges, collapsed or shattered buildings, destroyed or cracked roads, etc. An example of a non-physical damage is the signs of smoke due to fire on a building or bridge---in this particular task, we do not consider such damage types. So in such cases, please select the ``no damage'' category.}%\smallskip\noindent\textit{\textbf{1- Severe damage}: Images that show the substantial destruction of an infrastructure belong to the severe damage category. A non-livable or non-usable building, a non-crossable bridge, or a non-driveable road are all examples of severely damaged infrastructures.}%Specifically,%Building: If one or more building in the focus of the image shows substantial loss of amenity/roof.  If the image shows a building that is unsafe to use, it should be marked severely damaged.%Bridge: If a bridge is visibly not safe to use because parts of it are collapsing and should not be driven or walked upon, it should be listed as severe damage.%Road: If a road should not be used because there has been substantial damage, it should be marked as having severe damage.  %Examples: Due to an avalanche, there may be huge rocks piled up and you cannot drive or only a narrow part of the road is open.  Due to an earthquake, you see a sinkhole, substantial part of the road has sunk and the road cannot be navigated safely, that is severe damage.  %\smallskip\noindent\textit{\textbf{2- Mild damage}:
...
%\textcolor{red}{\bf TODO: Imran to fix the image with discussed idea}
}
\label{fig:pipeline}
...
%&\includegraphics[width=0.2\textwidth]{imgs/banner3.jpg}
\end{tabular}
...
%In order not to have a very restricted definition of relevancy that may lead to discarding potentially relevant and useful data, we have avoided constructing a set of rules or hand-designing a set of features by ourselves to decide whether an image is relevant or not. 
...
% \subsection{Relevant and Irrelevant Image Annotations}% \textcolor{red}{\bf TODO: Ferda}% Explain here the procedure to get irrelevant image using ImageNet model. Manual selection of irrelevant terms etc.% \begin{itemize}% \setlength\itemsep{1em}% \item Using the VGG-16 pretrained model to classify the whole dataset.% \item Pick up random images labeled by non-object such as: website, sign, suit, mobile phone, computer, notebook, envelope ...% \item Randomly pick  crisis related images labeled by "mild", "severe"% \item 7036 images of relevant and irrelevant images% \item fine-tune VGG16 to train a binary classification% \item Accuracy on test set: {\bf 0.98}% \end{itemize}\subsection{De-duplication Filtering}%\textcolor{red}{\bf TODO: Imran}\\
...
%.FO. This argument is not specific to de-duplication. It is valid for relevancy filtering, too. And I think we have already presented this motivation at the same level of detail in the introduction. Let us not repeat it for the sake of word count limitation.%More importantly, human-annotation task depends on an available budget, which could be \emph{monetary} in the case of paid-crowdsourcing or \emph{time} in the case of volunteers. Even performed on the voluntary basis, volunteers' time is precious, it is neither unlimited nor free of cost. Presenting the same, or almost the same, images to workers for annotation may introduce a drop-out in the worker's pool and may result in a waste of budget by labeling duplicate images multiple times unnecessarily. Given a certain budget, ideally, the human annotation should be performed on distinct items which are relevant to the event or the task in-hand. %so to properly utilize the budget. in this experiment, we show the significance of removing duplicate images to save budget and the effects of duplicates in classification accuracy. 
...
%For the performance evaluation of the different system components, we report on several well-known metrics such as accuracy, precision, recall, F1-score\footnote{\url{https://en.wikipedia.org/wiki/Precision_and_recall}} and area under the precision-recall curve (AUC)\footnote{\url{https://en.wikipedia.org/wiki/Receiver_operating_characteristic}}. These metrics are commonly used in machine learning and data mining literature.% and defined as follows.\iffalse
...
%In our case, precision describes what percentage of the time the model predicted a certain damage category, say severe damage, correctly. Note that precision is also known as positive predictive value.
...
%In our case, recall refers to the percentage of the correctly predicted instances of a certain damage category to the total number of instances of that category in the dataset. Note that recall is also known as true positive rate or sensitivity.
...
%Therefore, it is more useful to combine them into a single measure such as F1 score.
...
%, which deems the task easy for the binary classifier. 
...
%\textcolor{red}{The last column (i.e., overall reduction) represents the percentage of duplicate and irrelevant images our system filters out. This implies overall we are saving 62\% of our resources.}
}
\centering
\begin{tabular}{rrrrr}
\toprule
...
%In other words, all noisy data will be used as is. 
...
%That is, this setting will have only relevant images. However, duplicate images may still exist in the dataset. 
...
%This is exactly what we test in our first setting (S1). 
...
%We use various performance metrics such as precision, recall, F1-score, and AUC.%Even though the data is randomly picked, we maintain the class distribution proportions as in the original dataset. Table~\ref{tbl:im2train} shows the class-wise data distribution and Table~\ref{tbl:classification_report} presents the classifier performance in various settings.
...
%The saved budget could be used to get more images labeled. 
