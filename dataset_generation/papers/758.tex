...
%\begin{flushright}
% \textcolor{red}{Published in Proc. of IEEE 9th International Conference on Biometrics: Theory, Applications and Systems (BTAS), (Los Angeles, CA), October 2018.}
%\end{flushright}
...
%acquire pseudonymize (or fully anonymize) personal data of individuals with highest privacy standards by default, and the regulation prohibits any processing of personal data without explicit user consent. 
...
%The notion of soft-biometric privacy was first raised by Mirjalili and Ross~\cite{mirjalili_soft_2017}, who developed an approach to prevent the automated extraction of soft-biometric attributes while still maintaining the utility of the data for automated biometric recognition; therefore, biometric utility and privacy of users are protected at the same time. 


\section{Related Work}


%Machine learning~\cite{raschka_python_2017}
...
%Defenses against adversarial attacks~\cite{kannan_adversarial_2018,rozsa_towards_2018}
...
%Conditional GAN~\cite{antipov_boosting_2017}

The major contributions of this work are as follows:%\vspace{-1em}
\begin{itemize}[noitemsep,nolistsep]
%\item Addressing the issue of generalizability of gender privacy with respect to multiple arbitrary gender classifiers. %\vspace{-1em}
\item Designing an ensemble of SANs to address the problem of generalizability across unseen gender classifiers.
%\hl{to me, "exhaustive" really means "all gender classifiers that are out there"})
...
%\hl{this is more of a proof of concept paper, but it would still be good to mention how/if such an ensemble can be used in practice}
\end{itemize}
%\vspace{-1em}
...
%Diversity in an ensemble of classifiers has been widely studied~\cite{strauss_ensemble_2017, hansen_neural_1990, kuncheva_combining_2004}. 
...
%Here, we use similar cost functions and architectures for the autoencoder, the auxiliary gender classifiers, and the auxiliary face matcher. 
...
%In addition, we compute the average shape of faces for each of the eight groups and aligned the faces onto the average shape using affine transformation. The affine transformed prototypes are shown in Fig.~\ref{fig:gender-prototypes}.
...
%(\hl{SR: Since we don't have so many points, I think it looks better if we omit the bulleted list}):
...
%(\hl{AR: Not clear. Please use figures and tables to explain this important part of the paper. }) 
...
\begin{tabular}{ l c c c}
 %\multicolumn{4}{c}{\bf Ensemble1: Regular} 
 \toprule
...
%\small
%\item \noindent Notation: A: for training auxiliary gender classifiers, B: for SAN training, C: for SAN evaluation, D: for training unseen gender classifiers used for evaluating SAN models. 
%(\hl{Images? or Subject? How many images? How many subjects?})
%\end{tablenotes}
\end{threeparttable}
\end{center}
\end{table}
...
%{\tiny
%\begin{equation}
%Entropy = \frac{2}{N(t-1)} \sum_{i=1}^N \min \left\{\left(\sum_{j=1}^t y_{i,j}\right), \left(t-\sum_{j=1}^t y_{i,j}\right)\right\}
%\end{equation}}
%(\hl{AR: What is this??})

%Furthermore, we hypothesized that the best-performing auxiliary gender classifiers would be the hardest to confound during the SAN training. Consequently, a SAN trained with the strongest auxiliary gender classifier would likely result in face image perturbations that generalize well to unseen gender classifiers (\hl{AR: We must be careful here. The whole notion of diversity is contrary to this!! }). 
...
\end{center}
  ...
  %(\hl{fix ref number in image once finalized}) 
  ...
  %(\hl{include more images and make the figure larger})}
...
\end{center}
   ...
   %(\hl{need to check in the end that Ref[8] and Ref[28] is still the correct ref})}
...
\end{center}
   ...
   %\hl{adding ICB2018-SAN results for comparison and figure legends is in progress -- Replace png with eps file}}
...
%As a result, each SAN model may separately confound gender in a diverse set of samples, increasing the chance that at least one of the SAN models is able to confound an arbitrary gender classifier. The ROC curves for the mixing approach~\cite{othman_privacy_2014} are consistently degraded across all nine unseen gender classifiers. 



%We noted that G-COTS classifier performs poorly on the original unmodified test images of the MORPH-test set. Looking at the confusion matrices of G-COTS on the different dataset that contain images of people with black and white skin color (Figure~\ref{fig:gcots-confusion-matrices}), we found that G-COTS performance has an inexplicably high error rate on the black subpopulation of MORPH-test (near-random predictions for both male and female images). However, the confusion matrices of the MUCT and CelebA test sets indicate such a racial bias (random male/female predictions for the black subpopulation) does not exist for the other image databases.

%\begin{figure}
%\begin{center}
% \includegraphics[width=0.97\linewidth]{figs/confusing-matrices.pdf}
%\end{center}
%\caption{Confusion matrices showing the correct and incorrect predictions of G-COTS on the MORPH, CelebA, and MUCT test partitions separated by race (black and white). \hl{(AR: What is the purpose of this matrix?})}
%\label{fig:gcots-confusion-matrices}
%\end{figure}
...
% \begin{center}
% %\fbox{\rule{0pt}{2in} \rule{0.9\linewidth}{0pt}}
%    \includegraphics[width=0.97\linewidth]{figs/roc-cnntrained-gpred.png}
% \end{center}
%    \caption{.}
% \label{fig:roc-cnntrained-gpreds}
% %\label{fig:onecol}
% \end{figure*}
...
%Comparing the performance of selecting the most-gender-perturbed sample for a particular unseen gender classifier with random selection shows that the best-perturbed sample will flip the predicted gender label which is not useful since one can flip the predicted label to get the true-label of the original sample with high confidence. On the other hand, randomly
