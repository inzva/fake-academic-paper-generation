...
\authorrunning{Clément Pinard \etal}
...
%%critic 
%criticism
%for quality measurement of "depth prediction from a single image", 
%AM: je comprends pas bien l'expression 'from a single image "validation"'
%which is not suited for practical use cases, because depth is only know up to a scale factor that is not determined. This limitation makes questionable the quality
%by questioning the usability
...
actually
learns 
%structure from motion, from motion instead of only structure. 
...
%a much more robust approach when considering possible 
...
%Our motivation relies on the fact that UAV navigation is a very specific use case compared to other ego motion videos. The two main characteristics of a UAV footage are the availability of orientation and the presence of heterogeneous structures.
...
%Although working with stabilized videos, our algorithm can be extended to unstabilized videos, 
...
%LC : il faut que tu ajoutes une remarque sur les autres papiers et l'absnece de facteur d'échelle dans l'intro, pour que ce soit bien clair que ce que tu fais est mieux. explain "the scale factor problem"
...
%overcome this assumptions by trying to comput 
do without this assumption by computing
a residual optical flow 
%that will then solve 
to resolve
...
%An important thing to notice from 
...
%characteristic geometric context
...
%computing the scale factor with 
...
%lacks robustness on an undefined dataset 
...
%AM: l'expression "the same for translation" est ambiguë ; est-ce qu'on pourrait écrire
%AM: "the scale factor for the depth map is theoretically the same as for translation" ??
...
%AM: ça me paraît un peu compliqué dit comme ça ; on peut avoir l'impression au début 
%AM: que tu inventes une nouvelle métrique qui t'arrange, alors que c'est au contraire
%AM: une métrique plus juste. D'autre part je trouve que les notations "D" et "P" ne 
%AM: sont pas très parlantes. Est-ce que ce ne serait pas plus clair de préciser si 
%AM: le facteur d'échelle est fourni par la vérité terrain "GT" ou par la vitesse "S" ??
...
%because otherwise we won't have an 
in order to guarantee the
equivariance between depth and motion, 
%just like in original DepthNet training schedule.
...
%we sample $I_i$ pixels bilinearily.
...
%and $C_1 = 1\mathrm{e}{-2}$ and $C_2 = 9\mathrm{e}{-2}$ are constant. To compute them, we convolute 
the image with Gaussian kernels of size $3 \times 3$.
$C_1 = 0.01$ and $C_2 = 0.09$ are two constants.
%AM: l'estimation de la "moyenne" en convoluant avec une gaussienne est à peu près 
%AM: évidente, mais pour la (co)variance ça l'est moins... est-ce que tu calcules directement
%AM: la moyenne dans le vois 3x3 des carrés (et produits) pondérés par la gaussienne ?
%One can thus also note that since SSIM is working with convolution, the receptive field of our loss is augmented.
...
\begin{equation}
\label{SSIM}
%\textsc{SSIM}(I_t, I_i) = \frac{(2\mu_{I_t}\mu_{I_i} + C_1) + (2 \sigma _{xy} + C_2)} 
...
\end{equation}
%AM: d'ailleurs il faudrait aussi justifier un peu le fait de mixer les deux 
%AM: fonctions de loss photométriques, pourquoi pas SSIM seule ?
...
%Laure : pourquoi laplacian depth n'est pas en norme L1 ?
...
% \subsection{Flexible shifts training}
% \label{sec:flexible}
% A common problem that to our knowledge has not been much discussed in precedent attempts to learning depth from motion, is how to choose frames to build the training sequences. In fact, it is always assumed that image sequences contain movement, static frames being detected and discarded at the pre-processing level.
% One can compute optical flow on consecutive frames, and based on threshold value, pairs with too small
% optical flow will be discarded. Although apparently sufficient for sanitizing KITTI dataset, the threshold value for optical flow seems too arbitrary and not easily linked with optimal displacement.

% Instead, we designed a training step where we recompute frame sequences on the fly. While keeping the same middle frame, prior and posterior shifts are adjusted so that DepthNet output remains
% in the same typical range. Since DepthNet outputs depth maps assuming a constant displacement, too high values can be lowered when taking frames further apart.
% More generally, this can be summed up by equation \ref{eq:flexible} considering a particular sequence of $N$ frames within a longer video, whose middle frame index is noted $i_t$. For example, in a sequence of $5$ frames, if  we first have consecutive frames, then we have $(s_i)_{0 \leq i < N} = [-2,-1,0,1,2]$ and $i_t$ is the index of the $3$rd frame relative to the whole video.

% \begin{equation}
% \label{eq:flexible}
% \forall i \in \left\llbracket0,N\right\llbracket\backslash\left\lbrace \left\lfloor\frac{N}{2}\right\rfloor \right\rbrace, s_i^{\textsc{adjust}} = s_i \times \frac{\left\Vert\zeta\left(I^\textsc{stab}_{i_t+s_i}, I_{i_t}\right)\right\Vert_1}{\zeta_0} \times \frac{\left|i - i_t\right|}{N}
% \end{equation}

% $\zeta$ is DepthNet output, $s_i$ is the time shift value for the $i$th frame in the current sequence, $\zeta_0$ is a mean target depth value, supposed to be optimal for DepthNet. Going back to our example, pairs constructed with middle frame should have DepthNet outputs of mean values $[\zeta_0, 2\zeta_0, \infty, 2\zeta_0, \zeta_0]$ and the shift values $s_i$ will be updated accordingly, assuming displacement magnitude is linear to temporal shift, which can be approximated when considering short time lengths and low accelerations.

% One should note that this operation is not idempotent. Indeed, there is no guarantee that the new pairs will make DepthNet output optimal values since input pair has changed. To overcome potential shift divergence problems, such as when there is no displacement leading to an infinite output for DepthNet and thus an infinite value for $s_i$, a max shift value $S$ can be used, along with a strict monotony, knowing the middle shift $s_{\left\lfloor\frac{N}{2}\right\rfloor}$ will always remain $0$: 
% \begin{equation}\left\lbrace
% \begin{array}{l}
% s_{\left\lfloor\frac{N}{2}\right\rfloor} = 0\\
% \forall i \in \left\llbracket0,N-1\right\llbracket, s_{i+1} > s_i\\
% \forall i \in \left\llbracket0,N\right\llbracket, |s_i| < S
% \end{array}\right.
% \end{equation}

% We can specify an optimal depth value $\zeta_0$ according to a nominal displacement $D_0$ and let the whole training algorithm take care of the rest, regardless of potential static frames in the training video. Empirically, Pinard \etal \cite{pinard:hal-01587658} considered $50$ m as a good value when considering nominal displacement of $30$ cm. This is very convenient for UAV videos where high altitude can lead to very small optical flow, and in the case of hovering states, where the camera is not moving.
...
%%Some of the flights have low displacement sequences and even hovering states where the drone is completely motionless. This is a specific usecase where consecutive frames are not optimal, for training and need filtering, whether manually or automatically as discussed in part \ref{sec:flexible}.
...
% ce n'est pas clair que c'est le cas pour l'autre papier. Au passage, est ce que DepthNet est meilleur si on applique le ratio des médianes? est ce que tu aurais le temps de tester?
%LC: peut être changer l'ordre en mettant la 2eme phrase (However, when scale factor...) à la fin du paragraphe ?
