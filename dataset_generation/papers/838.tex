...
%A cascaded classifier, though able to balance accuracy and speed, typically requires the inputs of all stages to be the same, thus difficult to handle smaller objects. 
In this paper, we propose a novel classification architecture ProNet based on convolutional neural networks. 
%It learns to propose promising image boxes that might contain objects, and ``zooms'' onto those boxes. 
...
%(1) Based on my knowledge of recent papers, there are quite a few CNN cascades for pose/face, but they require very well localized key points or bboxes to proceed. Ours is based on pretty loose image-level annotations (especially true for COCO).
%(2) Because that bboxes are unavailable for training, and that we want to "zoom-in" to interesting regions. It is impossible to pull hard negatives while maintaining all positives, as we don't know what is actually positive and what is background. Algorithm 1 offers a solution to train cascade without knowing bboxes, basically we want to use a "less aggressive" strategy to avoid overfitting too easily.
%(3) Another perspective to look at the cascade is that we generate "region" proposals directly from the FCN feature maps, which is different from the selective search method used in fast RCNN. We show that for classification, our object-specific region proposal leads to improvement even with <10 proposals per image.

% Related:
% http://arxiv.org/abs/1510.04445


%%%%%%%%% BODY TEXT
\section{Introduction}
% How to organize this paper?
% 1. Introduce the problem of object classification, global methods might face difficulty, and does not support localization
% 2. To search over locations and scales, FCN is an economical approach. But not clear how to train it. We use LSE-pooling over multiple scales.
% 3. Motivated by cascaded classifiers, we want to extend the framework to multiple layers of CNNs, and gradually zooming onto promising boxes. However, we need to be careful not to overfit on easy examples when train the cascade.
% 4. We further investigate the benefit of using different experts for different categories.
% 5. Describe our main contribution.
...
%To allow object localization, we treat boxes as basic units for classification. A box is a rectangular image region defined by its upper left and bottom right corners. We choose to represent the boxes with CNNs for their state-of-the-art performance on various visual recognition tasks. In particular, we use the fully-convolutional network (FCN) architecture~\cite{DBLP:journals/corr/LongSD14,Oquab_2015_CVPR,pinheiro:2015a,DBLP:journals/corr/SermanetEZMFL13}, which replaces the fully-connected layers of a standard CNN (e.g. AlexNet~\cite{NIPS2012_4824}) with convolutional layers. An FCN takes images with arbitrary sizes, and generates classification scores for different boxes in a sliding window fashion. Score computation can be done efficiently (\eg avoid re-computation on overlapping regions) and parallelized by a modern GPU. The sampling strides and box sizes are fixed and controlled by FCN's network architecture.




%Training FCNs for objects of different scales with image-level labels is nontrivial. One solution proposed by Oquab et al.~\cite{Oquab_2015_CVPR} is to randomly rescale the input images, and generates image-level scores by \textit{max-pooling} score maps of the selected scales. Image labels can then be used directly to compute losses for back-propagation. This algorithm forces the network to learn scale-invariance, as each randomly scaled image is equally important. To address this issue, we use a multi-stream multi-scale architecture. Each stream generates score maps for a certain scale. We then apply pooling techniques to generate image-level classification scores, but by searching over locations and scales.
...
%\item We show that FCNs are able to propose object-specific boxes when trained with image labels, and pooling over multiple scales helps improve classification and localization performance.
...
% Object classification with weak supervision
% Object detection
% Cascade
% FCN
...
%ProNet has two basic components: an object-specific box proposal unit, and a verification unit. The box proposal unit searches over image boxes at different locations and scales, and assigns confidence scores. The verification unit then verifies the promising boxes and updates their confidence scores. We implement these two units with convolutional neural networks. In the remaining part of this section, we will demonstrate how to use CNNs to generate object-specific box proposals with only image labels, and the sampling strategies during training and testing of ProNet. It is worth noting that although we demonstrate ProNet with particular CNN architectures, the framework itself is general.
...
%a possible solution is to \textit{average pooling} over the score maps to generate image-level scores. 

% In our case, the bounding box annotations are not available during training. One na{\"i}ve way to address this problem is to simply assume all locations share the same labels. This can be implemented by doing an  over feature maps across different locations and scales, given by
%\begin{equation}
%s_{c} = \frac{1}{M}\sum_{x,y,k}s_{c, x, y, k}
%\end{equation}
%where $c$ is the category, $k$ corresponds to the $k$-th stream of FCN, $x,y$ correspond to location in the feature map, and $M$ is the total number of such elements.

%The purpose of average pooling is to 

%When objects are small, average pooling might not work well as it forces the network to learn from background. Instead, 
...
% loss function
% Another way to achieve scale invariance, cvpr baseline paper
...
%convolutional layers can have different padding strategies, computing the exact receptive field for a neuron given an image is not straight-forward. First, we assume that the width and height of the receptive field to be the same as the non-fully convolutional version of the same CNN architecture (\ie classifiers are implemented by fully-connected layers rather than convolutional layers). Next, 
...
%\rowcolor{gray!30}Fast RCNN~\cite{DBLP:journals/corr/Girshick15} & TODO & TODO\\
%\hline
...
%proposal number evaluated by classification and localization mAP on VOC 2012 validation set. We set the filter threshold to 0.1 and select at most $k$ proposals for each class. The average number of proposals for each image is reported in the second column.
...
%The CNN column specifies the CNN models, \textbf{A} refers to AlexNet and its variants pre-trained on standard 1,000 ImageNet categories. \textbf{A+} refers to AlexNet pre-trained on extended ImageNet categories, which has been shown to outperform \textbf{A}. \textbf{V} refers to the VGG-16 pre-trained on standard 1,000 ImageNet categories. \cite{Simonyan14c} extracts VGG-16 features over multiple scales and aggregates them into image-level representation, it cannot be used for localization directly.
\label{voc12_cls}
\end{table*}
...
% Note that every proposed box only go through a single root-to-leaf path in the tree
...
%Table~\ref{voc12_cls} and Table~\ref{voc12_loc}. Mention VGG-16 numbers here.
%Oquab is very slow, they use better network and more scales
%\textbf{Comparison with state-of-the-art on COCO.} Table~\ref{tab:coco_result}.
...
%We list the performance of previous systems trained using bounding box annotations in Table~\ref{voc12_cls} and Table~\ref{voc12_loc}. It can be seen that the best systems still outperform our proposed system trained with image-level annotations on both classification and point-based localization tasks.
