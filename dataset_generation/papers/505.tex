...
%\category{I.5}{Computing Methodologies}{Pattern Recognition}
...
\begin{figure}[!htb]
\centering
% \epsfig{figure=motivation.pdf,width=3.1in}
...
% some problems for the model structure, we have two full connection layers
...
%To summarize, the novelties of this paper include:(1) A deep model with structural alternatives for activity recognition; (2)An  EM-type optimization algorithm for deep learning; (3) Transfer learning to borrow strength from 2D data to 3D data.

\begin{figure*}[!ht]
\centering
% \epsfig{figure=Architecture.pdf,width=7.0in}
...
\begin{figure}[!ht]
\centering
% \epsfig{figure=3d_conv.pdf,width=3.0in}
...
\begin{figure}[!htb]
\centering
% \epsfig{figure=inference.pdf,width=\columnwidth}
...
\begin{figure}[!htb]
\centering
% \epsfig{figure=learning.pdf,width=\columnwidth}
...
%
%Algorithms( table )
%Step 1: We divide the frames of input video $v$ into $N$ parts with equal length.
%Step 2: For each parts, we exploit the above segment of architecture to learn spatio-temporal features.
%Step 3: We combine outputs to do fine-tuning to refine the whole architecture.
%Step 4: After fine-tuning the architecture, we exhaust all possible partitions to find the one with the maximum score for input video $v$.
%Step 5: If satisfy stop condition, terminate, otherwise return to Step 1.
...
\begin{figure*}[!htb]
\centering
% \epsfig{figure=dataset.pdf,width=\textwidth}
...
\begin{figure}[!htb]
\centering
% \epsfig{figure=accuracyBar.pdf,width=2.6in}
...
%Specially, since skeleton points in CAD-120 dataset is reliable, we exploit them to crop subjects roughly and then remove backgrounds slightly. For fair compromise, we also apply this technique on the competing methods.
...
\begin{figure}[!htb]
\centering
% \epsfig{figure=pretrain.pdf,width=2.9in}
...
\begin{figure}[!htb]
\centering
%\epsfig{figure=latentStructure, width=2.8in}
