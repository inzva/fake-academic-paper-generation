...
% obtained through the IMU/GPS readings. 
...
% \subsection{Feature for reconstruction} \label{sec:expRes_feat}

% In Sec.\ref{sec:method_featSuper}, we propose to have an additional supervision signal from feature reconstruction for overcoming Lambertian surface assumption and texture-less region issue. The selection of dense feature is introduced in this section. We explore different features for feature reconstruction loss.

% \paragraph{NYUv2 Feature}
% Conventionally, people rely on handcrafted visual features \cite{lowe2004sift}\cite{dalal2005hog} to establish correspondence between images. Those features cause ambiguous matches at texture-less regions. In \cite{weerasekera2017feature}, a dense visual descriptor is learned for the matching task, which is robust to ambiguous regions. More important, the descriptor is trained for general matching purpose which does not biased on dataset heavily. Therefore, no finetuning is required on the descriptors. \cite{weerasekera2017feature} trains the descriptor in NYUv2 dataset, which can be used offshore. The features extracted from \cite{weerasekera2017feature} are 32-channel dense features.


% \paragraph{Depth Feature}
% We also explore the use of deep features extracted from the depth ConvNet itself. Note that the features in the decoder of our depth ConvNet is 1-channel, which is basically a scaled version of inverse depth, which is not desirable for the purpose. Therefore, we replaced our decoder to some learnable up-convolution blocks for this experiment. At the end of the decoder, it outputs a 32-channel feature map. Therefore, a 1x1 convolution layer is used to convert the feature map into inverse depth prediction. The 32-channel feature map is used for feature reconstruction loss.
...
% We have shown that the use of binocular stereo sequences for learning the two tasks jointly predicts odometry in metric scale using 2 frames only. % in a monocular training framework.
...
% FIXME/UPDATEME list
% \cite{weerasekera2017feature}: update to correct citation
% missing results in the table
% voTraj example
% depthEg: results from other existing methods