...
%Generating a dictionary with many samples per class (SPC), would cause a enormously heavy dictionary. With popular alternating direction method ofg multipliers (ADMM) \cite{boyd2011distributed}, solving a Lasso problem with a dictionary $A$ with feature dimension $d$, class number $n$, and SPC $s$, the time complexity we is $\mathcal{O}(\max( d^2 sn, (sn)^{3}))$, as detailed in the following sections. Thus, higher SPC with larger class number would cause inefficiency in solving the lasso problem. To resolve the issue, we propose to cacluate a reweighted matrix $M \in \mathbb{R}^{(n \times s) \times n}$ that maps SPC from $s$ to $1$. The reweighted matrix has third benefits. First, the complexity of solving the lasso problem in the ADMM framework could be reduced to $\mathcal{O}(\max(d^2sn)$. Although in the worst case, the complexity is the same as the vanilla Lasso, the updating of $M$ could be done parallelly, since the weights are indepentdent for each class. Therefore, the computation of the large dictionary learning is efficient. Second, we could know which domain the testing data possible lie in since a testing sample is more close to an indicated domain with larger weights in $M$. Third, since the dictionary relies on the training sample from the indicated domain more, the classfication could be more accurate.
...
%The vanilla Lasso with $M$ is formulated as follows.
% The lasso problem with masking is fomulated as follows. We formulate this constraint into a projection onto the surface of $L_1$ ball with projection matrix $P_k^{L_1}$ w.r.t. class k. Thus, The overall projection could be written as $[P_k^{L_1}M_k], \forall k \in [1,n]$, or we can just write in matrix form $P^{L_1}M$.
%\begin{equation}
%\label{v_lasso_AMM}
%\begin{split}
%\min_x \quad(1/2)\|A_MMx-b\|^2_2+\lambda\|x\|_1.
...
%{\bf Complexity Analysis}: For the vanilla Lasso problem under the ADMM framework with a dictionary $A \in \mathbb{R}^{d \times (s \times n)}$ with SPC = s, the time complexity bottleneck is at the step of updating x, needing $\mathcal{O}(\max( d^2 sn, (sn)^{3}))$ from  matrix multiplication and inverseion. For the MDDL with the weighting matrix $M$, the time complexity bottleneck is at the step of updating the $M_k$, needing $\mathcal{O}(\max(d^2s)$ for each class, and thus $\mathcal{O}(\max(d^2sn)$ overall. Compared with the vanilla Lasso problem, the proposed MDDL is more efficient when $(sn)^2>d^2$, as is a common case for multi-class and higher SPC classification. Even if $(sn)^2<d^2$, the complexity for the GAN Dictionary Learning is the same as the vanilla Lasso; however, the updating of $M_k$ for each class is independent, and could be done in parallel processing, so the computation time is less. Overall, our propsed GAN Dictionary Learning is more efficient than the original problem.
...
%    \STATE {\bf Initialization}: $x,z,y=0$.
...
%       \STATE Update $P^{L_1}_kM_k$ by Eq.(\ref{Lag_v_lasso_AMM_M_ans}) and Eq.(\ref{Lag_v_lasso_AMM_P_ans}).
