...
%%%%%%%%% TITLE
%\title{``Alignment Free'' End-to-end Face Recognition}
...
% Also, in many ensemble methods~\cite{sun2014deep,sun2013hybrid}, face patches are heuristically chosen according to landmarks, which may possibly be sub-optimal. 
...
%[possible figure choices: (1) the learnt transformation is robust to scale/position/in-plane rotation variations (of detection results) (2) show intermediate images and system architecture overview.]
%-------------------------------------------------------------------------
...
%-------------------------------------------------------------------------
%\subsection{Attentional Model? Face landmark detection/alignment studies? pose-invariant/partial face recognition??}
...
% Two stream hypothesis regarding human visual perception. ~dorsal pathway handles "where the object is", ~ventral pathway handles "what the object is". Unclear if this applies to face perception..... There has been a hypothesis that face detection and identification might have separated dedicated brain resources/mechanisms. In short, detection in the peripheral retina, whereas idendification in the fovea. [?]
...
% We limit ourself to gray-scale images only in this work. 
...
According to the results, we chose the network consisting of 3 convolutional layers and 1 fully connected layers, each with a PReLU layer inserted afterwards, in our implementation.
	...
	%\includegraphics[width=3in]{st.png}
...
During the backward propagation, we need to calculate the gradient of $V_i$ with respect to each of the eight transformation parameters. The function shown in (\ref{eq_sample}) may not be differentiable when $w=x_i^s$ or $y=y_i^s$. However, this seldom happens since the chance that the calculated $x_i^s$ or $y_i^s$ are integers is very low in practice. We empirically set the gradient at these points to be $0$ considering that their effect on the back propagation process are negligible. For differentiable points, the chain rule can be applied to get the gradient. An example regarding $G$ is shown in (\ref{eq_devG}), in which the gradient with respect to the source coordinates are defined in (\ref{eq_devsrc}) and (\ref{eq_devsgn}). 
	...
	%\subsection{(Cascading and Assembling Transformers?) no result...consider deleting this part? (feature values extracted from patches other than the main one shrink to 0 after training.)}
	%Previous works on CNN-based face recognition demonstrate that it is often helpful to ensemble deep models trained on different patches of face areas, e.g. in DeepId??, the author trained simultaneously 200 networks on ??? different face patches and apply PCA on the concatenated features. We borrow the idea of model ensemble and try to incorporate multiple pathways of transformed images in our network. 
	
	%\subsection{The Necessity of Transformation}
	%Although FaceNet demonstrates excellent performance... Experiments on smaller datasets show that it is often necessary to do face alignment before recognition. Here, we train the same recognition network alone without the spatial transformer module on the same data, and report the result as a comparison.
