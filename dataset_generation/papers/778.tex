...
% colour coding tables https://texblog.org/2013/06/13/latex-heatmap-using-tabular/
\usepackage{tikz}
\usepackage{collcell}
...
%\begin{align}
%\label{eq:eacc-def}
%\text{E.Acc}(\tilde{A})=\frac{1}{|A\setminus \tilde{A}|}\underset{a\in A\setminus \tilde{A}}{\sum}\text{E-split accuracy on attribute }a
%\end{align}
%where $A$ is the set of all attributes ($|A|=12$ in our case). 
...
%BatchNorm && \xmark & \xmark && \xmark & \xmark & \xmark & \xmark && \cmark & \cmark & \cmark & \cmark && \cmark & \cmark & \cmark & \cmark & \cmark && \cmark & \cmark & \cmark & \cmark \tabularnewline
\cline{1-1} \cline{3-4} \cline{6-9} \cline{11-14} \cline{16-20} \cline{22-25}
\end{tabular}
