...
%\setlength{\dblfloatsep}{9pt plus 2.0pt minus 2.0pt}
%\setlength{\textfloatsep}{16.9pt plus 1.0pt minus 2.0pt}
%\setlength{\dbltextfloatsep}{16.9pt plus 1.0pt minus 2.0pt}
...
%\author[1, 2]{\small Tewodros Amberbir Habtegebrial}
%\author[2]{\small Kiran Varanasi}
%\author[2]{\small Christian Bailer}
%\author[1, 2]{\small Didier Stricker}
%\affil[1]{\footnotesize TU Kaiserslautern}
%\affil[2]{\footnotesize German Research Center for Artificial Intelligence}
%\institute{}
...
\newline
%Depth prediction approaches are complementary to novel view synthesis approaches as they can provide scene geometry. The depth prediction method used in this paper is inspired by the idea of marrying the stereo-matching network of Kendall at. al.~\cite{gcnet} and unsupervised losses of the monocular depth predictors of Godard at. al.~\cite{godard}, Zhou et. al.~\cite{zhou2017}.

%Even though DeepStereo produces good quality novel views it takes minutes to render a 500x500 pixel frame. In this paper we propose another novel views synthesis approach which is more accurate and faster than DeepStereo. We aim to achieve these goals at the expense of requiring a stereo camera. %TODO convince the reader using stereo camera is not a big deal these days, smart-phones have stereo camera.
%In order to achieve higher rendering speed we adopt use stereo cameras.
...
\newline
%We first train the depth prediction network using a collection of stereo-pairs. Then, the input views are forward-mapped(to the novel view-point) using their respective depth maps estimated by the trained depth predictor. Finally, the texture inpainting network is trained to predict the target views from forward-mapped input views.
...
% The feature extraction network used in our work network has a stack of 2d convolutional and residual layers. 
\begin{table}[htb!]
\centering
...
%Similar to GCNet~\cite{gcnet}, we use 3D convolutional encode-decoder network as feature volume filtering.
%\(\left[D, \lfloor H/2 \rfloor, \lfloor W/2 \rfloor \right]\) dimensional 
%The encoder has 8 3D-convolutional layers while the decoder stage has 5 transposed 3D-convolutional layers. The middle 3 layers of the decoder stage have skip connections to features in the encoder stage. At layers that have skip connections, the input feature maps received via skip connections and the input from a layer below are concatenated across the feature dimension.
...
%In our experiments we have tested three different spacings, $0.8, 1.6, 2.4$ meters. 
\begin{table}[htb!]
\centering
\begin{tabular}{|l|l|l|l|l|}
\hline
...
%Further, we have tested on how using residual blocks gives better results compared to regular convolutional layers.
% We hypothesize this is due to the easy flow of activations and gradients as a result of the reduced of impedance of residual blocks.
...
%Furthermore, our current outputs tend to have a small amount of blurriness. We would like to mitigate this problem by: forward mapping super-pixels(instead of single pixels) and/or reformulating the texture inpainting objective via Conditional Generative Adversarial Networks. We would also like to extend the work to a more challenging scenarios which contain 3D object rotations and strong lighting changes.

%Last but not least, instead of forward-mapping (and inpainting) input images, we could forward-map and in-paint the input depth maps. This removes the need to hallucinate color, therefore, it leads to an efficient use of the lighting information present in the input images. 

%with super-pixel forward-mapping. Super-pixel forward-mapping will result in a better utilization of the input images, by projecting larger number of input-pixels to the warped-views. Moreover, 
%%We believe there are better realizations of this paradigm that we haven't had the chance to explore in this work. 


%Evaluations on the KITTI dataset show that the proposed stereo-based approach generates realistic novel views. The method is not s single algorithm it is more of an IBR paradigm that uses neural networks for depth prediction and inpainting. 
