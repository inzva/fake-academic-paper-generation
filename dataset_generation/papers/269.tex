...
%        1         2         3         4         5         6         7         8
%
% Slightly modified by M. Daoudi for FG 2019
%
...
%An appealing representation of emotions is the use of emotional
%attributes such as arousal (passive versus active), valence
...
% vale eikones apo tin affwild edw pou na perigrafoun ta action units
\begin{comment}
...
%Because the Aff-Wild database contains many different expression and emotional states, finding videos in which the subjects display the limited seven basic emotions was not an easy job. 
...
values in the annotated part of Aff-Wild.
% Finally, Figure \ref{fig:basi_expr_dist_va} presents graphs of the distribution of seven basic expressions in the VA-Space.
...
%%%% TA AUs eixan mean accuracy; ta 7 basic eixan total accuracy ; vale kai to mean accuracy ws metric i vale to mean accuracy anti gia to total
...
%We trained the above described GANs in total in three scenarios: i) the discriminator is also a classifier for only AUs , ii) the discriminator is also a regressor for only VA and  iii) as said in Section \ref{gans}, the discriminator is a classifier of AUs and also a regressor for VA. In the followings, we compare the performance and generation capabilities of the developed GANs between these three scenarios.
