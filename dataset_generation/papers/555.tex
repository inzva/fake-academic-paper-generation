...
%		Graph-based Isometry Invariant Representation Learnings}
	%
	%
	...
	\begin{figure}
		...
		\begin{tabular}{cc}
			%	\vspace{-0.3cm}
			%	\raisebox{0.05cm}{\rotatebox{90}{Training}} &
			%	\includegraphics[width=0.8\linewidth]{images/RotatedFM/short_train_examples.png} \\
			%	\vspace{-0.2cm}
...
%		\item We design a new graph-based deep learning framework that learns isometric invariant features;
%		\item We propose a new statistical layer that leads to effective transformation-invariant classification of images described by graph-based features;
%		\item \renata{We define the quasi-equivariance property  of the polynomial filters and theoretically show that the representation that we learn is quasi-equivariant to any isometric transformation of the input signal;}
%		\item Through experiments, we show that our method is able to correctly classify rotated or translated images even if such deformations are not present in the training data.
%	\end{itemize}
	%
	%The remainder of the paper is organized as follows. In Section~\ref{s:related_work} we describe the related work. Section~\ref{s:gsp} reviews elements of graph signal processing, which are later used to design graph filters. Our new graph-based architecture is presented in details in Section~\ref{s:overview}. \renata{Then, we show the invariance of the graph-based features to isometric transformations in Section~\ref{s:equivaricance}}. Finally, our experiments and their analysis are presented in Section~\ref{s:exp} and we conclude in Section~\ref{s:conclusion}.
	...
	%The authors in~\cite{bb:fasel2006rotation} construct deep neural networks that operate in parallel on the original and transformed images simultaneously with weight-shared convolutional filters. \renata{Then, the authors in~\cite{bb:Coors2018VISAPP} combine data augmentation with an unsupervised loss.}
	%Further, the authors in~\cite{bb:dima} propose to use max-pooling to combine the outputs of these networks. %extend this multi-column deep neural networks with averaging the output of all the columns to provide the final classification label.
	%A different approach was proposed in~\cite{bb:STN}, where the authors introduce a new spatial transformer layer that deforms images according to a predefined transformation class. \renata{The authors in~\cite{bb:dai2018} extend this work by proposing deformable convolutional networks, which learns offsets for augmenting the spatial sampling locations.} Then, the work in \cite{bb:marcos2016learning} suggests using rotated filter banks and a special max pooling operation to combine their outcomes and improve invariance to transformations.
	...
	%The authors in \cite{bb:cohen2016group} propose a generalization of the ConvNets and introduce equivariance to $90\degree$ rotations and flips. Finally, the authors in \cite{bb:dieleman2015rotation} exploit rotation symmetry in the Convolutional Network for the specific problem of galaxy morphology prediction. This work has been extended in~\cite{bb:dieleman-cyclic-2016} which introduces an additional layer that makes the network to be partially invariant to rotations. All the above methods, however, still need to be trained on a large dataset of randomly rotated images in order to be rotation invariant and achieve effective performance.
	...
	%\pascal{you might list a couple of more references here, like Weiling for example, without getting into details}.
	%The graph representation is particularly interesting because it is very generic and possibly applicable to various datasets and modalities.
	...
	%\renata{Further, the authors in~\cite{bb:CayleyNets} improve the algorithm of detection of the narrow frequency bands of importance by introducing CayleyNets. }
	...
	%is not directly amenable to transformation-invariant image classification.
	...
	%\renata{In this paper we theoretically extend the work \cite{bb:KhasanovaF17}, which proposes to bridge this gap and present a novel method that uses the power of graph signal processing to add translation and rotation invariance to the image feature representation learned by deep networks.}
	...
	%If we denote by $\mathcal{F}(\cdot)$ the action of the spectral convolutional filter, the equivariance property means that the following condition holds:
	%
	%%\begin{equation}
	%%\mathcal{F}\left(R\left(y\right)\right) = R\left(\mathcal{F}\left(y \right) \right),
	%\end{equation}
	%\noindent
	%for all isometric transformations $R = R(\cdot)$ and graph signals $y$. Several examples of isometric transformations are shown by Fig.~\ref{fig:iso}. The equivariance to isometric transformations is valid as the filters operate on a regular grid graph $G$.
	%% \pascal{You had mentioned that the equivariance is also due to the fact that the filters are locally defined in a M-hop neighborhood. Why is that? You might want to add it back if you think it is key.}
	%The signal $y_i(v_k)$ on the vertex $v_k$ without loss of generality can be represented in the spatial domain as:
	%	%
	%	\begin{equation}
	%	\mathcal{F} * y_i(v_k) = \sum_{j=0}^A w_j y_i(v_j),
	%	\label{eq:hop}
	%	\end{equation}
	%	%
	%	where $y_i(v_j)$ is a signal on the vertex $v_j$, which is in the neighborhood of $v_k$ and $w_j, j\in [0, A]$ are scalar weights. Moreover, weight $w_{j_1} = w_{j_2}$ if vertex $v_{j_1}$ and $v_{j_2}$ are in the same hop $H(v_k)$ of node $v_k$. Here, by hop we refer to the set of nodes, which have the same edge distance to the node $v_k$. Fig.~\ref{fig:hop} illustrates different hops with different colors for a node $i$. The power $m$ of the Laplacian matrix defines the $m$-hop $H_{mi}$ from the vertex $v_i$ to which convolutional filter with weight $\alpha_m$ is applied. This property makes the filter equivariant to the isometric transformation of a signal on a regular grid graph.
	%
	%\begin{figure}
	%	\centering
	%	\includegraphics[width=0.5\linewidth]{images/hop.png}  \\
	%	\caption{The figure depicts the $H_{0i}$ (blue), $H_{1i}$ (red) and $H_{2i}$ (green) hops for the node $v_i$ (best seen in colors).}
	%	\label{fig:hop}
	%\end{figure}
	%
	%
	%\begin{figure}[h!]
	%	\begin{tabular}{cc}
	%		\raisebox{0.6cm}{Rotation} & \includegraphics[width=0.49\linewidth]{images/iso/Rot}  \\
	%		\raisebox{0.6cm}{Translation} & \includegraphics[width=0.49\linewidth]{images/iso/Tr} \\
	%		\raisebox{0.6cm}{Reflection} & \includegraphics[width=0.49\linewidth]{images/iso/Refl}  \\
	%	\end{tabular}
	%	\caption{Examples of isometric transformations $R$.}
	%	\label{fig:iso}
	%\end{figure}


	%Specifically, the sets ${\mathcal{N}_i^{(1)}}=\mathcal{V}, \ \forall i$, represents all the nodes of the graph in the first convolutional layer (i.e., the filtering operators are applied on all the vertices of the input images), while ${\mathcal{N}_i^{l}}$ represents only a subset of nodes selected by the pooling layers at the next levels of the network.
	...
	\begin{algorithm}[h!]
		\begin{algorithmic}[1]
			...
			%	\STATE {\bf Output: }  new set of active nodes $\mathcal{N}^{l}$
			\vspace{5pt}
			\FOR{$i \in [1,K_l]$}
	...
	\begin{figure}[!t]
		...
		\caption{Pooling process, with succession of dynamic pooling layers with operators $\mathcal{P}_i^{l}$ that each selects the vertices with maximum intensity according to Eq.~(\ref{eq:omega}).
			%	from the set $\Omega^{l-1}=\bigcup\limits_{i=1}^{K_{l-1}}\mathcal{N}_i^{l-1}$ given by the previous network layer.
		}
		\label{fig:pool}
	\end{figure}
	...
	%\renata{We further remark that the pooling task is equivalent to task of finding $J_l$ highest statistics and the average computational complexity with the selection algorithm is $O(N)$~\cite{bb:Knuth98}. This complexity is comparable with complexity of the classical pooling operator.}
	%\pascal{is that fully correct? Do we want to give a bit more details?} \pascal{If the complexity is indeed relevant, we should say if it is high, low, manageable...}.
	...
	%\pascal{how? give at least some high level intuitions/explanations/reasons}.
	...
	%%graph isometric transformations of $y$ (e.g. image rotations by $90^\circ$ and translations by an integer number of pixels in horizontal and vertical directions).
	...
	%A formal definition of the quasi-invariance property is provided in Section~\ref{s:git}.

	\subsection{Graph isometric transformation}
	\label{sec:90deg}
	%The image is represented as a graph $G$ which has $N$ nodes $v_i, i \in [1, .., N]$.
	...
	%by their edge weights $w(v_i, v_j)=1$.
	%The isometric transformation preserves distances, which we define between all pairs of adjacent nodes $v_i \sim v_j$ by their edge weights $w(v_i, v_j)=1$.
	%Additionally, as we define $g$ on graphs in such a way that the transformed signal always matches the nodes, then $g$ can be seen as a permutation function.
	%
	...
	%The whole set of these transformations $g$ include the following operations, applied to the input image $y$: rotations by $90^\circ, 180^\circ, 270^\circ$; reflection; translation by an integer number of pixels and their combinations.
	...
	\begin{theorem}
		...
		%The filters from Eq.~\ref{eq:pol_filt_1} produce features equivariant to $g$.
		\label{t:pol_fil}
	\end{theorem}
	...
	\begin{proof}[Proof of Theorem~\ref{t:pol_fil}]
		...
		% According to its definition, the graph isometric transformation $g$ is a permutation function that can be formally described as
		%\begin{equation}
		%\small
		%\forall v_k \in \mathcal{V}, \; \exists \; !\; v_j \in \mathcal{V}: y_g(v_k) = y(v_j) \;,
		%\label{eq:def}
		%\end{equation}
		%\noindent
		%where $!$ indicates that the correspondence between vertices $v_k$ and $v_j$ is a bijective mapping. Given this we can prove the theorem by induction.
		...
		%	\begin{equation}
		%	\forall v_l \sim v_k, \; \exists \; !\; v_i \sim v_j \; : \; y_g(v_l) = y(v_i) \;.
		%	\label{eq:filt_neigh}
		%	\end{equation}
		%
		%by $y$ where each $k^{\text{\tiny th}}$ signal value $y_g(v_k)=y(v_j)$ equal to the value of signal $y$ on node $v_i$.
		...
		%		We show that $f_g(v_k)=f(v_j)$
		%	To do so, we evaluate the former at a random graph node $v_k \in G$ that is mapped to $v_j$ via $g$, which we use for the evaluation of $f$.
		...
		%As the transformation $g$ is isometric and preserves distances defined on the graph, the signal values $y_g(v_l), v_l \sim v_k$ of adjacent nodes to $v_k$ are the same as $y(v_i), v_i \sim v_j$.%~(see~Eq.~(\ref{eq:def})).
		%Thus, as $y_g = g(y)$ and $y_g(v_k)=y(v_j)$ the filtered version of the signal has the following relationship:
		\begin{equation}
		...
		%Now, by induction let us show that the rules holds for any polynomial degree.
...
Based on Eqs.~(\ref{eq:ind_f_blast})~and~(\ref{eq:ind_f_last}) we can conclude that a polynomial filter of any degree produces features  that are equivariant  to the graph isometric transformations.
	...
	%We now prove that the filters from Eq.~\ref{eq:pol_filt_1} produce features equivariant to $g$ by induction.
	...
	%
	%The general isometric transformation of the signal $y$, defined on the graph $G$, is a transformation that
	...
	% we approximate $g_{\gamma,\xi}$ with a graph isometric transformation $\bar{g}_{\gamma,\xi}$ (i.e, the bijective mapping that preserves the local neighborhood, as defined in Section~\ref{sec:90deg}). This
	...
	%In this section we show that, given some assumptions on the smoothness of the signal $y$ defined on the graph nodes, Laplacian polynomial filters are quasi-equivariant to general isometric transformation, and not only by graph isometric transformations given in Definition \ref{def:graph_eq}.
	\begin{mydef}
		...
		%\renata{which without loss of generality can be rewritten as }
		\begin{equation}
	...
	% any isometric transformation.
	%\renata{Here, we assume that the signal $y$ is a piece-wise linear function as images have discrete nature and for any two pixels signal can be defined as a linear function and for larger neighbourhoods can be approximated by it. }

	%More formally,
	%
	%Similarly to the case of graph isometric transformations, provided we ignore the boundary effects, the general isometric transformations satisfy all four fundamental group properties and therefore form a group of operations.
...
%As we are considering image rotations around the vertex $v$, where $\mathcal{F}$ is applied, we do not change the neighborhood of $v$, therefore without loss of generality we can replace $\bar{g}_{\gamma,\xi}$ with the identity transform.


%	This allows us to rewrite the equivariance condition of
...
\begin{theorem}
		%Provided a signal $y$, defined on the graph $G$ can be approximated by a piece-wise linear function $f$, then the response of the polynomial filter $\mathcal{F}$, applied to such  $f$, is equivariant with respect to an arbitrary image rotation.
		...
		%	Any polynomial filer $\mathcal{F}$ defined in Eq.~(\ref{eq:pol_filt_1})  is equivariant with respect to $g_\gamma$, which is equivalent to proving that the following inequality is valid
		%
		\begin{equation}
		...
		\end{equation}
		\noindent
		%	for any given positive $\epsilon$:
		%	\begin{equation}
		%	\epsilon \geq 2^{M-1} \left| (1 - \sin\gamma - \cos\gamma) \bar{\mathcal{Z}} \right| \;,
		%	\label{eq:rot_cond}
		%	\end{equation}
		%	\noindent
		...
		\small
		\bar{\mathcal{Z}} =
		%	\max_{0 \leq a,b \leq 1} \left| \partial_a^2 f(a,b) \Delta a^2 + \partial_b^2 f(a,b) \Delta b^2 \right| + o(\Delta a^2) + o(\Delta b^2).
	...
	\begin{proof}[Proof of Theorem \ref{t:im_rot}]
		...
		%\subsubsection{Random image rotation}

		%In this section we analyze equivariance to the random rotations of the filter responses.
		%Usually filters are applied to digital images
		...
		\begin{figure}
			\centering
			%\includegraphics[width=0.7\linewidth]{images/rotation_equivariance_1.pdf}
			...
			%\caption{(Left) Illustration of a 9-pixel image pattern, where $p_1, .., p_9$ are pixels intensities. (Right) Pattern rotated around pixel $5$ and the value of intensity of the pixel $p_2$ is replaced by the value $r_2$, which can be approximated by bilinear interpolation from the nearest points.}
			\label{fig:pattern}
		\end{figure}
		%
		...
		%\begin{equation}
		%\begin{array}{ll}
		%p_2 = p_5 + \partial_b f(a_5, b_5) \Delta b &+ R_2(b)\;, \\
		%p_4 = p_5 - \partial_a f(a_5 - \Delta a, b_5) \Delta a &- R_2(a)\;, \\
		%p_6 = p_5 +\partial_a f(a_5, b_5) \Delta a &+ R_2(a)\;, \\
		%p_8 = p_5 - \partial_b f(a_5, b_5- \Delta b) \Delta b &- R_2(b)\; , \\
		%\end{array}
		%\label{eq:points_o}
		%\end{equation}
		\noindent
		...
		%r_2 = r_5 + & \partial_a f(a_5, b_5) \Delta a \sin\gamma &+\\
		%&\partial_b f (a_5, b_5) \Delta b \cos\gamma &+ o(\Delta a^2 + \Delta b^2);\\
		%
		%r_4 = r_5 -& \partial_a f(a_5-\Delta a, b_5) \Delta a \cos\gamma &+\\
		%&\partial_b f (a_5, b_5) \Delta b \sin\gamma &- o(\Delta a^2 - \Delta b^2);\\
		%
		%r_6 = r_5 +& \partial_a f (a_5, b_5) \Delta a \cos\gamma &-\\
		%&\partial_b f (a_5, b_5-\Delta b) \Delta b \sin\gamma &+ o(\Delta a^2 - \Delta b^2);\\
		%
		%r_8 = r_5 -& \partial_a f (a_5-\Delta a, b_5) \Delta a \sin\gamma &-\\
		%&\partial_b f (a_5, b_5-\Delta b) \Delta b \cos\gamma &-o(\Delta a^2 + \Delta b^2).\\
		\label{eq:points_r}
		\end{equation}
		\noindent
		where
		...
		\end{bmatrix} \\
		 & + o(\Delta a^2) + o(\Delta b^2)\;.
		\end{aligned}
		%\big( [\partial_a f (a_5, b_5)  - \partial_a f (a_5-\Delta a, b_5)] \Delta a + \\
		%+ & [\partial_b f (a_5,b_5)  - \partial_b f (a_5,b_5-\Delta b)] \Delta b\big). \\
		\end{equation}
		\noindent
		...
		%Then the difference between these two responses can be calculated as:
		\begin{equation}
		\Delta_{\hmF^1}(v_5) = \frac{\alpha_1}{4} (1 - \sin\gamma - \cos\gamma) \mathcal{Z}(v_5).
		\label{eq:diff_resp}
		\end{equation}
		%
		%gives the results, described in Section~(\ref{sec:90deg}) and the differences between filter responses for signals $y$ and $g_\gamma(y)$ is equal to $0$.
		...
		%To derive the bound, defined in Eq.~(\ref{eq:sec_der}), we observe that for any graph node $p_5$
		%
		\begin{equation}
		\small
		...
		%To finish the proof of the theorem, we show that if a polynomial filter $\mathcal{F}$ of degree $M-1$ is equivariant with respect to a random image rotation $g_\gamma$, then the filter of degree $M$ is equally equivariant to $g_\gamma$.
		%
		...
		%Based on Eqs.~(\ref{eq:filtered_signal})~and~(\ref{eq:filtered_trans_signal}) we can then express Eq.~(\ref{eq:ind_rot}) as
		%
		\begin{equation}
		\small
		...
		\end{equation}
		\noindent
			%for every node $v \in G$. %, with $\Delta_{\mathcal{F}^{M}} (v)$ being the difference between the responses of a polynomial filter $\mathcal{F}^{M}$ applied to the original image $y$ and its rotated version $y_g$ at node $v_j$.
		...
		%
		%\renata{For piece-wise linear function $y$ in the $k$-hop neighborhood of each node, }
		%We omit the complete proof here due to the space limitations and because the proof is analogous to the proof of the Theorem~\ref{t:pol_fil} in Section~\ref{sec:90deg}.
		%The error is accumulated with the degree $M$ of the polynomial function:
		%\begin{equation}
		%\epsilon \geq M \left| (1 - \sin\gamma - \cos\gamma) \mathcal{Z} \right| \;.
		%\label{eq:error_any}
		%\end{equation}
		%The only difference is that with the growth of the degree of the filter we need to have stronger constraints on the smoothness of the signal $y$. More formally for a polynomial filter of degree $k$ we require that the signal $y$ can be approximated by a piece-wise linear function in the $k$-hop neighborhood of each node in order to preserve equivariant properties.
	\end{proof}
	...
	%Furthermore, we can see that for a piece-wise linear function $f(a,b)$ that has its second partial derivatives $\partial^2_a f$ and $\partial^2_b$ equal to $0$, the difference between filter responses, $\Delta_\mF$, is also equal to $0$ for any angle $\gamma \in [-\frac{\pi}{4},\frac{\pi}{4})$. \renata{Thus, equivariance gap depends on the image properties.}
	...
	%Similarly to Section~\ref{sec:rot} we observe that $y_g = g_\xi(y)$ is defined for any pair $(a,b)$: $0\leq a,b \leq1$. However, in the second term of the right hand side of Eq.~(\ref{eq:equivariance_ref_trans_notations}) the filter response $F(\cdot)$ is defined only for such $(a,b)$ that correspond to the graph nodes. Thus, we need to approximate transformation $g_\xi$ applied to it with a graph isometric one $\bar{g}_\xi$.
	...
	%\renata{This allows us to prove the following theorem.}

	%Given the fact that any random translation $g_\xi$ can be represented by a combination of translations in horizontal and vertical directions, here we show the proof only for horizontal image translation, as the proof for the other direction is identical. The equivariance of $\mathcal{F}$ in a general case, with respect to any random translation then naturally follows from these two cases and the transitivity property of isometric transformations.

	%For the sake of simplicity we consider a 1D signal $f(a), 0 \leq a \leq 1$ (extension to the 2D case is straightforward). Using the introduced notations, we can formulate the following theorem.
	...
	\begin{theorem}
		...
		%	for any given positive $\epsilon$:
		%	\begin{equation}
		%	\epsilon \geq 2^{M-1} \left|\max_{0 \leq a \leq 1} \partial^3_a f(a) \Delta a^3 + o(\Delta a^2)\right| \;,
		%	\end{equation}
		%Provided a signal $y$, defined on the graph $G$ can be approximated by a piece-wise quadratic function $f$, then the response of the polynomial filter $\mathcal{F}$, applied to such $f$, is equivariant with respect to a random image translation $g_\xi$.
		\label{t:im_trans}
	\end{theorem}

	%\subsubsection{Random image translation}
	\begin{proof}[Proof of Theorem~\ref{t:im_trans}]
		...
		%\begin{equation}
		%\Delta_\mathcal{F}(v_5) = \mathcal{F}(g_\xi(y))(v_5) - \mathcal{F}(y)(v_5) \;,
		%\label{eq:response_del_trans}
		%\end{equation}
		%\noindent
		...
		%We omit the complete proof here because it is analogous to the Theorems~\ref{t:im_rot}.
		...
		%- the only difference with Section~\ref{sec:90deg} is that with the growth of the degree of $\mathcal{F}$, grows the neighborhood, where the signal is required to be close to a linear or quadratic function in order to keep $\Delta \mathcal{F}$ close to $0$ and preserve equivariant properties.
		%
	\end{proof}
	...
	\label{s:discussion}
	%\renata{...Comment check that it is true for translation and rotation (before we had it only for rotation, but it is better to have for both cases)}
	...
	\begin{lemma}
		...
		%the difference $\Delta_\mathcal{F}$ between the responses of filter $\mathcal{F}$ applied to $y$ and its rotated version $y_g$ is lower then a predefined constant $\epsilon_\text{def}$.
		\label{l:im_res}
	\end{lemma}
	...
	%Theorem~\ref{t:im_rot} and Result~\ref{l:im_res} further allow us to compute the expected difference between the responses of the polynomial filter $\mathcal{F}$ applied to the input image signal $y$ and its rotated version. We now consider the angle of rotation $\gamma$ as a random variable that is uniformly distributed in the interval $[0, 2\pi)$. We can then write the expected difference between filter responses as:
	%\begin{equation}
	%\small
	%\mathrm{E}\left[\Delta_\mathcal{F}\right] = \int_{0}^{2\pi} A \gamma (1 -\sin\gamma - \cos\gamma) d \gamma = 2\pi (\pi+1) A\;,
	%\label{eq:expect}
	%\end{equation}
	%\noindent
	...
	%\begin{equation}
	%\small
	%A=\partial^2_a f (a_5, b_5) \Delta a^2 + \partial^2_b f (a_5, b_5) \Delta b^2 + o(\Delta a^2) + o(\Delta b^2).
	%\end{equation}
	%\noindent
	%As we can see, $A$ depends on the values of the second derivative of the input signal $f$, therefore we can conclude that if the signal changes smoothly, then $\mathrm{E}[\Delta_\mathcal{F}]$ is close to $0$. Further, in a special case of $f$ being a piece-wise linear function, the difference $\mathrm{E}[\Delta_\mathcal{F}]$ between filter responses is actually equal to $0$.
...
%	if the input image is smooth (i.e under some constraints on the second and third partial derivatives of the image signal) our polynomial filters are quasi-equivariant to random image rotations and translations.
%	%. Further this assumption is relaxed to allowing the $y$ being close to piecewise quadratic to make polynomial filters $\mathcal{F}$ equivariant to random image translations.
%	Additionally, as shown in Section~\ref{sec:90deg} our polynomial filter $\mathcal{F}$ is equivariant to reflection transformation without any assumptions on the input image signal. This, altogether, proves that our polynomial filters are quasi-equivariant to any isometric transformation as the latter can be represented as the combination of rotation, translation and reflection.
	...
	\begin{itemize}[topsep=0pt, partopsep=0pt]
		...
		%\paragraph{Rotated and translated MNIST.}
	...
	\begin{figure}[t!]
		...
		%\pascal{the text on the x-axis should be 'spectrum', for example, as you don't explicitly show the eigenvalues. on the y-axis, it should 'magnitude', or similar.}}
		\label{fig:filter_ex}
	\end{figure}
	%
	\begin{figure}[t!]
		...
		%\pascal{Does $M=1$ make sense? the filter would be very simple in that case, no?} \renata{[I have updated the image for $M = 3$]}}
...
%	\begin{equation}
%	\delta = \mathcal{F}_\text{inv_rot}(y_\text{_rot}) - \mathcal{F}(y_\text{orig}),
%	\end{equation}
%	where $\mF_\text{inv_rot}(y_\text{_rot})$ denotes inverse rotated feature map. The fig.~\ref{fig:rot_resol} shows result of the experiment.
%		\begin{figure}[t!]
%		\centering
%		\includegraphics[width=0.8\linewidth]{images/???}
%		\caption{Influence of image resolution to the quasi-invariance.}
%		\label{fig:rot_resol}
%	\end{figure}
	...
	% The specific architectures used in this experiments are given in Table~\ref{tab:arch_part2}.
	...
	\begin{table}[t!]
		...
		\begin{tabularx}{\linewidth}{ X c c }
			...
			\midrule
			%\multicolumn{2}{l}{\scriptsize{\bf{Training set without data augmentation}}}\\
	...
	%and has an effect on the image borders, which degrades the performance.
	...
	%\subsection{Rotation $\gamma$}

	%We apply the filter to images which are discretized version of a real world scene. We assume that real signal is continuous and differential function $f(x,y)$. Then, to calculate differences between two filtered version of the signal (rotated and original), we express of values of our discrete image from the value of the pixel to which we apply filter.
	%\begin{equation}
	%\begin{array}{l}
	%f(v_2) = f(v_5) + \sum_{n=1}^{\infty}\frac{1}{n!}\frac{\partial^n f (v_5)}{\partial y^n} \Delta y; \\
	%f(v_4) = f(v_5) - \sum_{n=1}^{\infty}\frac{1}{n!}\frac{\partial^n f (v_5 - \Delta x)}{\partial x^n} \Delta x; \\
	%f(v_6) = f(v_5) + \sum_{n=1}^{\infty}\frac{1}{n!}\frac{\partial^n f (v_5)}{\partial x^n} \Delta x; \\
	%f(v_8) = f(v_5) - \sum_{n=1}^{\infty}\frac{1}{n!}\frac{\partial^n f (v_5- \Delta y)}{\partial y^n} \Delta y. \\
	%\end{array}
	%\label{eq:response_pattern_o}
	%\end{equation}

	%Then, rotated pattern has the following response:
	%\begin{equation}
	%\begin{array}{ll}
	%f(v_2) = f(v_5) +& \sum_{n=1}^{\infty}\frac{1}{n!}\frac{\partial^n f (v_5)}{\partial x^n} \Delta x \sin\gamma +\\
	%&\sum_{n=1}^{\infty}\frac{1}{n!}\frac{\partial^n f (v_5)}{\partial y^n} \Delta y \cos\gamma;\\

	%f(v_4) = f(v_5) -& \sum_{n=1}^{\infty}\frac{1}{n!}\frac{\partial^n f (v_5-\Delta x)}{\partial x^n} \Delta x \cos\gamma +\\
	%&\sum_{n=1}^{\infty}\frac{1}{n!}\frac{\partial^n f (v_5)}{\partial y^n} \Delta y \sin\gamma;\\

	%f(v_6) = f(v_5) +& \sum_{n=1}^{\infty}\frac{1}{n!}\frac{\partial^n f (v_5)}{\partial x^n} \Delta x \cos\gamma -\\
	%&\sum_{n=1}^{\infty}\frac{1}{n!}\frac{\partial^n f (v_5-\Delta y)}{\partial y^n} \Delta y \sin\gamma;\\

	%f(v_8) = f(v_5) -& \sum_{n=1}^{\infty}\frac{1}{n!}\frac{\partial^n f (v_5-\Delta x)}{\partial x^n} \Delta x \sin\gamma -\\
	%&\sum_{n=1}^{\infty}\frac{1}{n!}\frac{\partial^n f (v_5-\Delta y)}{\partial y^n} \Delta y \cos\gamma;\\
