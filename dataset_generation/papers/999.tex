...
% \runninghead{Assia Benbihi, Matthieu Geist, Cedric Pradalier}{BMVC Author Guidelines}
...
%Many applications that relies on reccurent semantic segmentation 
%such as environement monitoring or medical image processing generates such dataset.
%For example, two image surveys of the same natural environment at two different times 
%have approximately the same semantic content but distinct pixel distribution 
%because of the image quality. This prevents a network trained on the 
%first survey to generalize to the other.
%And the content change prevents the use of the annotation of one survey to 
%another. 
...
%D1 VOC 12: acc: 79.92 - rec (/8): 84.52 - mIOU: 69.22
