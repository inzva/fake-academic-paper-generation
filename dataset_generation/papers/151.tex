...
%\author{Wei-Chih Hung$^1$, Yi-Hsuan Tsai$^2$, Yan-Ting Liou$^3$, Yen-Yu Lin$^4$, Ming-Hsuan Yang$^1$\\
%	$^1$University of California, Merced \hspace{20pt}%
%	$^2$NEC Laboratories America  \hspace{20pt} \\
%	$^3$National Taiwan University \hspace{20pt}
%	$^4$Academia Sinica, Taiwan \hspace{20pt}
%}
...
\begin{document}
	...
	% introduce semantic segmentation
	% most CNN-based method require sufficient amount of data
	...
	%in a high-order structure, rather than only the point-wise supervision of the label maps.
	...
	%
	%However, this approach does not improve the performance according to our experiments because the adversarial loss will aggressively encourage the predictions to be close to the ground truth distribution and neglects the correctness of segmentation.
	%
	...
	%\vspace{-3mm}
	%\noindent{\bf Fully-supervised semantic segmentation.}
	%
	%Recent state-of-the-art methods for semantic segmentation are based on the rapid development of CNN. As proposed by \cite{fcn}, one can transform a classification CNN, e.g. AlexNet~\cite{alexnet}, VGG~\cite{vgg}, or ResNet~\cite{resnet}, to a fully-convolutional network (FCN) that tackles the task of semantic segmentation.
	%%
	%Many advanced methods are then proposed based on the FCN framework. To employ the long-range inference, Conditional Random Fields (CRFs) are widely used as the post-processing module of FCN~\cite{deeplab,crfasrnn,piecewise}. \cite{deeplab} apply the dense CRF to the output of FCN, while \cite{crfasrnn} use Recurrent Neural Network (RNN) to simulate the mean-field optimization for CRF with learnable parameters. Furthermore, \cite{piecewise} directly train an extra network branch to learn the pair-wise potential term in the CRF.
	%%
	%
	%Other than CRF, recent methods show that the context information can be exploited by introducing moderate modifications to the network structure. \cite{dilated} propose to use dilated convolution for encouraging the network to ``see'' more, i.e., enlarging the receptive field, and thereby more context information is perceived.
	%%
	%In an aggressive way to consider global context, \cite{pspnet} add the spatial pyramid pooling layers to allow each spatial location to perceive the global features and achieve improvements on several datasets.
	%%
	%However, these methods require a substantial amount of training data and are not guaranteed to converge well under the scenario of lacking supervised training data.
	...
	%MH: methods... method... Do you really read what you write?
	%Different from the aforementioned methods, our method can leverage unlabeled images in model training, hence greatly saving the cost of manual annotation.
	...
	%
	%The methodology of adversarial training has been applied to a wide range of applications, 
	...
	%
	%\cite{kozinski2017adversarial} propose to use extra unlabeled data for adversarial learning, and train the discriminator to differentiate predictions between the training and validation sets, while in this work we train the fully convolutional discriminator with spatial support between predictions and ground truth label maps.
	%
	...
	%    It is trained to perform two classes classification on the pixel level to discriminate the segmentation network output and the ground truth label map.
	%
	...
	%MH: it is odd that you describe training objective first before describing network architecture. Why?
	...
	%MH: check the following two subsections in details.	
	...
	%
	%MH: why do you say this? I do not understand this statement
	%
	%Ntoe that we do nto emply any batch-normalization  layer~\cite{ioffe2015batch} as we find it highly unstable when the system can be only trained with small batch size
	...
	%when the system can be only trained with small batch size.
	...
	%, where label $0$ denotes samples from the segmentation network, and label $1$ represents samples from the ground truth label.
	...
	%
	%MH: How many times do I need to tell you that you do not start a sentence with math symol?
	%	$D(S(\mathbf{X}_n))^{(h,w)}$ is the confidence of map $D(S(\mathbf{X}_n))$ at location $(h,w)$ and $D(\mathbf{Y}_n)^{(h,w)}$ is defined similarly.
	...
	%
	%Note that the discriminator network takes a $C$-channel probability map as input.
	%
	...
	%
	%MH: odd sentence
	%With this loss, we train the segmentation network to fool the discriminator by maximizing the probability of the segmentation prediction being considered the ground truth distribution.
	%
	...
	%MH: why do you still make such mistakes?
	%	For semi-supervised training, we randomly interleave labeled and unlabeled data while applying the training scheme described in section \ref{section: training} accordingly.
	...
	%	
	%While the PASCAL VOC dataset contains common objects in photos captured in daily activities, the Cityscapes dataset is consisted of urban street scenes.
	%
	...
	%
	%MH: did you explain what SBD is when you first mention it?
	%	We use the extra annotation set in SBD \cite{sbd}, resulting in 10,582 training images. We evaluate our models on the standard validation set with 1449 images.
	%
	%
	%MH: do you mean the extra annotation set has 10,582 images? or you combine with something else?
	...
	%
	%MH: sometimes you use 10,582 and sometims you use 1449 (should be 1,449). Be consistent.
	%
	%We evaluate our models on the standard validation set with 1449 images.
	...
	%
	%We notice that our proposed semi-supervised method brings consistent $0.8\%$ improvement on top of the adversarial learning though different amount of unlabeled data is used.
	%
	%
	...
	%
	%MH: any more significant difference like what you have in your CVPR paper?
	%
	%Finally, we use the one-hot encoding for the ground truth transform while \cite{luc2016semantic} proposes a \textit{Scale} scheme to diffuse the ground truth probability map.
	%
	...
	%
	%We first evaluate the effect on $\lambda_{adv}$ using fully-supervised setting. Note that we do not use any unlabeled data, i.e., $\lambda_{semi} = 0$.
	%%
	%The baseline model without adversarial learning ($\lambda_{adv} = 0$) achieves $73.6\%$ mean IU. When $\lambda_{adv} = 0.01$, the model achieves $74.9\%$ mean IU with $1.3\%$ improvement.
	%%
	%When $\lambda_{adv} = 0.05$, the performance deprecates to $73.0\%$ mean IU, which indicates that the adversarial loss is too large.
	...
	%
	%MH: what kind of sentence is this?
	%Overall, $\lambda_{semi} = 0.1$ achieves the best mean IU of $69.5\%$ with $1.9\%$ gain.
	%
	...
	%The method performs the best with $T_{semi} = 0.2$.
	%
	...
	\begin{figure}[t]
		...
		\centering
		\begin{tabular}{@{}cccc@{}}
			%\includegraphics[width = 0.2\linewidth, height=0.1\linewidth]{imgs/confidence/1-1.png} &
			%\includegraphics[width = 0.2\linewidth, height=0.1\linewidth]{imgs/confidence/1-2.png} &
			%\includegraphics[width = 0.2\linewidth, height=0.1\linewidth]{imgs/confidence/1-3.png} &
			%\includegraphics[width = 0.2\linewidth, height=0.1\linewidth]{imgs/confidence/1-4.png} \\
			%\includegraphics[width = 0.2\linewidth, height=0.1\linewidth]{imgs/confidence/3-1.png} &
			%\includegraphics[width = 0.2\linewidth, height=0.1\linewidth]{imgs/confidence/3-2.png} &
			%\includegraphics[width = 0.2\linewidth, height=0.1\linewidth]{imgs/confidence/3-3.png} &
			%\includegraphics[width = 0.2\linewidth, height=0.1\linewidth]{imgs/confidence/3-4.png} \\
			
			%\includegraphics[width = 0.2\linewidth, height=0.1\linewidth]{imgs/confidence/2-1.png} &
			%\includegraphics[width = 0.2\linewidth, height=0.1\linewidth]{imgs/confidence/2-2.png} &
			%\includegraphics[width = 0.2\linewidth, height=0.1\linewidth]{imgs/confidence/2-3.png} &
			%\includegraphics[width = 0.2\linewidth, height=0.1\linewidth]{imgs/confidence/2-4.png} \\
	...
	%, while we perform bilinear resizing on the segmentation output map to $32 \times 32$.
	%
