...
%We present a novel method to learn features from images without using human annotation. The proposed method trains a neural network (a discriminator) to tell the difference between two images about the same scene, one of which is missing high level (semantic) details. Since simple image processing can be easily detected by a neural network, we train a denoising autoencoder to recover some of the missing information and use adversarial networks to improve its accuracy. The denoising autoencoder, however, is intentionally designed with a large bottleneck so that it is not forced to learn abstract representations of images, but just fills in realistic textural low-level features. In this way, the denoising autoencoder renders pseudo-real images with missing semantic content. 
%Our experiments show that the features learned by the discriminator transfer (via fine-tuning) to the object classification task on ILSVRC2012, Pascal VOC2007 and STL-10 with state of the art performance.
\end{abstract}
...
%Another example is shown in Figure~\ref{fig:SemanticTraining}: one could take the original image on the leftmost column, remove some low-level details (second column) and then try and recover them.
%In general, one could use adversarial training \cite{goodfellowGAN} to make the reconstruction as accurate as possible and allow for multiple solutions.
%Typically, the discriminator is only used to boost the quality of the generator network, which provides features are extracted.
\begin{figure}[t]
\begin{center}
%\fbox{\rule{0pt}{1.5in} \rule{0.46\linewidth}{0pt}}%
%\fbox{\rule{0pt}{1.5in} \rule{0.46\linewidth}{0pt}}
...
\label{fig:artifacts}
\end{figure}
%\footnotetext{Solution from left to right, top to bottom: fake, fake, real, fake, real, real, fake, fake, real, real, fake, fake, real, real, fake.}
...
% IN linear class SotA
% P2 linear class on par first 3, SotA 4,5
% VOC07 class SotA, det 51.5% on par, VOC12 seg 37% on par
% STL10 class SotA (AlexNet 81%)

% ablation studies running: Thu done

% visualization: Fri done

% NN search, attention maps, filters: Mon



%------------------------------------------------------------------------
% noroozi2017representation
%Check references in the Colorization papers.
%
%
...
%However, to the best of our knowledge no prior work has exploited the classification of real vs  with artifacts as a pretext task for feature learning.\\
...
%\end{subfigure}
%\end{center}
%   \caption{Two examples of corrupt images obtained from our Damage \& Repair network. (a) shows two original images from the ImageNet dataset. At the bottom-left corner of those images we show the masks applied to the encoded feature $\phi(\mathbf{x})$. These masks drop on average about $50\%$ of the encoded feature. (b) shows the output corrupt images. The repair network assists the decoder in inpainting texture that is only locally unnoticeable. However, at the global scale the objects are no longer recognizable as valid instances. (c) shows the output of the decoder when the repair network is not active. In this case the artifacts are very visible and easy to detect by exploiting low-level statistics. (d) shows the output of the decoder when the repair network is not masked. The repair network is then able to change the image more globally. This has a negative effect on the discriminator as it fails to predict the mask.}
%\label{fig:repair}
%\end{figure}
...
%Experiments: 
%1) all the transfer learning results on classification (ImageNet (SOA), PascalVOC (on par) and STL-10 (SOA)) [possible to improve]; 
%2) maybe detection on PascalVOC [to do];
%3) visualization of reconstructions, filter weights, activations (1M patches) [ready, ready, to do]; 
%4) show shared artifacts property by training cartooned AE + GAN and transfer to STL-10 [to do]; 
%5) show full system training without the feature matching term on STL-10 [to do]; 
%6) show poor results of just skipping low-level details and training directly a Discriminator as a binary classifier ? transfer to STL-10 [ready]; 
%7) show poor results of using an image to image AE and then training the Discriminator as a binary classifier between a reconstruction from the encoded feature and a reconstruction from a noisy encoded feature [just left to transfer to STL-10].
...
%- Real vs fake 
%
%- Different drop probabilities (10\%, 30\%, 50\%, 70\%, 90\%) 
%
%- Mask prediction Y/N 
%
%- Distributed network augmentation vs local 
%
%- Encoder with overlapping/independent features (vary stride) 
%
%- No gating in the repair/augmentation network 
%
%- No history of fake examples
%
%Visualisations: 
%
%- Nearest neighbour search 
%
%- Reduced number of repair layers 
%
%- Hand-crafted drop masks 
%
%- Visualisation of what the discriminator bases its decision on
%
%
%Transfer: 
%
%- Linear classification: Imagenet \& Places 
%
%- Detection: VOC2007 \& VOC2012 (COCO is not commonly done so I think it makes no sense?) 
%
%- Semantic Segmentation: VOC2012 
%
%- Normal Estimation: NYUD (only few methods have been tested on this)
...
\resizebox{\linewidth}{!}{
    ...
    \end{subfigure}
%    \begin{subfigure}[]{0.11\linewidth}
%        \includegraphics[height=4cm]{figures/abl_base}
%        \caption{}
%    \end{subfigure}
    \begin{subfigure}[]{0.11\linewidth}
...
%-------------------------------------------------------------------------
%%%%%%%%%%%%%%%% CONCLUSIONS ARE ALREADY EDITED %%%%%%%%%%%%%%%%%%
\color{black}
