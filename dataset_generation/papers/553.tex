...
%   draft.tex
%
%	einet89@postech.ac.kr
%	seungjin@postech.ac.kr
%
%	Last updated:
%	May 24, 2018
%
%	draft paper: icml-2018
%
%
...
\twocolumn[
%\icmltitle{Learning to Learn or Not to Learn in Meta-Learning}
%\icmltitle{Layerwise Riemannian Tangent Space Learning for Meta-Learning}
%\icmltitle{Layerwise Subspace Metric Learning for Meta-Learning}
%\icmltitle{Layerwise Subspace and Mahalanobis Metric Learned by a Meta-Learner}
%\icmltitle{Meta-Learning with Adaptive Layerwise Metric and Subspace}
...
% /* main manuscript begins */
\section{Introduction}
\label{sec:introduction}

% Deep learning = good, but weak on small data
...
\WHILE{not done}
    ...
    \FORALL{$\mathcal{T}_j$}
        \FOR{$i=1, \cdots, L$}
            ...
            %\STATE $\widetilde{\W}^{i}_{\calT_j}=\Wi-\alpha \nabla_{\Wi} \loss (\theta_{\W}, \theta_{\T},\calD_{\calT_j,train})$
        \ENDFOR
...
\WHILE{not done}
    ...
    \FORALL{$\mathcal{T}_j$}
        \FOR{$i=1, \cdots, L$}
            ...
            %\STATE $\widetilde{\W}^{i}_{\calT_j}=\Wi-\alpha \mask^i \odot \nabla_{\Wi}  \loss (\theta_{\W}, \theta_{\T},\calD_{\calT_j,tr})$        	
...
%If we view the product $\T \W$ as a single matrix, the update according to ($\ref{eq:theta_wt}$) step in the direction $\T  \T^{\top} \nabla_{\A} \loss$.
%This is exactly the direction with the steepest decrease in $\loss$ with respect to the metric $(\T \T^{\top})^{-1}$.
...
% Give this part a separate proposition?
...
% Ours. Relation to NN as learner
...
% RNN as learner, backprop as meta-learner
