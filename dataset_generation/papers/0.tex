...
% http://www.springer.com/mathematics/geometry/journal/41468 (Journal of Applied and Computational Topology)
...
\end{align*}
%Note that, by construction, both simplicial complex models satisfies $\cech(P,r)\subset \cech(P,s)$ and $\rips(P,r)\subset \rips(P,s)$ for $r\leq s$.
...
%For details, readers may refer to Appendix~\ref{sec:homology}.
...
\begin{figure}[htbp]
  ...
  % Note that, in the program, C=(lambda)^{-1} is used.
...
%Since PI uses Gaussian distributions, only birth-death pairs {\em around} such selected grids are important to distinguish between P-A and P-B. 
%When $\lambda$ becomes larger, the number of selected grids becomes smaller. Only a few especially important grinds are selected when large $\lambda$ is used. 
%The phenomenon reflects the fact that $\lambda$ controls the complexity of learned result.
%The sparseness is useful for the study with PD and ML since the learned information is strongly compressed and easy to understand.
...
%This is because such standard image classification techniques are developed for photos of faces, artificial objects, or landscapes, and not for images like the example in this paper.
...
%We also mention that SVM classifier with RBF kernel realize a bit better prediction performance. This means that a nonlinear transformation such as RBF kernel may show the better performance as we mention in Section~\ref{sec:intro}, but such a transformation makes it difficult to apply the inverse analysis.
...
\begin{table}[htbp]
  ...
  \begin{tabular}{|p{8cm}|l|} \hline
    ...
    %PI, SVM classifier with RBF kernel & 0.935 \\ \hline
...
% [Comparison with PWGK with support vector classifier:
% the mean accuracy is almost same as PIs (0.93),
% PWGK is appropriated by PIs with rbf kernel.
% ]
...
% alphabetic order of first author
