...
%An agent's goal is to maximize the cumulative future reward over a period of time via selecting a suitable action sequence. To find such a sequence, an agent has to learn a policy $\pi$ which represents an agents behavior by mapping states to actions. To find the optimal strategy, DQN, like many other RL algorithms, exploit the characteristics of the so called \emph{Bellman equation}, 
%\begin{equation}
%Q^{\ast}(s,a) = \mathbf{E}_{s^{\prime} \sim \mathcal{E}} \left[r + \gamma \max_{a^{\prime}} Q^{\ast}(s^{\prime}, a^{\prime}) | s,a \right], 
%\end{equation} 
%where, $\gamma$ is a discount factor for future rewards, and  $Q^{\ast}(s^{\prime}, a^{\prime})$ is the optimal value of being in $s^{\prime}$ at the next time-step taking action $a^{\prime}$. In contrast to the original Q-learning algorithm, DQN approximates the state-action value using a neural network with weights $\theta$, such that $Q(s,a; \Theta) \approx Q^{\ast}(s,a)$. The authors in \cite{Mnih.2013} refer to this kind of network as Q-network. 





%DQN relevant parameters:
%temporal_window
%layers
...
%Policy is the agents behavior, represented as a function that maps states to actions. 

%State-action value functions $Q^\pi (s,a)$ calculates the expected return when starting in state $s$, performing action $a$, and following policy $\pi$. 

%Motivation for Deep Q-Learning: huge state spaces requires a functional representation of the space as storing all state-action combinations is impractical. Q-Learning cannot generalize to unobserved states.
...
%maximum overall network outputs max
%a’ Q(s’, a’)
%- Set Q-value target for action to r + γmax
%a’ Q(s’, a’) (use
%the max calculated in step 2).
%- For all other actions, set the Q-value target to the same as originally
%returned from step 1, making the error 0 for those outputs.
%- Update the weights using backpropagation.

%DQN tricks: Experience replay: store experiences and creates mini-batches form them for the training process. 
%Fixed target network. Updating the network every 1000 steps instead of each step. increases stability of training process. Reward clipping: to standardize  rewards across games by setting all positive rewards to +1 and all negative to -1. Skipping frames, skipp every 4 frames to take action
...
% Vielleicht hier noch eine Formel wie sich der Reward dann verändert (Nicklas please? ;) )
