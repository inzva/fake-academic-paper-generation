...
%!TEX root = rmlw17.tex\section{Introduction}\label{sec:introduction}
...
%Our techniques provide a way to generate synthetic images%where the possible image (such as car position, %brightness, contrast, etc.) can be controlled by the user.%The generated dataset is used to study the CNN.%Targeted image sampling methods are used to extract information from %the CNN. Some examples are the CNN sensitivity to%image modifications or the isolation of blind spots. (i.e., images%for which the CNN is not able to detect a car)
