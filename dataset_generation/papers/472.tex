...
\name{\vspace{-3pt}\hspace{-4mm}Mohammad Tofighi, Tiantong Guo, Jairam K.P. Vanamala$^{\dagger}$, and Vishal Monga\vspace{-1pt}}
	...
	%\thanks{Research was supported by a National Science Foundation CAREER Award to V. Monga.}}
...
%For cell detection problem, in \cite{Xing2016}, Xing \textit{et al.} proposed a two-stage approach to detect the nucleus and segment the cells. It generates the initial shapes by training a deep CNN followed by selecting the shapes as boundary of the segmented cells via dictionary learning. However, the cell nuclei detection procedure in \cite{Xing2016} relies on the segmentation generated by a basic CNN, which does poorly in scenarios that involve overlaps in cell nuclei.

% \noindent \textbf{Note:}  We project the effect of shape information as similarity between the shapes and raw cell edges in our cost function of the CNN rather than incorporating it in boundary detection approaches as active contours method or learning a dictionary. Moreover, these methods used shape priors for segmentation, as we used it as prior information for a more accurate nuclei detection.
...
%modeling the shape of the cells as prior information and incorporating it in our cost function and accordingly in our network. Specifically, a new cost function and a new network setup is proposed in this paper. This method is called Shape Priors for Convolutional Neural Networks (SP-CNN).\vspace{-7pt}

\section{Shape Priors with Convolutional Neural Networks (SP-CNN)}\vspace{-3pt}
%There are two parts of SP-CNN, a $D$-layer CNN that performs the nuclei detection by producing detection maps and special prior structure that incorporates the cell nuclei shape information into the training of the CNN.\vspace{-3.5mm}
...
%which has a constant valued $p\times p$ window at each detected location. 
...
%\begin{figure}[t]
%	\centering
%	  \hspace{-2pt}\includegraphics[width=1\linewidth]{voidRegions/voidRegion.pdf}
%	  \caption{\ninept Void training tuple (raw image $\mathbf{x}$, raw edge image $\mathbf{\hat{x}}$, ground truth label image $\mathbf{y}$) elimination procedure.} \vspace{-8pt}
