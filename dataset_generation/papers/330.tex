...
%\thanks{Manuscript received 14 Mar. 2018; revised 21 Jun. 2018}
...
%To preserve the original image size and hand surface information,
...
%The sliced 3D volumetric representation preserves the spatial information but abandons the part of depth information,
%so we use the Deeply-Fusion Network to fuse the features from original depth image and sliced 3D volumetric representation
%hierarchically.

%The left path is \emph{Baseline Regression Network}, which could extract depth information from the
%original depth image. And the right path is similar to the left part which called \emph{Sliced 3D Input Network},
%it adds some $1 \times 1$ convolution kernel to further fuse features channel-wise, by feeding the sliced
%3D volumetric representation into right path, hand structural information is extracted.
...
%Given a sequence of $T$ depth images $(I_1, I_2, \dots, I_T)$, the corresponding pose is $(\J_1, \J_2, \dots, \J_T)$.
