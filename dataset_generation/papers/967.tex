...
% Carl:
% - write experiments for crossmodal classification 
% - write experiments for tSNE
% - update introduction and abstract
...
\label{sec:dataset}
	...
	% We collected 14,830 training examples and 2,050 validation examples.
	
	% Since we keep the location of objects in the sketches, we are able to do data augmentation on the dataset and generate synthetic images by randomly jittering the objects a scene by $[-50, 50]$ pixels in each direction, as long as they remain inside the sketch.
...
\section{Cross-Modal Scene Representation}
    ...
    %% Lluis: For space constraints, I think it's better if we skip this figure
     \begin{figure}
         \centering
    ...
    %To formalize this, we describe the optimization problem we solve during learning. 
    ...
    \begin{table*}[t!]
    	\begin{center}
            ...
            %\multicolumn{2}{|l||} { BL-Shared-All }   & 15.8 & 10.4 & 9.6 & 1.5 & 10.6 & 8.0 & 8.1 & 1.2 & 5.8 & 6.5 & 3.8 & 1.1 & 5.1 & 6.0 & 4.0 & 1.2 & 0.9 & 1.2 & 1.5 & 1.2 & \footnotesize{5.2}\\
             \hline
...
\begin{table*}[t!]
	\begin{center}
		...
		\scriptsize\begin{tabular}{|c|l||r r r r||r r r r||r r r r||r r r r||r r r r||r|}
			...
			%\multicolumn{2}{|l||} { BL-Shared-All }   & 15.7 & 10.5 & 10.3 & 0.9 & 19.2 & 8.0 & 8.9 & 0.5 & 9.1 & 6.8 & 3.9 & 0.3 & 8.4 & 6.0 & 3.5 & 0.3 & 0.7 & 0.8 & 0.9 & 0.8 & 5.8\\ 
			\hline
...
\end{table*}
    ...
    % To reduce the number of parameters, we use diagonal covariances $\Sigma_k$ assuming unit activations are independent.
    ...
    %We illustrate this transfer in Figure \ref{fig:gmm_regularization}.
	...
	% THE NUMBERS ARE WRONG. THEY ARE OLD AND FROM THE PREVIOUS VERSION.
% 	\begin{table}[t!]
%         \begin{center}
%         	\setlength{\tabcolsep}{.2em}
%         	\bgroup
...
%         		 { A: Tune (Free)}  & 19.4 & 22.9 &  85.0 & 13.5 & 34.2 & \bfseries 35.0\\ 
%         		 { B: StatReg (Gaussian) }  & 19.4 & 31.1 & 84.0 & 17.3 & 11.1 & 32.6\\ 
%         		 { B: StatReg (GMM) }  & 19.3 & 31.1 & 82.5 & 16.7 & 13.5 & 32.6\\ 
%         		 { C: Tune + B: StatReg (GMM) }  & 20.2 & 22.5 & 82.2 & 13.1 & 37.0 & \bfseries 35.0\\ 
%         		\hline
%         	\end{tabular}
%         	\egroup
%         \end{center}
%         \caption{\textbf{Within Modal Retrieval mAPs:} We report the mean average precision (mAP) for retrieving images within the same modality using \texttt{fc7} features.}
%         \label{table:within_modal_maps}
%     \end{table}
    ...
    % and decrease the learning rate by a factor of $ \gamma = 0.1$ after a fixed number of iterations.
...
\section{Experimental Results}
	...
	% Our goal in this paper is to learn a representation that is aligned across modalities. We show three main results that evaluate how well our methods address this problem. First, we perform cross-modal retrieval of similar content, evaluated with the scene labels. Secondly, we show visualizations of the learned representations that give a qualitative measure of how this alignment is achieved. Finally, we show we can reconstruct natural images from other modalities using the features in the aligned representation, which gives a qualitative measure of which semantics are preserved in our cross-modal representation. 
	...
	%\textbf{Finetuning with a single shared CNN (BL-Shared-All):} here we use a single instance of Places-CNN shared by all modalities. We finetune it using batches that contain data from each modality. Note that this baseline can only be applied to pixel data because of the different architecture required for text, hence we excluded the descriptions here. However, we employed the textual features from BL-Ind for completeness.
	...
	% PROBABLY ADD THIS BACK FOR CAMERA READY!!!!!!
	% THE NUMBERS ARE WRONG
	%Table \ref{table:within_modal_maps} reports the within-modal retrieval results. By performing alignment through proposed methods, we also increase within-modal retrieval results on average. Table \ref{table:cmr_across_layers} shows the mean performances across layers. We can observe how in general the proposed methods outperform the different baselines in cross-modal retrieval for each of the layers. We can also observe how, as we use features from higher layers in the CNN, the results improve, since they represent higher-level semantics closer to the scene label.
	...
	\begin{figure*}
        ...
        % For instance, unit 115 (which responds most strongly in the location where there is a bed in the images) has become effectively agnostic to the type of input modality, without ever specifying what is a bed and which images or description in the training set contain or talk about beds and without any alignment across modalities during training.
    ...
    \begin{table}[t]
        ...
        %{BL-Individual} & 49.7 & 20.8 & 26.2 & 4.4 \\
	...
	% We now investigate what input data activates units in our shared representation. For \textbf{visual data}, we use a visualization similar to \cite{zhou2014object}. We compute which images produce the highest activations in any of the units forming each of the \texttt{pool5} filters. Then, for each of these images with high activations, we create a binary mask with the same size as the input images by upsampling and assigning the value of each of the 6 x 6 units to the center of their receptive field, and then using Gaussian smoothing and thresholding the obtained values. This method allows us to obtain masks that coarsely highlight which part of the input image produced the strong response of that unit. 
	...
	After running the experiment, we selected those results in which at least 4 images for the real modality were selected. This ensured that the results were not noisy and were produced using units with consistent activations, as we empirically found this to be a good indicator of whether a unit represented an aligned concept. We then computed the number of times subjects selected at least one image in each of the other modalities. With our method, 33\% of the times this process selected at least one image from each modality, whereas for the baseline this only happened 25\% of the times. Furthermore, 19\% of the times we selected at least two images for each modality as opposed to only 14\% for the baseline.  These results suggest that, when a unit is detecting a clear concept, our method outperforms the best finetuning method and can strongly align the different modalities.
    	...
    	\begin{figure*}[t]
    	\begin{center}
    		%\includegraphics[width=0.9\linewidth]{inversions4.pdf}
    		\includegraphics[width=1\linewidth]{inversions_pami.jpg}
    	\end{center}
    	\caption{
    	% \textbf{Inverting features across modalities:} We visualize some of the generated images by our inverting network trained on real images. \textbf{Top row:} reconstructions from real images. These preserve most of the details of the original image but are blurry because of the low dimensionality of the \texttt{pool5} representation. \textbf{Middle row:} reconstructions from line drawings. We can observe that the network adds photorealistic colors to the reconstructions while preserving the original scene composition. \textbf{Bottom row:} inversions from the spatial objects modality. Reconstructions are less detailed but roughly preserve the location, shape and colors of the different parts of the input scene.\vspace{-1em}
    ...
    % 	Our hypothesis is that the reconstruction network will learn to generate images that capture the statistics of the data in the output modality. Furthermore, if our representations are correctly aligned, we expect that the reconstructed images will depict the same concepts across modalities with similar spatial locations. Note that one limitation of these inversions is that the output image is very blurry, even when reconstructing images within a same modality, as the features in \texttt{pool5} discard fine-grained details. However, our reconstructions have similar quality to those in \cite{dosovitskiy2015inverting} when reconstructing from \texttt{pool5} features within a modality.
    ...
    % 	Figure \ref{fig:generated_imgs} shows some successful examples of reconstructions. Reconstructing an image across modalities is hard, arguably because the statistics of the activations in the common representation are very different across modalities despite the alignment. This is a result of the reduced amount of information in some of the modalities (i.e. clipart and spatial text images contain much less information that natural images). However, we note that in the examples the trained model is capable of reproducing the statistics of the output modality. Furthermore the reconstructions usually depict the same concepts present in the original image, indicating that our representation is aligning and preserving scene information across modalities.\vspace{-1em}
...
% The concepts of \emph{near} and \emph{far} transfer are well-known in human psychology  \cite{perkins1992transfer}. Near transfer corresponds to transferring between very similar contexts (e.g. applying the knowledge learned in the classroom for real world tasks), whereas far transfer is concerned with transferring between contexts which seem remote and alien to one another (e.g. employing strategies learned in chess to actual life matters). While near transfer has been explored in computer vision, far transfer is relatively less understood in computer vision, possibly because obtaining higher levels of representation has been challenging. In this paper, we explored learning a scene representation that is agnostic to modality, which we conjecture is an important step towards learning higher-level abstractions used in far transfer.


% Our algorithm for domain adaptation does not require one-to-one correspondences and is able to adapt modalities with big domain shifts into a single common representation. We show how nearest neighbors in this representation are semantically similar by retrieving the top ranked images across domains. This allows us to use data from one modality to train classifiers that generalize to the other categories, thus allowing us to train scene classifiers just from sentences or sketches. We show how decoders can be learned that invert features from the common representation into images of a specific domain. Future work includes learning improved decoders and using a better adaptation algorithm that originates more general common representations.
