...
%\title{Creating Task Representative Brain Graphs with Shallow Neural Networks\thanks{Supported by organization x.}}
%
...
%Many of these algorithms rely on feature extraction of fMRI data samples.


%Multi-Voxel Pattern Analysis (MVPA) and Region of Interest (ROI) approaches aims to find informative brain units and therefore decrease the spatial resolution of the brain image which can cause curse of dimensionality \cite{norman2006beyond}. General Linear Mixture(GLM) and Searchlight have been used along with Analysis of Variance(ANOVA) method to include spatial information to MVPA.

%Similarly, Gaussian Linear Mixture(GLM) models have been used to  Multi Voxel Pattern Analysis(MVPA) and Region of Interest(ROI) methods have been used to deal with the high dimensional nature of fMRI data by applying a feature selection on voxel time series.

%Different types of brain graphs have been
%Brain graphs have been a tool for brain decoding and 

%Brain graphs have applications in both neuro-science and machine learning applications on brain decoding. There are three main types of connectivity in brain graphs in the literature, namely structural, effective, and functional connectivity. Structural connectivity define physical pathways in the brain. Effective connectivity focuses on the influence of one region on another, where functional connectivity is defined as collaboration of anatomic regions during a cognitive process.



%With the use of brain networks in neuro-science, dysfunction and abnormalities in neurological disorders like schizophrenia and Alzheimer's disease \cite{lynall2010functional,menon2011large}. Brain graphs have been also used as a way of feature extraction for machine learning algorithms to classify cognitive states. The focus of these methods have been to design a graph in a way that the whole graph (nodes, edges and weights) would convey a meaningful representation of a neural process. The two main types of graphs that have been used in the literature of brain decoding are binary and weighted.

%Binary brain graphs have been used to ease the representation of weighted graphs. In binary graphs, only existence of a connection is noted. To create a binary graph, a threshold is applied to a directed or undirected weighted graph.


%As the concern of this study, we consider only weighted connections, and types of weighted connections are directed and undirected. Directed brain graphs defines directions for their edges as a mean of causality and they are mostly employed to describe effective connectivity in the brain. The edges of undirected brain graphs have no direction and have its application mostly to describe correlation of anatomic regions and therefore in functional connections.

%Various methods have been proposed to estimate the weights of functional brain networks to solve different types of brain decoding problems. \cite{richiardi2011decoding} calculated the pairwise Pearson correlation between regions in different frequency subbands in creation of functional undirected connectivity graphs to perform classification in an audio-visual experiment. \cite{firat2013functional} applied Levinson-Durbin recursion \cite{vaidyanathan2007theory} solve the linear relationship among voxels which results in directed functional networks as features to predict the memorized word in memory experiment. Similarly, \cite{alchihabi2018dynamic} applied Levison-Durbin recursion to obtain a dynamic brain network for every brain volume in Complex Problem Solving experiment to study activation differences between sub-phases of problem solving. \cite{onal2015modeling} estimated the linear relations with employing closed form ridge regression solution.

%Considering only pairwise relations in the brain like in \cite{richiardi2011decoding} ignores the locality property of the brain. \cite{alchihabi2018dynamic,firat2013functional} conserve the locality of the anatomic regions by applying a linear regression between a region and its neighbors, but estimation of a network for every sample discards temporal information of anatomic regions. Closed form solution of ridge regression, explained in more details in section 3.2, relies on the assumption of linear independence of features and hence can be prone to error in existence of correlation. Also, the mentioned method does not consider associativity in the resulting brain graph since the estimation processes of weights a node has kept separate. 

%In this study, we construct our algorithms based on the idea of \cite{onal2015modeling} by applying a ridge regression on activations an anatomic region's neighbors to estimate the relevant region's activation. Instead of using a closed form solution of ridge regression, we design two different neural network architectures to solve the linear relationship among the anatomic regions in a time interval of fMRI signals. Our first neural network architecture, which we call directed Artificial Brain Networks(dABN), is designed to solve regression in a way that the weights of the optimized model represent the weight of a directed brain graph while in purpose of our second model, undirected Artificial Brain Networks(uABN), is to estimate weights of undirected functional connections.
...
%{\color{red} We tested our approaches in two ... Burada architecture'lari HCP'deki tasklari ve  ve Tower of London daki surecleri estimate etmede kullandik. Diger methodlara (MVPA, pearson ve mesh variantlari) gore daha ustun performans elde ettik gibi 3-4 cumlelik bir paragraf yazmak gerekiyor.}
...
%{\color{red} w yu hem weight hem de window length icin kullanmissin. Length'i L olarak degistirdim.}
...
% Each graph, directed or undirected, considers the activation of all brain regions within a  time interval. Through a cognitive process, non-overlapping windows are used to create multiple graphs for that cognitive process. 

% These brain networks are generated for every non-overlapping sliding windows with a fixed window size,$W$ in a particular cognitive task. In this study, all created brain graphs are fully connected. Hence, the collaboration of every anatomic brain region is considered.

% Anatomic region activations are measured as average of Brain Oxygen Level Dependent(BOLD) intensity values of the voxels they contain. We denote the activation of an anatomic region with index $i$ at time $t$ as $v_i(t)$.
...
%The entries of the weight vector is estimated by assuming the following linear relationships among the anatomic region  $r_i$ with time series $\hat{b_i}$, for $i=1,....,N$ regions as follows:
...
%For a time period, weights of the edges,$a_{i,j}$ connected to a region node $v_i$, are regression coefficients to estimate the activation of region $v_i$,$\hat{v_i}$, for every time instant, $t= \{k, k+1, k+2,...k+W\}$.
...
%We minimize the following expected value of sum of squared error to estimate the edge weights,

% \begin{equation}
% E(\epsilon^{2}) = E(( b_i^{(t')} - \hat{b_i}^{(t')} ) ^2) \approx \frac{1}{L} \sum_{t'=t}^{t+L} ( b_i^{(t')} - \sum_{j\neq i,j=1}^{M} a_{i,j} b_j) ^2 ,
% \label{eq:mesh_loss}
% \end{equation


%E(\epsilon^{2}) = E(\sum_{t'=t}^{t+L} ( \hat{v_i}^{(t')} - v_i^{(t')} ) ^2) = \frac{1}{L} %\sum_{t'=t}^{t+L} ( v_i^{(t')} - \sum_{j=1}^{M} a_{i,j} v_j) ^2 
...
%To estimate the adjacency matrix, A, we try to apply similar connections in our neural network.  {\color{red}Bu cumle ne demek istiyor anlayamadim.}
...
\begin{equation}
% Loss(output_i) = E(\sum_{t'=t}^{t+L} ( v_i^{(t')} - \sum_{j\neq i,j=1}^{j=M} w_{i,j} v_j) ^2 + \lambda \sum w w^{T})
...
% add connections pairs ((i,j),(j,i)) where i!=j with different color
%(1,2)
...
% The loss equation is the same for directed graphs equation \ref{eq:dir_loss}. Since the half of the edges are shared on this neural network structure, the update rule is different from Equation \ref{eq:dirUpdate}. The weights of the neural network is initialized under the constraint that, $w_{ij} = w_{ji}$ and to preserve this symmetry, in every epoch the shared weights are updated as the following:
...
%In \cite{onal2015modeling}, functional connectivity estimations between a region and its neighbors are calculated by applying closed from solution of ridge regression. 
...
%The experiments in HCP considers anatomic level information, where Cerebellum and Vermis regions are discarded and 90 out of 116 anatomic regions are into consideration.
...
%After creating brain networks and forming the feature vectors from edge weights of the brain networks, we performed within-subject and cross-subject experiments using Support Vector Machines with linear kernel. {\color{red}In within-subjects experiments, we performed 3-fold cross validation using only the samples of a single subject.} Table \ref{table:within_itir} shows the average of within-subject experiment results over 808 subjects when the classification is performed using a single subjects brain networks of K tasks as features, $\{T_{i}^{s} \}_{i=1}^{K}$. {\color{red} Bu ifadeyi anlayamadim $\{T_{i}^{s} \}_{i=1}^{K}$'da K = 7 mi? T ile neyi kastediyoruz?}. {\color{red}In cross-subjects experiments, we performed 3-fold cross validation using the samples obtained from all of the 808 subjects. For each fold we employed the samples from .. subjects to train and .. subject to test the classifier.} Table \ref{table:across_itir} shows the cross-subject experiment results. %For all experiments k-fold cross validation is applied with $k=3$.
...
%{\color{red} Baran tablolara refer ettikten sonra sonuclarla ilgili cumleler yazman lazim.  Ornegin Tablo 3 gostermektedir ki dABN her lambda degeri icin best degeri vermektedir. undirected versiyonu slightly worse perform etmektedir. Ancak genel olarak ABN ler pairwise correlationdan ve ridge regressiondan iyi sonuc vermektedir gibi. Ayni seyi Tablo 1 ve Tablo 2 icin de yapmalisin.}

\section{Discussion and Future Work}

%directed > undirected > ridge > pearson
...
% The network properties of the generated brain networks can be studied,
