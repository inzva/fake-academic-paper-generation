...
\twocolumn[
% \icmltitle{Manifold-paced View for the Generalization of Deep Neural Networks}
...
\label{sec:related}
%%% We get a big savings in space by commenting out the following sentence. -MEH
% We briefly review recent work concerning the generalization behavior of DNNs, strategies for noisy label learning, and the use of dimensionality analysis in supervised learning.
...
% These techniques exploit the intrinsic geometry of the problem and the probability distribution that generates the data. Such analysis can be utilized to regularize the solution space or the underlying data manifold for optimal solutions, or domain adaption \cite{pan2011domain}, label propagation \cite{wang2008label} for semi-supervised learning. For example, an intrinsic geometry regularization over reproducing kernel Hilbert spaces (RKHS) was proposed in \cite{belkin2006manifold} to enforce smoothness of solutions relative to the underlying manifold. Another Laplacian-based regularization was proposed in \cite{belkin2004regularization} using the weighted neighborhood graph.

% {\color{red}  Address Michael's comment  and cite some papers "we need to keep in mind that intrinsic dimensionality has been used for regularization before in manifold learning before - this literature should be cited too (as a starting point, please check this out. \url{http://www.mit.edu/~9.520/fall15/slides/class14/class14_manifold.pdf}}
% {\color{red}  Daniel: found no impact paper on dimensionality regularization on supervised learning. do find manifold regularization, however, most of them are for semi-supervised learning. And the formulation of manifold is quite different to our subspace perspective. And we are not familiar with manifold topic, hard to write a good review for ICML. Overall, I think it's good not to include such reviews for the good of this paper.}
...
% By treating probability mass as a proxy for volume, classical expansion models provide a \textit{local view} of the dimensional structure of the data, as their estimation is restricted to a neighborhood around the sample of interest.
...
% % \noindent $F(r)$ is analogous to the volume $V$ in Equation~\eqref{eq:expansion_example}; however, we note that the underlying distance measure need not be Euclidean. 
% The last equality of Equation~\eqref{eq:LID_r} follows by applying L'H\^{o}pital's rule to the limits~\cite{houle2017local1}. 
...
% The last layer transformation $h^{(n)}$ is a softmax function applied on the $c$-dimensional output logits.

% Since the LID estimation model regards the distances from $x$ to the members of $X$ as determined by independently-drawn samples from a distribution $\mathcal{P}$, the estimator can also be applied to the distances induced by any random batch, as it too would be drawn independently from the same distribution $\mathcal{P}$. 

% Conversely, as the size of the batch is reduced, the variance of the estimates would increase. However, if the expanded dimensionality is sufficiently large, even an extremely small batch size and/or small neighborhood size could conceivably produce estimates whose difference is sufficient to reveal the dimensionality expansion of $x$. As we shall show in Section~\ref{sec:experiments}, such estimation is useful for batch sizes as small as 128, and for neighborhood sizes as small as 20.
...
%In summary, it seems reasonable to conclude that 1) the true data manifold is of relatively low dimensionality, 2) dimensional compression occurs as this data manifold is learned, and 3) dimensional expansion occurs with overfitting to noisy labels.
...
% Although the initial neighborhood of $x$ would likely consist of other points in the vicinity of $A$, the LID estimate associated with $x$ could be high or low depending on the distributional error at $x$ relative to $A$, or the distributional variation within $A$ in the vicinity of $x$. 
...
% Migration of a mislabeled point x t o the neighborhood of a new point y would change the profile of distances within that neighborhood. The general effect of such changes would tend to increase the LID estimate of y. The accommodation of new points x within the neighborhood of y would require an embedding of higher dimension, due to the distortion in the neighborhood distances. And yes, data of high ID is inherently harder to compress than data of low ID. This explanation will be added in the revision. -- Rebuttal response.
...
% Although LID estimation needs to compute the pairwise distance, it is computed within a batch rather than the entire dataset. Thus, the computation of LID is almost negligible compared to the overall training time: $t_{LID}/t_{training} \approx 1 - 2\%$. 
...
% and code is available at \url{xxxxxx}
% The reason for the exponential decay here is that we observed an exponential negative correlation of the expansion of manifold dimensionality and the validation performance of DNNs. 
% The reason behind the use of minimum LID score of previous epochs is that this is the simplest data manifold that can be learned by the network, any expansion from this point would lead to certain degree of overfitting.
...
% Parameter $\lambda$ allows more flexible control over the learning process, for example it can be set to $\lambda=i/T$ at the $i^{th}$ training epoch (assuming $T$ epochs in total) to indicates the increasing strength of D2L as training proceeds.  But in the experiments, we simply set $\lambda=1$.
...
% Please don't use negative array subscripts etc.
% For simplicity, new and gimmicky features of programming languages should be avoided. -MEH
\begin{algorithm}[tb]
...
% All networks were trained for 150 epochs using Adam optimizer with an initial learning rate of 0.001, $\beta_1=0.9$ and $\beta_2=0.999$.
...
% {\color{red}JAMES: are there any other papers which use strong noise?  Can we justify this is not a weird choice?}

% We use 1\% random training samples (corrupted as well) as validation set for parameter tuning and model selection. In all experiments, we chose the model with the lowest validation loss. The test accuracy (Top 1) of the networks trained using cross-entropy loss with clean labels is 99.24\% on MNIST, 91.02\% on SVHN, 91.73\% on CIFAR-10 and 68.59\% on CIFAR-100.
...
% Several baseline models are selected based on their robustness against noisy labels as well as their efficiency for training: 

% The ``forward" and ``backward" methods assume that the the noise model is available, therefore, we provide the models a transition matrix with the probability of one class flipped to the other. In practice, such information is often unavailable and should be estimated by a pre-trained network \cite{patrini2017making}. For ``boot-soft" and ``boot-hard" we employ the suggested setting in the original paper \cite{reed2014training}: the weights for the raw/predicted labels are 0.8/0.2 for boot-hard and that for the raw labels/predictions (probabilities) are 0.95/0.05 boot-soft. 
...
% from a perspective of 1) the dimensional complexity of the learned data manifold, 2) the complexity of the learned hypothesis and 3) the quality of the learned deep representations.
...
% We also notice that `forward' method achieved better test accuracy and simpler subspaces than other baselines. This is partially because the model is aware of the noise model, which may not be available in real-world noisy data. 
...
%Traditional complexity measures, \textit{e.g.}, VC-dimension \cite{vapnik1998statistical}, norm/margin-based measures \cite{bartlett2002rademacher}, uniform stability \cite{bousquet2002stability} and Sharpness \cite{keskar2016large}, have been questioned in their ability explaining the generalization behavior of DNN \cite{zhang2016understanding,neyshabur2017exploring}. 
...
% Combining the hypothesis complexity shown here with the test accuracy shown in Figure \ref{fig:acc}, we can confirm that simpler hypothesis does lead to better generalization. This also implies that our proposed strategy is capable of learning more smooth decision boundaries than other models. This is because our learning strategy learns the low-dimensional true manifold rather than overfitting to the high-dimensional corrupted manifold. {\color{blue} Sarah: did we use the same 40\% noise in this experiment?}
...
% \multicolumn{2}{c|}{Dataset / Noise Rate} & cross-entropy & forward & backward & boot-hard & boot-soft & D2L  \\ \hline
% \multirow{5}{*}{MNIST} 
% & 0\% & 99.24$\pm$0.06 & 99.26$\pm$0.01 & 99.29$\pm$0.03 & 99.24$\pm$0.07 & 99.25$\pm$0.02 & \textbf{99.30$\pm$0.02}  \\
% & 20\% & 88.04$\pm$0.05 & 97.78$\pm$0.02 & 90.92$\pm$0.07 & 88.31$\pm$0.09 & 87.99$\pm$0.13 & \textbf{98.83$\pm$0.05}  \\
% & 40\% & 67.89$\pm$0.07  & 94.26$\pm$0.06 & 76.48$\pm$0.06  & 69.05$\pm$0.04 & 67.89$\pm$0.07 & \textbf{98.81$\pm$0.21}  \\
% & 60\% & 45.71$\pm$0.09 & 83.48$\pm$0.07 & 78.63$\pm$0.14 & 46.71$\pm$0.16 & 45.91$\pm$0.11 & \textbf{98.52$\pm$0.13}  \\ \hline
% \multirow{5}{*}{SVHN}
% & 0\% & \textbf{91.02$\pm$0.09} & 90.85$\pm$0.03 & 90.73$\pm$0.07 & 90.72$\pm$0.11 & 90.92$\pm$0.17 & 90.79$\pm$0.09 \\
% & 20\% & 78.12$\pm$0.17 & 85.72$\pm$0.22 & 77.93$\pm$0.30 & 78.59$\pm$0.14 & 77.79$\pm$0.31 & \textbf{86.64$\pm$0.19}  \\
% & 40\% & 60.26$\pm$0.35  & 78.39$\pm$0.16 & 63.93$\pm$0.21 & 61.11$\pm$0.31 & 59.46$\pm$0.21 & \textbf{84.10$\pm$0.18}  \\
% & 60\% & 38.48$\pm$0.41 & 62.24$\pm$0.32 & 50.45$\pm$0.51 & 40.17$\pm$0.83 & 38.86$\pm$0.51 & \textbf{82.57$\pm$0.22}  \\ \hline
% \multirow{5}{*}{CIFAR-10} 
% & 0\% & \textbf{91.73$\pm$0.11} & 91.09$\pm$0.09 & 90.61$\pm$0.08 & 91.19$\pm$0.09 & 91.11$\pm$0.12 & \textbf{91.73$\pm$0.11}  \\
% & 20\% & 72.31$\pm$0.15  & 85.67$\pm$0.13 &79.26$\pm$0.20 & 79.06$\pm$0.17 & 69.35$\pm$0.31 & \textbf{86.13$\pm$0.21}  \\
% & 40\% & 66.30$\pm$0.63  & 71.04$\pm$0.49 & 80.10$\pm$0.76 & 77.92$\pm$0.82 & 67.01$\pm$1.02 & \textbf{84.13$\pm$0.66}  \\
% & 60\% & 56.56$\pm$0.71  & 71.33$\pm$0.59 & 31.32$\pm$0.83 & 57.99$\pm$1.20 & 55.28$\pm$0.97 & \textbf{72.84$\pm$0.85}  \\
% \hline
% \multirow{5}{*}{CIFAR-100} 
% & 0\% & 68.59$\pm$1.05 & \textbf{68.68$\pm$0.94} & 68.48$\pm$1.31 & 68.45$\pm$1.07 & 67.34$\pm$1.33 & 68.60$\pm$1.21  \\
% & 20\% & 62.46$\pm$1.21 & 64.25$\pm$1.19 & 58.72$\pm$1.33 & 58.49$\pm$1.51 & 56.21$\pm$1.43 & \textbf{65.09$\pm$1.29}  \\
% & 40\% & 53.85$\pm$1.30 & 57.27$\pm$1.41 & 45.42$\pm$1.41 & 46.44$\pm$1.63 & 43.32$\pm$1.52 & \textbf{59.54$\pm$1.39}  \\
% & 60\% & 39.21$\pm$2.72 & 44.75$\pm$1.34 & 30.49$\pm$2.57 & 42.11$\pm$3.50 & 40.18$\pm$2.65 & \textbf{48.38$\pm$2.38}  \\
% \hline
% \end{tabular}
% %\vspace{-0.2in}
% \end{table*}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% Removed for this version - a more detailed investigation is needed? -MEH
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%\textbf{Analysis of Neighborhood Shift:}
%In order to gain more insights into the subspace dimensionality and neighborhood dynamics during the learning process, we keep track of the changes in neighborhood membership of a mislabeled sample and show how it may affect the dimensionality of its local subspace. The experiment was conducted on CIFAR-10 with 40\% label noise, for the same 12-layer CNN as used in previous experiments. In Figure \ref{fig:turning_point} we show a mislabeled training sample (blue cross) and its 30 nearest neighbors, which are categorized according to whether they are from the same class as the mislabeled sample (the `True' group), or possess the same labels as the mislabeled sample (the `False' group), or do not fall into either of the other two categories (the `Other' group). The neighborhoods were shown before (at epoch 20) and after (at epoch 60) the turning point of dimensionality compression and expansion determined by D2L (around epoch 40, as shown in Figure \ref{fig:lid_cifar}), based on the 2D t-SNE embeddings at layer 11. 

%As can be seen, the mislabeled sample was locally surrounded by more samples from the True group before the turning point; however, after the turning point, the local subspace in the vicinity of the mislabeled sample had comparatively more samples from the False or Other groups. We confirmed that this is a common phenomenon for learning of mislabeled training samples. This is in line with our analysis that learning with mislabeled samples would inevitably involves subspace migration, which thus leads to dimensionality expansion. This is substantiated by the increase in LID score (of the mislabeled sample) from 3.26 to 3.92 before and after the turning point.
...
%\centering
%\small
%\includegraphics[width=0.98\linewidth]{figures/neighborhood_ce_cifar-10_40.png}
%\caption{Neighborhood dynamics before (at epoch 20) and after (at epoch 60) %the turning point (around epoch 40) from dimensionality compression to expansion on CIFAR-10 with 40\% noisy labels.}
...
%\textbf{Complexity Analysis:} Although LID estimation needs to compute the pairwise distance, our LID is computed within a batch rather than the entire dataset. Thus, the computation of LID is almost negligible compared to the overall training time: $\frac{t_{LID}}{t_{training}} = 1 - 2\%$. 
...
% All networks were trained using the Adam optimizer with an initial learning rate of 0.001, $\beta_1=0.9$ and $\beta_2=0.999$. The networks were trained for 50 epochs for MNIST/SVHN datasets, 120 epochs for CIFAR-10, and 150 epochs for CIFAR-100.
...
% In particular, D2L achieves an accuracy of 98.52\% (99.30\% for purely clean labels), 82.57\% (90.79\% for purely clean labels), 72.84\% (91.73\% for purely clean labels) and 48.38\% (68.60\% for purely clean labels) even with 60\% noisy labels on MNIST, SVHN, CIFAR-10 and CIFAR-100 respectively. 
%Such strong performance substantiates the robustness of our proposed learning strategy against noisy labels.
...
% It would also be interesting to see how the local subspace dimensionality is related to transfer learning and domain adaptation.
%
...
%(in addition to label noise) 
can affect local subspace dimensionality and DNN learning behavior.
%, where new findings could eventually lead to improved denoising techniques or more robust learning models, or a better understanding of how data augmentation can contribute to the generalization capacity of DNNs. 
% Further investigation of the dimensional properties of deep learning models in semi-supervised or unsupervised learning settings may also be worthwhile.

% Such an understanding would be of great potential for a wide range of applications such as denoising techniques, semi-supervised learning, domain adaption as well as learning with novelty including zero-shot and one-shot learning.
% semi-supervised learning,
% input noise
% sample-wised dimensionality analysis

%{\color{red} Paper limit: 8 pages of main content + 2 pages of references. }
