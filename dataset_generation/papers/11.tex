% Use class orption [extendedabs] to prepare the 1-page extended abstract.
...
% Definitions for frames (biorthogonal)
...
% Definitions for scalar variables
\newcommand{\xs}{\ensuremath{x}}        % 

% Definitions for vector variables
...
% Definitions for matrix variables
...
% More definitions for posterior mean, covariance, and their info form analogs
\newcommand{\mup}{\muv}
...
% we set out to solve all of them with a common architecture. 
...
%  become the mainstream of computer vision research. 
...
%The breadth of this (incomplete) list clearly indicates the breadth of the potential of CNNs.
% Even though all such problems are typically tackled by training task-adapted variations of the original network, 
 ...
 %This can be useful for instance if a large amount of cheaply available low-level supervisory signals is available (e.g. for colorization \cite{ZhangIE16}, normal estimation, inpainting \cite{pathakCVPR16context}) and we use these to bootstrap a network that can eventually solve high-level tasks.
...
%if one were to introduce a new task, that particular 
%it might not want to relabel that particular dataset. 
...
%^We are currently in the process of integrating the tasks of (vi) symmetry detection (vii) depth estimation  and (x) viewpoint estimation, establishing a full vision decathlon.
...
%As shown repeatedly in works on visualizing 
%Even though a thorough review of the relevant literature over the past few years would be a daunting task; but we can consider 
%As works that relveal the span of computer vision tasks that have succesfully been tackled by CNNs, 
...
%While most of the works mentioned in the introduction rely on fine-tuning some common pre-trained CNN, several of them  introduce task-specific processing steps; for instance tasks which require labelling at high spatial resolution may use  deconvolutional layers or
%E ven though this task-specific processing can yield systematic improvements, we omit it for the sake of simplicity;  

%As shown in \reffig{fig:multi-task} we adopt an architecture that combines several standard ideas, detailed below.
...
\begin{figure*}
	...
	%{
	% \centerline{	
	%\includegraphics[width=.59\linewidth]{./F2.png}\\
	%}
	%\input{psGraphics}
	%\includegraphics[width=.99\linewidth]{./asd1bw.pdf}
	%}
	\begin{center}
...
%; this has been systematically validated in \cite{hed} where it was shown that every single layer adds something to the performance of a network trained for the low-level task of boundary detection. 
...
We also use {{convolution with holes}} (\`a trous) \cite{PapandreouKS15,ChenPK0Y16} which allows us to control the spatial resolution of the output layer. In particular we use \`a trous convolution to obtain an output stride of 8 (rather than 16), which gives us a moderate boost in tasks such as boundary detection or semantic segmentation. 
 	% to decouple the  calibration of the norm of the layer-specific weights, $\weight ^T_{t,k}$ (second step) from 
 	%Something on concatenating and then adding up.
 	
%, even though the final output can be written as 
...
% We intend to remove this exception by integrating advances from \cite{DaiLHS16}. 
...
% allows us to repeatedly search for 
%There is  clearly potential for improvements in performance by using additional layers, and domain-specific layers, but  we have preferred to keep the network simple and address instead problems that related to scaling-up multi-task training to many and diverse tasks. 
%, which still makes scaling up to many tasks is challenging %. Having resolve the problems related to scaling up, we intend to explore these extensions in future work. 
%We now describe the key deviations of our architecture with respect to the most standard,.
%In order to achieve high spatial resolution combined with skip-layers  In order to deal with variabilities in image scale we use a pyramidal-type processing, as  - at 
%combinations with boundary detection in \cite{edg},
%
%for fine resol
%, we consider it safe to say that for all of these a reasonally strong 
% is a large variety of task-specific architectures 
%We attribute this success to the use of (i) side layers and deep supervision, as proposed in \cite{hed} (ii) the use of a multi-scale architecture that is trained end-to-end, as proposed in \cite{iclr16} and (iii) the use of batch-normalization at the side layers, and (iv) the use of deeper layers for dense labelling, in a `fully convolutional' manner. (iii) and (iv) have already been used e.g. for semantic segmentation in  and \cite{deeplab} respectively, but we are now employing them also for low-level tasks.
...
%In particular we use a common architecture for the tasks of  semantic segmentation and object recognition, as shown in \reffig{fig:multi-task}. 
...
%We also adapt our experimental findings from that work to the task at hand, involving 
%For this we build on our parallel work on super-human boundary detection and integrate it into a single network that solves all of these tasks jointly through a multi-scale architecture. 

% \caption{Network architecture used for multi-resolution HED training: three differently scaled versions of the input image are provided as inputs to three FCNN networks that share weights - their multi-resolution outputs are fused in a late fusion stage, extending DSN to multi-resolution training. 	 \label{fig:hedtrain}} \end{figure*}


%As illustrated in \reffig{fig:multi-task} 
%While starting from a common basis, we  have in the end two different  processing streams, indicated by different colors ( magenta for semantic segmentation, blue for  object detection). 

%The mapping from the convolutional layers, shown in blue boxes, to the final desired outputs is implemented through `convolutionally implemented' fully-connected layers \cite{sermanet2013overfeat,LongSD14,PKM15}, shown as grey boxes. 


% so as to have a 1-1 mapping from pix

%Region proposals, indicated by the  `Region-of-Interest' (ROI) square are generated by processing the boundary maps delivered by our system. 
...
\label{diverse}
 ...
 \newcommand{\losst}[3]{L_{#1}\left(#2,#3\right)}
  ...
  %
%\newcommand{\lossdd}[4]{\frac{\partial L_{#1}\left(#2,#3\right)}{\partial #4}}
...
%onsidering for instance that our network is 
%Even though it was relatively straightforward to train the low- and high- level task networks in isolation, 
...
This however can be become challenging if one wants to jointly tackle problems for which no common dataset exists.}
 %
%The same could be said for the ImageNet dataset versus the MSCOCO/PASCAL datasets. 
%
%In order to handle this situation we train with a loss function during training that adaptively penalizs
%
...
%To pin things we write below our training objective, and describe below how this is optimized through back-propagation. 
...
%To simplify notation we have omitted  task-specific scaling factors for the losses, that can calibrate the importance of the different tasks and regularization terms; these are additional hyperparameters that we set empirically, but further improvements could be obtained through a proper cross-validation.
...
%, to ensure that at any point the resulting stochastic  gradient estimates are unbiased.

%We consider first the simpler case where $\mydelta{\task}{i}=1$, which corresponds to all images having annotations for all tasks. 
...
%\COMMENT{minibatch construction}  
...
%I NEED TO TAKE CARE OF SCALING FACTORS
...
%we sum over training samples that contained information 
%set of indexes 
%DIFFERENT BATCHSIZES
% - which happens synchronously on a single GPU.



%(0) 
%the fused result is a linear \hhn{T},\prd{k}{T},\raw{T})


%\Loss_{T} = \Loss(\prms{T},\hhn{T})  = \sum_{k=1}^{K} 
%\Loss_{Direct}(F(\prd{k}{T},\vis{k}) + \Loss_{fuse}(\hhn{T},\prd{k}{T},\raw{T})\\

%\Loss(\wm{,\wmm,\hh) = \sum_{k=1}^{K} \Loss_{side}(\ww,\wmm) + \Loss_{fuse}(\ww,\wmm,\hh), \nonumber

%Firstly, we tie the parameters used for the different predictions, i.e. we set $\prm{k}{T} = \prms{T}$,

%Secondly, as in the the DSN/HED work of \cite{HED,DSN} that we used for boundary detection, here too 

% \mathrm{Boundary},Segment,Object\}$, 
...
% - which means one does not free up the memory once a gradient signal is used in the first time. 

\newcommand{\nl}{L}

%, where $T$ is th  number of tasks we wish to learn. 
...
%To pin this down, we consider for simplicity that every layer requires $\nbytes$ of memory for its activations, and gradient signals - 
...
% Applying these algorithms to our case would result in  a memory complexity  in the order of $\sqrt{\nlcnn}   +  T \sqrt{\nltask}$, which was still too high for the networks we use (even for $T=1$, current systems relying e.g. on ResNets are on the limits of high-end, 12GB GPUs).
 
% As we show however, we can exploit the decomposable form of our loss function and extend these algorithms to further reduce the complexity to $\sqrt{\nlcnn}  + \sqrt{\nltask}$, allowing us effectively to handle an unlimited number of tasks.
%We start by presenting the idea behind the existing algorithms, and then turn to describing how we extend it to our case. 
 


%This however results in memory demands that increase linearly in the number of network layers, and linearly also in the number of tasks. 
%To pin this down, we consider for simplicity that every layer requires $\nbytes$ of memory for its activations, and gradient signals - if we have say $\nlcnn$ layers for the shared cnn trunk and $\nltask$ layers per task, the memory complexity of a naive implementation would be $2 \nbytes (\nlcnn   +  T \nltask )$, where $T$ is the number of tasks we wish to learn. 

%We build on recent advances on reducing the large $\nlcnn$ or $ \nltask$ factors to square-root memory complexity.

%\newcommand{\wdf}{.45\textwidth}



%factor 10 		& 			& 	57.4 	&	49.2 	& 0.797 	& 50.694 	& 0.8409	& 65.206 	& 65.206 		\\\hline

%08-14_22-33-13

% and extend them to handle the effect of large $T$. We end up with an algorithm that, bitrarily large values of $T$. 
%Recent works \cite{ 


%Clearly, a large number of 
%We also not that high-resolution architectures, and pyramidal implementations of dense labelling tasks are quite demanding in memory on their own, so having a linear growth in the number of tasks quickly 
%The memory de
...
% (even though one can also anticipate multi-task training to yield  suggest 
%) freely be set by a single 
...
%train the single-task networks with the subset of the multi-task network training set that pertains to the particular task.  

%For this the training  objectives and settings for the single- and multi- task training need to be as comparable as possible.
%Regarding having comparable objectives, we use as training set for the single-task networks the task-related images of the united training set used for multi-task training - thereby ensuring that the  task-specific loss is identical.
%Regarding having comparable setups, We detail this in the following.
...
%We  finetune with 10000 iterations, to ensure we have a good starting point for training our multi-task or single-task experiments. 
%Even though this network was trained only to serve as a starting point for the remaining experiments, our results give a positive signal in favor of multi-task training, as described below. 

\textbf{Datasets: } 
%We use networks pretrained on MS-COCO, as detailed above, but do not  use MS-COCO for finetuning. 
...
%we note that the large size of the NYU, MSRA and BSD datasets when united could mean that the high-level tasks be

%For finetuning the high-level tasks (detection, semantic segmentation, semantic boundaries, human parts) we use the PASCAL VOC 2007 trainval dataset (5011 images) and the PASCAL VOC2012 training set (5717 images). The VOC2012 validation set (5823 images) is  used for 

%\newcommand{\hspt}{@{\hspace{5pt}}}
...
\begin{table}
	\begin{center}
			\resizebox{\linewidth}{!}{
				%\begin{tabular}{l@{}c\hspt c\hspt c\hspt c\hspt c\hspt cc}
...
%to ensure that we can replicate the results of \cite{Girshick15}, but not elsewhere, since this is where human parts and semantic boundaries are validated on. 
%
%This could be resulting in some systematic bias against our presented results on semantic segmentation and object detection. For saliency we 



%\renewcommand{\rt}{/Users/iasonas/Dropbox/ubernet_im}
\newcommand{\resfig}[1]{
...
%One of our main concerns has been to ensure that we have high performance for the task of object detection, since it is one of the  dominant research problems in computer vision. 
%
...
%When training our `seed' network, as described above, we set it to have in the beginning the parameters of yjr  When run `out of the box' the two networks should therefore have identical performance. This is verified in the `Ours, 1-Task' row, where we
\begin{table}[!h]
...
\begin{table}[!h]
	...
	Method	& mean IoU \\\hline\hline
		%Best & 75.896  \\\hline
		%Validation &  75.683\\\hline
		%%factor 10 & 65.206 \\\hline
		%no Aug &  75.266\\\hline
		%repd x 3 &  74.561 \\\hline
		%fct 10 &  69.687 \\\hline
		%nrm10 & 69.174 \\\hline
		...
		%Deeplab V2 -CRF \cite{ChenPK0Y16} & 79.7 \\\hline
		Ours, 1-Task  & 72.4\\
		Ours, 2-Task  & 72.3\\
		%Ours, 5-Task & 69.7 \\
...
%The architectures are very similar, with the difference that we now also use skip layers, and initialize from the two-task network. 
...
%We  also include the performance of the ResNet-based network of \cite{ChenPK0Y16} to highlight the kind of improvements we can anticipate by using deeper architectures, which is one of our immediate next goals. 
 ...
 \begin{table}[!h]
 	\begin{tabular}{|l|c|c|c|}
 		...
 		% %Deeplab V2 -CRF \cite{ChenPK0Y16} & 62.76 \\\hline
 		Ours, 1-Task & 51.98\\\hline
 		Ours, 7-Task & 48.82 \\\hline
 		%Best & 48.833  \\\hline
 		%Validation &  47.679\\\hline
 		%factor 10 & 50.694 \\\hline
 		%no Aug &   51.781 \\\hline
 		%repd x 3 &  52.199 \\\hline
 		%fct 10 &  53.676 \\\hline
 		%nrm10 & 51.741 \\\hline
 	\end{tabular}
 ...
 %Still, using a deeper ResNet, as in \cite{ChenPK0Y16} yields even larger improvements - we anticipate to obtain similar improvements in our case as well. 
 %Turning to evaluating our method, 
...
%Segdet & 72.28670
%Seg single scale &  69.92
%Lon5, lob2 & 68.07810 
%Lon5, lob2 & 68.23830
%s3 Lon5, lob2 & 68.57540
%all_repd3_trainval_lon_10 & 66.74550
%all_repd_2_nrd_lon_10  & 66.73510
...
\textbf{Semantic Boundary Detection:}
 ...
 %  This can skew the network to reflect the annotator behavior in VOC2010, rather than BSD, and thereby harm a bit performance, when assessed on BSD.
  %For instance for boundaries we use $\simeq 10K$ images from BSD and $\simeq 20K$ images for VOC2010, since this is the proportion of datasets used for the multi-task network. 
  %however we do not use DSN, or even graduated-DSN \cite{iclr16} losses for the skip layers, since this turned out to harm performance in certain tasks. 
  ...
  \begin{table}[!h]
 	\begin{center}
 			...
 			\begin{tabular}{|l|c @{\hsps} c @{\hsps} c @{\hsp} c @{\hsp} c|}
 				\hline		 
 				%& \multicolumn{2}{|c|}{Angle Distance} & \multicolumn{3}{|c|}{Within t degrees} 	\\
 			Method	&  Mean &  Median &  $11.25^{\circ}$ & $22.5^{\circ}$ & $30^{\circ}$  \\\hline\hline
 				...
 				%Ours, 5-Task 	& 24.6 & 19.1  & 27.7 & 57.3 & 70.2\\\hline
...
Even though our multi-task network's performance  is not too different from the plain CNN-based result of \cite{WangFG15}, it is clear that there we have a somewhat unique gap in performance, when compared to what we seen in the remaining tasks. 
 ...
 % of the normal estimation tasks  Learning nonlinear functions on top of these skip layers, as e.g. in \cite{Chen2015iclr,BansalRG16} is possible, but if memory is a concern, these need to factorize over  the individual layers, as in \cite{Chen2015iclr}; we leave this as an option for  the future. 
 
 

% We provide now some details on how we obtain this initialization:
 
%We therefore need to ensure that the
%In order to examine this w
%  firstly  and secondly
% This will ensure that the single- and multi- task training objectives will be as similar as possible.  
...
%of the 
%$\mathrm{fc6,fc7}$
%of the semantic segmentation branch uses 
%. Since we want to compare to strong baselines, we use a particular %initialization which allows us to score high on all tas
%, a 
%e want to compare to strong baselines, and therefore use networks that have been pre-trained on MS-COCO. 
%of a `frankenstein' sort \cite{} in the sense that it blends
% that has been trained so as to satisfy both a dense labelling task, such as semantic segmentation and the region-based object detection task.
%this  


%In particular, we note that apart from $\gamma_t$ in \refeq{eq:objective}, the relative importance of a task is also implicitly determined by the number of images containing ground-truth inforrmation for the task - in particular, we have a factor $\frac{1}{N}\sum_{i=1}^N  \mydelta{\task}{i}$ which diminishes the effect of tasks for which we have limited ground truth annotations. This effect can be compensated for by redifining $\gamma'_{\task} = \gamma_{\task}\frac{N}{\sum_{i} \mydelta{\task}{i}}$  in \refeq{eq:objective}. 
...
%and accomplishing many tasks may come at the cost of decreasing the average performance. 
\mycomment{
\begin{table*}[!h]
	\resizebox{\textwidth}{!}{
...
%, we can expect that the network will learn nothing about it, while if the we 
%The experimental results described above suggest that even though for the case of a few tasks multi-task training can  yield even improvements over the case of single-task training 
...
%{
	\begin{figure*}
			{\setlength{\tabcolsep}{.1em}	
				%\begin{tabular}{c@{}c@{}c@{}c@{}c@{}c@{}c@{}c}
			\begin{tabular}{cccccccc}
		%\tf{ Input} & \tf{ Boundaries} & \tf{ Normals} & \tf{Saliency} & \tf{Sem. Boundaries} & \tf{Sem. Segmentation} & \tf{Human Parts} & \tf{Object Detection}\\
		%\resline{1}
		%\resline{3}
...
%while  tasks which come with their own dataset (e.g. saliency, or normals in this work) can be learned on equal grounds with tasks that use a large common dataset (PASCAL VOC for the remaining tasks in this work). 
...
%In future work we intend to explore how the flow of information across tasks can be exploited, either with feedforward or recurrent connections, as well as how the tasks can be combined with structured prediction, along the lines of \cite{ChandraK16}.

%An online demonstration of the system is available from \url{http://cvn.ecp.fr/ubernet} - all of the training and testing code  will soon be made publicly available to accelerate progress in this direction.
